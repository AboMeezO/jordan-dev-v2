...
const MIDDLEWARE_HINT =
	/headers\.set\(\s*["']Content-Security-Policy["']/i;
const META_TAG_HINT =
...
export function detectCsp(cwd = process.cwd()) {
	...
	walk(cwd, cwd, 0, (absPath, relPath, body) => {
		...
		if (
			SCAN_EXTS.has(ext) &&
			/packages\/[^/]+\/src\/.*(config|next-config|security)/.test(
				relPath,
			) &&
		...
		if (
			SCAN_EXTS.has(ext) &&
			/(^|\/)(next|nuxt|vite|astro|svelte)\.config\./.test(
				relPath,
			) &&
	...
	if (hits.appendArrays.length > 0) {
		return {
			shape: "append-arrays",
			signals: hits.appendArrays,
		};
	}
	if (hits.appendString.length > 0) {
		return {
			shape: "append-string",
			signals: hits.appendString,
		};
	}
