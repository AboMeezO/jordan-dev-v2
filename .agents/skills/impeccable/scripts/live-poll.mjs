...
const SELF_DIR = path.dirname(
	fileURLToPath(import.meta.url),
);
const scriptCmd = (name) =>
	`node "${path.join(SELF_DIR, name)}"`;
	...
	if (dataIdx !== -1 && dataIdx + 1 < args.length) {
		try {
			...
			const wrapped = new Error(
				"--data must be valid JSON: " + err.message,
			);
...
export async function postReply(base, token, reply) {
	const res = await fetch(`${base}/poll`, {
		...
		body: JSON.stringify(
			buildPollReplyPayload(token, reply),
		),
	});
	if (!res.ok) {
...
export async function augmentEventWithAcceptHandling(
	event,
	base,
	token,
) {
...
export function buildAcceptScriptArgs(event) {
	...
	if (
		...
		scriptArgs.push(
			"--param-values",
			JSON.stringify(event.paramValues),
		);
	}
	return scriptArgs;
}
...
export async function runPollStream(
	...
	while (shouldContinue()) {
		const event = await fetchNextEvent(base, token);
		await augmentEventWithAcceptHandling(
			event,
			base,
			token,
		);
		...
		if (requiresAgentReply(event)) {
			const acked = await waitForEventAck(
				base,
				token,
				event.id,
				{
...
Harness note:
  ...
  --stream is experimental for harnesses with fast incremental stdout; do not use on Cursor.`);
	...
	const ackTimeoutArg = args.find((a) =>
		a.startsWith("--ack-timeout="),
	);
