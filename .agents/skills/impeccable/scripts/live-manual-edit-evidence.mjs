	...
	for (const entry of entries) {
		...
		for (const op of entry.ops || []) {
			out.push({
				...
				classes: Array.isArray(op.classes)
					? op.classes
					: [],
...
function buildContextHintsByRef(entry) {
	...
	for (const op of entry.ops || []) {
		...
		if (typeof entry.element?.textContent === "string") {
			for (const chunk of entry.element.textContent.split(
				/\s{2,}|\n|\t/,
			))
				add(chunk);
		}
...
function buildCandidatesForOp(op, cwd, searchFiles) {
	...
	return {
		...
		contextTextMatches: findContextMatches(
			searchFiles,
			contextNeedles,
			{
...
function normalizeSourceHint(hint) {
	...
	let line = Number.isFinite(Number(hint.line))
		? Number(hint.line)
		: null;
...
function findLiteralMatches(searchFiles, needle, { max }) {
	return findMatches(searchFiles, needle, {
		kind: "text",
		max,
	});
}
...
function findLocatorMatches(searchFiles, op, { max }) {
	...
	for (const { kind, needle } of needles) {
		for (const match of findMatches(searchFiles, needle, {
			kind,
			max,
		})) {
			const key =
