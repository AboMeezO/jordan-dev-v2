...
function resolveSerif(fontFamily) {
	...
	const primary =
		tokens.find((f) => f && !GENERIC_FONTS.has(f)) || null;
...
function checkHtmlPatterns(html) {
	...
	while ((im = imgTagRe.exec(html)) !== null) {
		if (
			/\bhover:(?:scale|rotate|translate|skew)-/.test(im[1])
		) {
			findings.push({
				id: "image-hover-transform",
			...
			if (anyVisible) {
				...
				const pad = {
					top:
						...
						0,
					right:
						resolveLengthPx(style.paddingRight, fontSize) ??
						0,
					bottom:
						resolveLengthPx(
							style.paddingBottom,
					...
					left:
						resolveLengthPx(style.paddingLeft, fontSize) ??
						0,
				};
...
function checkElementQualityDOM(el) {
	...
	const hasDirectText = [...el.childNodes].some(
		(n) =>
			n.nodeType === 3 && n.textContent.trim().length > 10,
	);
...
function checkElementHeroEyebrow(
	...
	return checkHeroEyebrow({
		...
		siblingFontSize,
		siblingLetterSpacing:
			resolveLengthPx(letterSpacingRaw, siblingFontSize) ||
			0,
...
function checkCreamPalette(doc, win) {
	...
	const body =
		doc.body ||
		(doc.querySelector ? doc.querySelector("body") : null);
...
function positionedChildHasSubstantiveContent(child) {
	...
	if (typeof child.matches === "function") {
		try {
			if (
				child.matches(POSITIONED_CHILD_INTERACTIVE_SELECTOR)
			)
				return true;
		} catch {}
	}
