...
(function () {
	...
	const ANTIPATTERNS = [
		...
		},
		{
			...
			skillSection: "Color & Contrast",
			skillGuideline:
				"cream and beige as the default surface",
		},
		{
			id: "nested-cards",
			category: "slop",
		...
		},
		{
			...
			skillSection: "Typography",
			skillGuideline:
				"oversized italic serif as the hero headline",
		},
		{
			...
			skillSection: "Typography",
			skillGuideline:
				"tiny uppercase tracked label above the hero headline",
		},
		{
		...
		},
		{
			...
			skillSection: "Typography",
			skillGuideline:
				"letter spacing crushed past legibility",
		},
		{
			id: "broken-image",
			category: "quality",
		...
		},
		{
			...
			skillSection: "Layout & Space",
			skillGuideline:
				"inside bordered or colored containers",
		},
		{
		...
		},
		{
			...
			skillSection: "Layout & Space",
			skillGuideline:
				"overflow container clipping positioned children",
		},
		{
			...
			skillSection: "Typography",
			skillGuideline:
				"font family outside the project design system",
		},
		{
			...
			skillSection: "Color & Contrast",
			skillGuideline:
				"literal color outside the project design system",
		},
		{
			...
			skillSection: "Visual Details",
			skillGuideline:
				"border radius outside the project design system",
		},
		...
		// ── Provider tells: opt-in via --gpt / --gemini (gated off by default) ──
		{
			...
			skillSection: "Visual Details",
			skillGuideline:
				"hairline border plus wide diffuse shadow",
		},
		{
	...
	function parseGradientColors(bgImage) {
		...
		for (const m of bgImage.matchAll(
	...
	function isEmojiOnlyText(text) {
		...
		return (
			text.replace(EMOJI_CHARS_GLOBAL, "").trim() === ""
		);
	}
	...
	function checkColors(opts) {
		...
		if (hasDirectText && textColor && !isEmojiOnly) {
			...
			if (bgs) {
				...
				const ratios = bgs.map((b) =>
					contrastRatio(textColor, b),
				);
				let worstIdx = 0;
				...
				const isLargeText =
					...
					(fontSize >= WCAG_LARGE_BOLD_TEXT_PX &&
						fontWeight >= 700);
		...
		if (classList) {
			...
			if (
				/\bfrom-(?:purple|violet|indigo)-\d+\b/.test(
					classStr,
				) &&
	...
	function checkIconTile(opts) {
		...
		if (
			iconChildWidth &&
			iconChildWidth >= siblingWidth * 0.95
		)
			return [];
	...
	function resolveSerif(fontFamily) {
		...
		const primary =
			tokens.find((f) => f && !GENERIC_FONTS.has(f)) ||
			null;
	...
	function isAccentColor(cssColor) {
		...
		const hslM = /hsla?\(\s*[\d.]+\s*,\s*([\d.]+)%/i.exec(
			s,
		);
		if (hslM) {
	...
	function checkHeroEyebrow(opts) {
		...
		const isClassicTracked =
			isUppercased && siblingLetterSpacing >= 1.6;
		...
		const isAccentBold =
			weight >= 700 && isAccentColor(siblingColor || "");

		if (!isClassicTracked && !isAccentBold) return [];

		const headingTextSnippet = (headingText || "")
			.trim()
			.slice(0, 60);
	...
	function checkMotion(opts) {
		...
		if (
			animationName &&
			animationName !== "none" &&
			/bounce|elastic|wobble|jiggle|spring/i.test(
				animationName,
			)
		) {
			findings.push({
		...
		if (timingFunctions) {
			...
			let m;
			while (
				(m = bezierRe.exec(timingFunctions)) !== null
			) {
		...
		if (
			...
			const layoutFound = props.filter((p) =>
				LAYOUT_TRANSITION_PROPS.has(p),
			);
	...
	function checkGlow(opts) {
		...
		for (const shadow of parts) {
			...
			const afterColor = shadow.substring(
				shadow.indexOf(colorMatch[0]) +
					colorMatch[0].length,
			);
			const beforeColor = shadow.substring(
				0,
				shadow.indexOf(colorMatch[0]),
			);
			const pxVals = [
	...
	function checkHtmlPatterns(html) {
		...
		if (
			/\bbg-clip-text\b/.test(html) &&
			/\bbg-gradient-to-/.test(html)
		) {
			findings.push({
				id: "gradient-text",
				snippet: "bg-clip-text + bg-gradient (Tailwind)",
		...
		const roundedSpacing = spacingValues.map(
			(v) => Math.round(v / 4) * 4,
		);
		if (roundedSpacing.length >= 10) {
			...
			const unique = [...new Set(roundedSpacing)].filter(
				(v) => v > 0,
			);
		...
		if (bounceMatch) {
			const animationToken = bounceMatch[1]
				.split(/[,\s]+/)
				.find((part) =>
					/bounce|elastic|wobble|jiggle|spring/i.test(part),
				);
			findings.push({
				id: "bounce-easing",
		...
		const twDarkBg =
			/\bbg-(?:gray|slate|zinc|neutral|stone)-(?:9\d{2}|800)\b/;
		...
		if (
			/repeating-(?:linear|radial|conic)-gradient\s*\(/i.test(
				html,
			)
		) {
			findings.push({
		...
		while ((im = imgTagRe.exec(html)) !== null) {
			if (
				/\bhover:(?:scale|rotate|translate|skew)-/.test(
					im[1],
				)
			) {
				findings.push({
	...
	function resolveBorderRadiusPx(el, style, widthPx, win) {
		const fromComputed = parseRadiusToPx(
			style.borderRadius,
			widthPx,
		);
	...
	function checkElementColorsDOM(el) {
		...
		return checkColors({
			...
			effectiveBg,
			effectiveBgStops: effectiveBg
				? null
				: resolveGradientStops(el),
	...
	function checkElementIconTileDOM(el) {
		...
		return checkIconTile({
			...
			siblingBorderWidth:
				parseFloat(sibStyle.borderTopWidth) || 0,
			siblingBorderRadius:
				parseFloat(sibStyle.borderRadius) || 0,
	...
	function checkElementHeroEyebrowDOM(el) {
		...
		return checkHeroEyebrow({
			...
			siblingLetterSpacing:
				parseFloat(sibStyle.letterSpacing) || 0,
	...
	function parseColorResolved(str, customPropMap) {
		if (!str) return null;
		const resolved = customPropMap
			? resolveVarRefs(str, customPropMap)
			: str;
	...
	function isRepeatedKickerCardContext(heading, kicker) {
		const item = heading.closest?.(
			REPEATED_KICKER_CARD_CONTEXT_SELECTOR,
		);
		return Boolean(
			item && (!item.contains || item.contains(kicker)),
		);
	}
	...
	function isRepeatedKickerCandidate(opts) {
		...
		if (
			!kickerText ||
			kickerText.length < 2 ||
			kickerText.length > 34
		)
			return false;
		if (
			/^step\s*\d+/i.test(kickerText) ||
			/^\d{1,2}$/.test(kickerText)
		)
			return false;
		...
		if (!(kickerFontSize > 0 && kickerFontSize <= 14))
			return false;
		const minTrackedSpacing = Math.max(
			1,
			kickerFontSize * 0.08,
		);
	...
	function collectRepeatedSectionKickerCandidates(
		...
		for (const heading of doc.querySelectorAll(
			...
			if (
				!kicker ||
				kicker.closest?.(REPEATED_KICKER_SKIP_SELECTOR)
			)
				continue;
	...
	function checkRepeatedSectionKickersDOM() {
		const candidates =
			collectRepeatedSectionKickerCandidates(
				...
				(value, fontSize) =>
					resolveLengthPx(value, fontSize) || 0,
			);
	...
	function checkElementGlowDOM(el) {
		...
		if (!parentBg) {
			...
			while (cur && cur.nodeType === 1) {
				const bgImage =
					getComputedStyle(cur).backgroundImage || "";
	...
	function checkElementAIPaletteDOM(el) {
		...
		if (textColor && hasChroma(textColor, 80)) {
			...
			if (isAIPalette) {
				...
				if (
					effectiveBg &&
					relativeLuminance(effectiveBg) < 0.1
				) {
					const label =
						hue >= 260 ? "Purple/violet" : "Cyan";
					findings.push({
						id: "ai-color-palette",
	...
	function hasMeaningfulDirectText(node) {
		...
		for (const child of node.childNodes) {
			if (
				child.nodeType === 3 &&
				child.textContent.trim().length > 4
			)
				return true;
		}
		return false;
	...
	function textDescendantsFlushSides(el, rect) {
		...
		for (const node of candidates) {
			if (
				!TEXT_EDGE_TAGS.has(node.tagName) ||
	...
	function checkQuality(opts) {
		...
		const isInlineCode =
			tag === "code" && !(el.closest && el.closest("pre"));
		if (
			!isInlineCode &&
			rect &&
			hasDirectText &&
			...
			const borderCount = Object.values(borders).filter(
				(w) => w > 0,
			).length;
			const hasBg = hasVisibleBackgroundBoundary(
				style,
				el,
				win,
			);
			if (borderCount >= 2 || hasBg) {
				...
				const vMin = vPads.length
					? Math.min(...vPads)
					: Infinity;
				const hMin = hPads.length
					? Math.min(...hPads)
					: Infinity;
		...
		// visual failure mode.
		{
			...
			if (
				...
				const borderVisible = {
					top:
						borderW.top > 0 &&
						!cssColorIsTransparent(style.borderTopColor),
					right:
						borderW.right > 0 &&
						!cssColorIsTransparent(style.borderRightColor),
					bottom:
						borderW.bottom > 0 &&
						!cssColorIsTransparent(style.borderBottomColor),
					left:
						borderW.left > 0 &&
						!cssColorIsTransparent(style.borderLeftColor),
				};
				...
				if (!outlineW && style.outline) {
					const wMatch = style.outline.match(
						/(\d+(?:\.\d+)?)\s*px/,
					);
				...
				const bgVisible = hasVisibleBackgroundBoundary(
					style,
					el,
					win,
				);
				...
				if (anyVisible) {
					...
					const pad = {
						top:
							resolveLengthPx(style.paddingTop, fontSize) ??
							0,
						right:
							resolveLengthPx(
								style.paddingRight,
					...
					for (const child of el.children) {
						let childStyle = getComputedStyleFor(
							win,
							child,
						);
						...
						const childPad = {
							top:
								resolveLengthPx(
									childStyle.paddingTop,
									fontSize,
								) ?? 0,
							right:
								resolveLengthPx(
									childStyle.paddingRight,
									fontSize,
								) ?? 0,
							bottom:
								resolveLengthPx(
									childStyle.paddingBottom,
									fontSize,
								) ?? 0,
							left:
								resolveLengthPx(
									childStyle.paddingLeft,
									fontSize,
								) ?? 0,
						};
						const childMargin = {
						...
						if (
							rect &&
							typeof child.getBoundingClientRect ===
								"function"
						) {
							try {
								const childRect =
					...
					const textFlush = rect
						? textDescendantsFlushSides(el, rect)
						: null;
					...
					if (flushSides.length > 0) {
						...
						if (hasTextChild) {
							...
							if (borderSidesVisible.length === 4)
								...
								boundaryParts.push(
									`border-${borderSidesVisible.join("/")}`,
								);
							if (outlineVisible)
		...
		if (
			...
			const isPositioned = ["fixed", "absolute"].includes(
				style.position || "",
			);
		...
		if (hasDirectText && style.textAlign === "justify") {
			const hyphens =
				style.hyphens || style.webkitHyphens || "";
			if (hyphens !== "auto") {
				findings.push({
					id: "justified-text",
					snippet:
	...
	function checkElementQualityDOM(el) {
		...
		const lineHeightPx = resolveLengthPx(
			style.lineHeight,
			fontSize,
		);
		const letterSpacingPx = resolveLengthPx(
			style.letterSpacing,
			fontSize,
		);
		const rect = el.getBoundingClientRect();
	...
	function checkElementColors(
		...
		const effectiveBg = resolveBackground(
			el,
			window,
			customPropMap,
		);
		...
		return checkColors({
			...
			effectiveBgStops: effectiveBg
				? null
				: resolveGradientStops(el, window),
			fontSize: parseFloat(style.fontSize) || 16,
			fontWeight: parseInt(style.fontWeight) || 400,
			hasDirectText,
			isEmojiOnly: isEmojiOnlyText(directText),
	...
	function checkElementHeroEyebrow(
		el,
		style,
		tag,
		window,
	...
	function checkTypography() {
		...
		for (const el of document.querySelectorAll(
			...
			const primary = stack.find(
				(f) => f && !GENERIC_FONTS.has(f),
			);
			if (!primary) continue;
			fontUsage.set(
				primary,
				(fontUsage.get(primary) || 0) + 1,
			);
			totalTextElements++;
		}
	...
	function isCardLikeDOM(el) {
		...
		const hasRadius =
			parseFloat(style.borderRadius) > 0 ||
			/\brounded(?:-sm|-md|-lg|-xl|-2xl|-full)?\b/.test(
				cls,
			);
		const hasBg =
			(style.backgroundColor &&
	...
	function checkLayout() {
		...
		for (const el of document.querySelectorAll("*")) {
			...
			if (
				...
				continue;
			if (
				/\b(?:dropdown|popover|tooltip|menu|modal|dialog)\b/i.test(
					cls,
				)
			)
				continue;
	...
	function checkPageTypography(doc, win) {
		...
		while ((m = gfRe.exec(html)) !== null) {
			const families = m[1]
				.split("|")
				.map((f) =>
					f.split(":")[0].replace(/\+/g, " ").toLowerCase(),
				);
		...
		for (const el of textEls) {
			const fontSize = parseFloat(
				win.getComputedStyle(el).fontSize,
			);
	...
	function isCardLike(el, win) {
		...
		const hasBg =
			/\bbg-(?:white|gray-\d+|slate-\d+)\b/.test(cls) ||
			/background(?:-color)?\s*:\s*(?!transparent)/i.test(
				rawStyle,
			);
	...
	function creamFromClassList(cls) {
		...
		if (
			arb &&
			isCreamColor(parseAnyColor(arb[1].replace(/_/g, " ")))
		)
			return `bg-[${arb[1]}]`;
	...
	function checkOversizedH1({
		...
		if (
			fontSize >= OVERSIZED_H1_FONT_PX &&
			textLen >= OVERSIZED_H1_MIN_CHARS
		) {
			let viewportDetail = "";
			if (rect && viewportWidth > 0 && viewportHeight > 0) {
				...
				const dominatesViewport =
					heightRatio >=
						OVERSIZED_H1_MIN_VIEWPORT_HEIGHT_RATIO ||
	...
	function positionedChildEscapesClip(
		el,
		child,
		clipX,
		clipY,
	...
	function checkClippedOverflow(el, style, getStyle) {
		...
		const anyScroll =
			scrolls(ox) || scrolls(oy) || scrolls(ov);
		...
		for (const child of el.querySelectorAll("*")) {
			...
			if (pos === "absolute" || pos === "fixed") {
				...
				const escapes = positionedChildEscapesClip(
					el,
					child,
					clipX,
					clipY,
				...
				if (
					escapes === null &&
					!positionedStyleImpliesEscape(childStyle)
				)
					continue;
				return [
					{
	...
	function checkElementClippedOverflow(
		el,
		style,
		tag,
		window,
	) {
		return checkClippedOverflow(el, style, (n) =>
	...
	function clippedByInset(clipPath) {
		...
		const beforeRound = match[1]
			.split(/\s+round\s+/)[0]
			.trim();
		if (!beforeRound) return false;
		const values = expandBoxShorthand(
			beforeRound.split(/\s+/).slice(0, 4),
		);
	...
	function isScreenReaderOnlyTextStyle(
		style,
		metrics = {},
	) {
		...
		const fontSize =
			metricLengthPx(style.fontSize, 16) || 16;
	...
	if (IS_BROWSER) {
		...
		const EXTENSION_MODE =
			(_myScript &&
				_myScript.dataset.impeccableExtension === "true") ||
			document.documentElement.dataset
				.impeccableExtension === "true";
  ...
  `;
		...
		function showSpotlight(target) {
			...
			if (
				window.__IMPECCABLE_CONFIG__?.spotlightBlur ===
				false
			) {
		...
		document.addEventListener("transitionend", (e) => {
			...
			for (const o of overlays) {
				if (
					...
					continue;
				if (
					e.target === o._targetEl ||
					e.target.contains(o._targetEl)
				) {
					positionOverlay(o);
				}
			}
		...
		const highlight = function (el, findings) {
			...
			Object.assign(outline.style, {
				...
				top: fixed
					? `${rect.top - 2}px`
					: `${rect.top + scrollY - 2}px`,
				left: fixed
					? `${rect.left - 2}px`
					: `${rect.left + scrollX - 2}px`,
			...
			const entries = findings.map((f) => {
				const name =
					TYPE_LABELS[f.type || f.id] || f.type || f.id;
				const prefix =
			...
			function updateCycleText() {
				...
				textSpan.textContent = isHovered
					? e.detail
					: e.name;
			}
			...
			outline._checkLabel = () => {
				if (
					entries.length > 1 &&
					label.offsetWidth > outline.offsetWidth
				) {
					enableCycleMode();
				}
			};
		...
		const showPageBanner = function (findings) {
			...
			banner.className =
				"impeccable-overlay impeccable-banner";
			...
			if (!EXTENSION_MODE) {
				...
				toggle.addEventListener("click", () => {
					...
					toggle.textContent = overlaysVisible
						? "\u25C9"
						: "\u25CB"; // filled vs empty circle
					toggle.style.opacity = overlaysVisible
						? "0.85"
						: "0.5";
				});
		...
		function isLikelyHashedClass(c) {
			if (!c) return true;
			if (
				/^(css|sc|emotion|jsx|module)-[\w-]{4,}$/i.test(c)
			)
				return true;
		...
		function buildSelectorSegment(el) {
			...
			if (el.classList && el.classList.length > 0) {
				const classes = [...el.classList]
					.filter(
						(c) =>
							!c.startsWith("impeccable-") &&
							!isLikelyHashedClass(c),
					)
					.slice(0, 2);
				if (classes.length > 0) {
					sel +=
		...
		function generateSelector(el) {
			...
			while (
				...
				try {
					const matches =
						document.querySelectorAll(trySelector);
		...
		function collectVisualContrastReasons(el, style) {
			...
			const sampleRect =
				getDirectTextRect(el) || el.getBoundingClientRect();
		...
		function collectVisualContrastCandidates(options = {}) {
			...
			for (const el of document.querySelectorAll("*")) {
				...
				const rect =
					getDirectTextRect(el) ||
					el.getBoundingClientRect();
				if (!rect || rect.width < 4 || rect.height < 4)
					continue;

				const reasons = collectVisualContrastReasons(
					el,
					style,
				);
				...
				const clip = {
					x: Math.max(
						0,
						Math.floor(rect.left + window.scrollX - 2),
					),
					y: Math.max(
						0,
						Math.floor(rect.top + window.scrollY - 2),
					),
				...
				candidates.push({
					...
					backgroundClipText: reasons.includes(
		...
		function resolvePaintedImageRect(
			...
			if (size === "cover" || size === "contain") {
				...
				if (/%$/.test(widthToken))
					...
					paintedWidth =
						parseFloat(widthToken) || paintedWidth;
				if (heightToken === "auto")
					paintedHeight =
						paintedWidth *
					...
					paintedHeight =
						...
						(parseFloat(heightToken) / 100);
				else if (/px$/.test(heightToken))
					paintedHeight =
						parseFloat(heightToken) || paintedHeight;
			}
		...
		function resolveObjectImageRect(
			containerRect,
			image,
			style,
		) {
			...
			const [xToken, yToken] = parseObjectPosition(
		...
		function sampleDrawablePixel(drawable, sourcePoint) {
			if (visualContrastRasterCache.has(drawable)) {
				const cached =
					visualContrastRasterCache.get(drawable);
				...
				try {
					...
					const data = cached.ctx.getImageData(
						x,
						y,
						1,
						1,
					...
					return {
						status: "unresolved",
						reason: /taint|cross-origin|Security/i.test(
							err?.message || "",
						)
							? "tainted image"
							: "image sample failed",
			...
			canvas.width = Math.max(
				1,
				Math.round(intrinsicWidth * scale),
			);
			canvas.height = Math.max(
				1,
				Math.round(intrinsicHeight * scale),
			);
			...
			try {
				...
				const reason = /taint|cross-origin|Security/i.test(
					err?.message || "",
				)
					? "tainted image"
					: "image sample failed";
		...
		async function sampleCssBackground(
			el,
			style,
			point,
			textColor,
			...
			if (bgImage && bgImage !== "none") {
				...
				if (/url\s*\(/i.test(bgImage)) {
					const img = await loadVisualContrastImage(
						firstCssUrl(bgImage),
					);
					if (!img)
						return {
					...
					const paintedRect = resolvePaintedImageRect(
						...
						getLayerValue(style.backgroundPosition) ||
							"50% 50%",
					);
					const sourcePoint = pointToImageSource(
						point,
						paintedRect,
					);
					if (!sourcePoint)
					...
					const sample = sampleDrawablePixel(
						img,
						sourcePoint,
					);
		...
		async function sampleImageElement(img, point) {
			...
			const paintedRect = resolveObjectImageRect(
				rect,
				img,
				style,
			);
			...
			if (img.currentSrc || img.src) {
				const loaded = await loadVisualContrastImage(
				...
				if (loaded) {
					...
					const loadedPoint = pointToImageSource(
						point,
						loadedRect,
					);
					if (loadedPoint) {
						const loadedSample = sampleDrawablePixel(
							loaded,
							loadedPoint,
						);
		...
		function textSamplePoints(rect) {
			const insetX = Math.min(
				12,
				Math.max(1, rect.width * 0.12),
			);
			const insetY = Math.min(
				8,
				Math.max(1, rect.height * 0.22),
			);
			const xs =
				rect.width < 28
		...
		async function sampleVisualBackgroundAtPoint(
			...
			for (const node of nodes) {
				...
				if (tag === "img") {
					const sample = await sampleImageElement(
						node,
						point,
					);
				...
				if (tag === "canvas" || tag === "video") {
					...
					const sourcePoint = pointToImageSource(point, {
						...
						height: rect.height,
						intrinsicWidth:
							node.width || node.videoWidth || rect.width,
						intrinsicHeight:
							node.height ||
							node.videoHeight ||
							rect.height,
				...
				const sample = await sampleCssBackground(
					node,
					style,
					point,
					textColor,
			...
			return {
				status: "unresolved",
				reason:
					[...new Set(unresolved.filter(Boolean))]
						.slice(0, 3)
						.join(", ") || "no readable visual background",
			};
		}

		async function analyzeVisualContrastCandidate(
			candidate,
		) {
			let el;
			try {
			...
			const style = getComputedStyle(el);
			const textColor =
				parseRgb(style.color) || candidate.textColor;
			if (!textColor)
				return {
					...candidate,
					status: "unresolved",
			...
			if (ratios.length < Math.min(3, points.length)) {
				return {
					...
					reason:
						[...new Set(unresolved.filter(Boolean))]
							.slice(0, 3)
							.join(", ") || "not enough readable samples",
				};
			}
			...
			const pick = (pct) =>
				ratios[
					Math.min(
						...
						Math.max(
							0,
							Math.floor((pct / 100) * ratios.length),
						),
					)
				];
			...
			const status =
				measuredRatio < candidate.threshold
					? "fail"
					: "pass";
			const method =
				[...methods].sort().join(", ") || "browser-visual";
			const textLabel = candidate.text
				? ` "${candidate.text}"`
				: "";
			...
			return {
				...candidate,
				status,
				confidence: method.includes("canvas-")
					? "high"
					: "medium",
				method,
				ratio: measuredRatio,
				...
				finding:
					status === "fail"
						? { id: "low-contrast", snippet: detail }
						: null,
			};
		}
		...
		async function analyzeVisualContrast(options = {}) {
			const candidates =
				collectVisualContrastCandidates(options);
			const results = [];
			const shouldScrollOffscreen =
				options.scrollOffscreen === true;
			const restoreScroll = {
				x: window.scrollX,
				y: window.scrollY,
			};
			for (const candidate of candidates) {
				...
				let result =
					await analyzeVisualContrastCandidate(candidate);
				if (
					shouldScrollOffscreen &&
					...
					if (
						...
						await waitForVisualPaint();
						result =
							await analyzeVisualContrastCandidate(
								candidate,
							);
					}
				}
		...
		function browserPrimaryFont(stack) {
			...
			return (
				String(stack || "")
					...
					.find(
						(font) => font && !GENERIC_FONTS.has(font),
					) || ""
			);
		}
		...
		function browserDesignSystemConfig() {
			const raw =
				window.__IMPECCABLE_CONFIG__?.designSystem;
			...
			return {
				present: true,
				hasFonts:
					raw.hasFonts === true && allowedFonts.size > 0,
				allowedFonts,
				hasColors:
					raw.hasColors === true &&
				...
				allowedColors,
				hasRadii:
					raw.hasRadii === true && allowedRadii.length > 0,
				allowedRadii,
		...
		function isBrowserDesignColorAllowed(
			raw,
			designSystem,
		) {
		...
		function isBrowserDesignRadiusAllowed(
			raw,
			designSystem,
		) {
		...
		function shouldSkipDesignElement(el) {
			const tag = el.tagName?.toLowerCase?.() || "";
			return (
				DESIGN_SKIP_TAGS.has(tag) || isElementHidden(el)
			);
		}

		function checkElementDesignSystemDOM(
			el,
			designSystem,
			seen,
		) {
			if (
				!designSystem?.present ||
				shouldSkipDesignElement(el)
			)
				return [];
			const findings = [];
			...
			if (
				designSystem.hasFonts &&
				browserHasDirectText(el)
			) {
				const font = browserPrimaryFont(
					style.fontFamily || "",
				);
				if (
					font &&
			...
			if (designSystem.hasColors) {
				...
				for (const [kind, raw] of colorChecks) {
					const label = String(raw || "")
						...
						.replace(/\s+/g, " ");
					if (
						isBrowserDesignColorAllowed(label, designSystem)
					)
						continue;
			...
			if (designSystem.hasRadii) {
				for (const token of browserRadiusTokens(
					style.borderRadius || "",
				)) {
					if (
						isBrowserDesignRadiusAllowed(
							token,
							designSystem,
						)
					)
		...
		function checkBrowserDesignSystemSources(
			designSystem,
			seen,
		) {
			...
			for (const link of document.querySelectorAll(
				...
				for (const match of href.matchAll(
					/[?&]family=([^&]+)/g,
				)) {
					const display = decodeBrowserGoogleFamily(
						match[1],
					);
		...
		function collectBrowserFindings() {
			...
			for (const el of document.querySelectorAll("*")) {
				...
				const eyebrowFindings = checkElementHeroEyebrowDOM(
					el,
				)
					.map((f) => ({ type: f.id, detail: f.snippet }))
					.filter((f) => _ruleOk(f.type));
				if (
					eyebrowFindings.length > 0 &&
			...
			if (designSourceFindings.length > 0) {
				...
				addBrowserFindings(
					groupMap,
					document.body,
					designSourceFindings,
				);
			}

			const typoFindings = checkTypography().filter((f) =>
				_ruleOk(f.type),
			);
			...
			if (sectionKickerFindings.length > 0) {
				...
				addBrowserFindings(
					groupMap,
					document.body,
					sectionKickerFindings,
				);
			}

			const layoutFindings = checkLayout().filter((f) =>
				_ruleOk(f.type),
			);
			...
			const htmlPatternFindings = checkHtmlPatterns(
				docClone.outerHTML,
			);
		...
		function visualContrastOptions(options = {}) {
			...
			const scrollOffscreen =
				typeof options.scrollOffscreen === "boolean"
					...
					: typeof options.visualContrastScrollOffscreen ===
						  "boolean"
		...
		function addVisualContrastResult(
			groupMap,
			result,
			options = {},
		) {
			...
			addBrowserFindings(groupMap, el, [
				{
					type: findingType,
					detail:
						result.finding.detail || result.finding.snippet,
				},
			]);
			if (
				options.decorate &&
		...
		function postSerializedFindings(
			groupMap,
			options = {},
		) {
		...
		function reportVisualContrastError(err, detail = {}) {
			window.dispatchEvent(
				new CustomEvent(
					"impeccable-visual-contrast-error",
					{
						detail: {
							...detail,
							message: err?.message || String(err),
		...
		function scheduleLazyVisualContrast(
			...
			const generation =
				runtime.generation || scanGeneration;
			...
			lazyVisualContrastObserver = new IntersectionObserver(
				(entries) => {
					for (const entry of entries) {
						...
						const candidate =
							lazyVisualContrastPending.get(el);
						if (
							!candidate ||
							lazyVisualContrastResolving.has(el)
						)
							continue;
						...
						waitForVisualPaint()
							.then(() =>
								analyzeVisualContrastCandidate(candidate),
							)
							.then((result) => {
								if (generation !== scanGeneration) return;
								rememberVisualContrastAnalysis(result);
								const added = addVisualContrastResult(
									groupMap,
									result,
									{ decorate: true },
								);
								if (added) {
									...
									window.dispatchEvent(
										new CustomEvent(
											"impeccable-visual-contrast-resolved",
											{
												detail: {
		...
		async function addVisualContrastFindings(
			...
			const resolvedOptions =
				visualContrastOptions(options);
			const analyses =
				await analyzeVisualContrast(resolvedOptions);
			if (
				runtime.generation &&
				runtime.generation !== scanGeneration
			)
		...
		async function collectBrowserFindingsAsync(
			options = {},
			runtime = {},
		) {
			const collected = collectBrowserFindings();
			await addVisualContrastFindings(
				collected.groupMap,
				options,
				runtime,
			);
			return {
				...collected,
				allFindings: browserFindingsFromMap(
					collected.groupMap,
				),
		...
		function renderBrowserFindings(
			collected,
			options = {},
		) {
		...
		const scan = function (options = {}) {
			...
			const collected = collectBrowserFindings();
			const allFindings = renderBrowserFindings(
				collected,
				options,
			);
			if (shouldRunVisualContrast(options)) {
		...
		const scanAsync = async function (options = {}) {
			...
			const generation = scanGeneration;
			if (shouldRunVisualContrast(options)) {
				const collected = await collectBrowserFindingsAsync(
					options,
					{ generation, scheduleLazy: true },
				);
				if (generation !== scanGeneration) return [];
				return renderBrowserFindings(collected, options);
			}
			lastVisualContrastAnalyses = [];
			return renderBrowserFindings(
		...
		window.impeccableAnalyzeVisualContrast =
