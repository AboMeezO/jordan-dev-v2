...
const hasRounded = (line) =>
	/\brounded(?:-\w+)?\b/.test(line);
const hasBorderRadius = (line) =>
	/border-radius/i.test(line);
...
const REGEX_MATCHERS = [
	...
	},
	{
		id: "side-tab",
		regex:
			/border-(?:left|right)\s*:\s*(\d+)px\s+solid[^;]*/gi,
		test: (m, line) => {
	...
	},
	{
		id: "side-tab",
		regex:
			/border-inline-(?:start|end)\s*:\s*(\d+)px\s+solid/gi,
		test: (m, line) => !isSafeElement(line) && +m[1] >= 3,
		fmt: (m) => m[0],
	},
	{
		id: "side-tab",
		regex:
			/border-inline-(?:start|end)-width\s*:\s*(\d+)px/gi,
		test: (m, line) => !isSafeElement(line) && +m[1] >= 3,
		fmt: (m) => m[0],
	},
	{
		id: "side-tab",
		regex:
			/border(?:Left|Right)\s*[:=]\s*["'`](\d+)px\s+solid/g,
		test: (m) => +m[1] >= 3,
		fmt: (m) => m[0],
	},
	...
	// --- Gradient text ---
	{
		id: "gradient-text",
		regex:
			/background-clip\s*:\s*text|-webkit-background-clip\s*:\s*text/gi,
	...
	// --- Tailwind AI palette ---
	{
		...
		test: (m, line) =>
			/\btext-(?:[2-9]xl|[3-9]xl)\b|<h[1-3]/i.test(line),
...
const REGEX_ANALYZERS = [
	...
	(content, filePath) => {
		...
		while ((m = clampRe.exec(content)) !== null) {
			sizes.add(
				Math.round(
					(m[2] === "px" ? +m[1] : +m[1] * REM) * 10,
				) / 10,
			);
			sizes.add(
				Math.round(
					(m[4] === "px" ? +m[3] : +m[3] * REM) * 10,
				) / 10,
			);
		}
		const TW = {
	...
	(content, filePath) => {
		...
		const unique = [...new Set(rounded)].filter(
			(v) => v > 0,
		);
	...
	(content, filePath) => {
		...
		const hasDarkBg =
			darkBgRe.test(content) || twDarkBg.test(content);
		if (!hasDarkBg) return [];
...
const CSS_IN_JS_EXTENSIONS = new Set([
	".js",
	".ts",
	".jsx",
	".tsx",
...
function runRegexMatchers(
	...
	const { profile, phase = "regex-matchers" } =
		options || {};
	const findings = [];
	if (!profile) {
...
function runTextContentAnalyzers(
	content,
	filePath,
	options = {},
) {
	...
	for (
		let i = 0;
		i < TEXT_CONTENT_ANALYZER_IDS.length;
		i++
	) {
...
function detectText(content, filePath, options = {}) {
	...
	const byProvider = filterByProviders(
		deduped,
		options?.providers,
	);
