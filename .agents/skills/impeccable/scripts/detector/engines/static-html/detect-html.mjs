...
function checkElementBrokenImage(el) {
	...
	if (trimmed === "" || trimmed === "#") {
		return [
			{ id: "broken-image", snippet: `<img src="${src}">` },
		];
	}
	return [];
}
...
const STATIC_ELEMENT_RULES = [
	...
	},
	{
		...
		run: (el, tag, _style, window) =>
			checkElementIconTile(el, tag, window),
	},
	{
		id: "italic-serif-display",
		selector: "h1,h2",
		run: (el, tag, style) =>
			checkElementItalicSerif(el, style, tag),
	},
	{
	...
	},
	{
		...
		selector: "*",
		run: (el, tag, style) =>
			checkElementGptBorderShadow(el, style),
	},
];
...
async function detectHtml(filePath, options = {}) {
	...
	try {
		modules = await profileStepAsync(
			...
			async () => {
				const [htmlparser2, cssSelect, csstree, domutils] =
					await Promise.all([
						import("htmlparser2"),
						import("css-select"),
						import("css-tree"),
	...
	if (options?.designSystem) {
		...
		findings.push(
			...mergeDesignSystemFindings(
				staticDesignFindings,
				sourceDesignFindings,
			),
		);
	}
	...
	if (isFullPage(html)) {
		...
		for (const f of runPageCheck("html-patterns", () =>
			checkHtmlPatterns(html).filter(
				(item) =>
					item.id !== "bounce-easing" &&
					item.id !== "layout-transition",
			),
		)) {
			findings.push(finding(f.id, filePath, f.snippet));
		}
		...
		for (const f of runPageCheck("text-content", () =>
			...
			findings.push(
				finding(f.antipattern, filePath, f.snippet),
			);
		}
	}
...
export {
	checkStaticPageTypography,
	STATIC_ELEMENT_RULES,
	detectHtml,
};
