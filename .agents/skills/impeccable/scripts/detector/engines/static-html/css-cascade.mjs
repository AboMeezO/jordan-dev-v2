		...
		for (const rule of rules) {
			...
			for (const [prop, side] of [
				...
				if (!perSide[side])
					perSide[side] = {
						width: 0,
						color: normalizeColorForCheck(resolved),
					};
			}
...
function cssPropToCamel(prop) {
	...
	return prop.replace(/-([a-z])/g, (_m, ch) =>
		ch.toUpperCase(),
	);
}
...
function normalizeStaticCssValue(
	...
	if (prop === "letterSpacing") {
		const base =
			parseFloat(
				currentStyle?.fontSize || parentStyle?.fontSize,
			) || 16;
		const px = resolveLengthPx(resolved, base);
		if (px != null) resolved = `${px}px`;
	}
...
function parseStaticFont(value) {
	...
	const weight = value.match(
		/\b([1-9]00|bold|normal|lighter|bolder)\b/i,
	);
	...
	if (slashParts) {
		...
		const familyStart =
			value.indexOf(slashParts[0]) + slashParts[0].length;
...
function expandStaticDeclaration(prop, value) {
	...
	if (p === "background") {
		...
		const beforeImage = hasImage
			...
			: v;
		const color = extractStaticColor(
			hasImage ? beforeImage : v,
		);
	...
	if (p === "outline") {
		...
		if (
			!parsed.width &&
			/^0(?:px|rem|em|%)?$/.test(v.trim())
		) {
			out.push(["outlineWidth", "0px"]);
		}
		return out;
	}
	const sideMatch = p.match(
		/^border-(top|right|bottom|left)$/,
	);
	if (sideMatch) {
		const parsed = parseStaticBorder(v);
		const side =
			sideMatch[1][0].toUpperCase() + sideMatch[1].slice(1);
		return [
			...(parsed.width
				? [[`border${side}Width`, parsed.width]]
				: []),
			...(parsed.color
				? [[`border${side}Color`, parsed.color]]
				: []),
		];
	}
	if (p === "border-width") {
	...
	if (p === "transition") {
		...
		return [
			...(parsed.property
				? [["transitionProperty", parsed.property]]
				: []),
			...(parsed.timing
...
function compareStaticPriority(a, b) {
	...
	for (let i = 0; i < 3; i++) {
		if (
			(b.specificity[i] || 0) !== (a.specificity[i] || 0)
		) {
			return (
				(b.specificity[i] || 0) > (a.specificity[i] || 0)
			);
...
function staticSpecificity(selector) {
	...
	const stripped = noWhere
		...
		.replace(/[*>+~(),]/g, " ");
	const types = (
		stripped.match(/\b[a-zA-Z][\w-]*\b/g) || []
	).length;
	return [ids, classes, types];
}

function applyStaticDeclaration(
	specified,
	node,
	prop,
	value,
	...
	for (const [
		expandedProp,
		expandedValue,
	] of expandStaticDeclaration(prop, value)) {
...
function parseStaticStyleAttribute(
	styleText,
	orderBase = 0,
) {
	const decls = [];
	...
	const walkList = (list, atRuleStack = []) => {
		list?.forEach?.((node) => {
			if (node.type === "Rule" && node.block) {
				if (
					atRuleStack.some((name) =>
						/keyframes$/i.test(name),
					)
				)
					return;
				const selectorText = csstree
...
class StaticElement {
	...
	querySelector(selector) {
		try {
			const found = this._doc.selectOne(
				selector,
				this.node.children || [],
			);
...
function collectStaticCssText(
	...
	const styleTexts = [];
	for (const styleEl of modules.selectAll(
		"style",
		root.children || [],
	)) {
		styleTexts.push(modules.domutils.textContent(styleEl));
	}
	const links = modules.selectAll(
		"link",
		root.children || [],
	);
...
function buildStaticStyleMap(
	...
	const allNodes = modules.selectAll(
		"*",
		root.children || [],
	);
	...
	profileStep(
		...
		() => {
			...
			for (const node of allNodes) {
				...
				for (const decl of parseStaticStyleAttribute(
					styleText,
					inlineOrder,
				)) {
					applyStaticDeclaration(
	...
	const computeNode = (
		...
		for (const [prop, decl] of specifiedMap) {
			if (prop.startsWith("--"))
				customProps.set(
					prop,
					resolveVarRefs(decl.value, customProps),
				);
		}
		const values = {};
		for (const prop of Object.keys(STATIC_DEFAULT_STYLE)) {
			if (
				STATIC_INHERITED_PROPS.has(prop) &&
				parentStyle?.[prop] != null
			)
