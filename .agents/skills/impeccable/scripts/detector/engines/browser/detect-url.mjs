...
function serializeDesignSystemForBrowser(designSystem) {
	...
	return {
		...
		allowedFonts: Array.from(
			designSystem.allowedFonts || [],
		),
		hasColors: designSystem.hasColors === true,
		allowedColors: Array.from(
			designSystem.allowedColorKeys?.values?.() || [],
		)
...
async function runVisualContrastFallback(
	...
	const scrollOffscreen =
		options?.visualContrastScrollOffscreen !== false;
	const existingLowContrastSelectors = new Set(
		serializedGroups
			.filter((group) =>
				group.findings?.some(
					(f) => f.type === "low-contrast",
				),
			)
			...
			async () => {
				browserAnalyses = await page.evaluate(
					async ({ maxCandidates, scrollOffscreen }) => {
						if (
							typeof window.impeccableAnalyzeVisualContrast !==
							"function"
						)
							return [];
				...
				return browserAnalyses
					.filter(
						(result) =>
							result.finding &&
							!existingLowContrastSelectors.has(
								result.selector,
							),
					)
	...
	let candidates =
		browserAnalyses.length > 0 ? browserAnalyses : [];
	...
	for (const candidate of filtered) {
		const result = await profileFindingsAsync(
			...
			async () => {
				const finding =
					await captureVisualContrastCandidate(
						page,
						candidate,
						viewport,
					);
...
async function detectUrl(url, options = {}) {
	...
	try {
		...
		const browserDesignSystem =
			serializeDesignSystemForBrowser(
