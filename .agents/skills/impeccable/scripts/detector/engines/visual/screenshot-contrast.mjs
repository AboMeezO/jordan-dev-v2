...
async function compareScreenshotContrast(
	...
	return page.evaluate(
		async ({ beforeBase64, afterBase64, candidate }) => {
			const loadImage = (base64) =>
				new Promise((resolve, reject) => {
					...
					img.onerror = () =>
						reject(
							new Error(
								"Could not decode contrast screenshot",
							),
						);
			...
			const beforePixels = ctx.getImageData(
				0,
				0,
				width,
				height,
			...
			const afterPixels = ctx.getImageData(
				0,
				0,
				width,
				height,
			...
			const cssTextColor =
				candidate.textColor &&
				!candidate.preferRenderedForeground
					? {
...
async function captureVisualContrastCandidate(
	page,
	candidate,
	viewport,
) {
	const clip = sanitizeScreenshotClip(
		candidate.clip,
		viewport,
	);
	if (!clip) return null;
