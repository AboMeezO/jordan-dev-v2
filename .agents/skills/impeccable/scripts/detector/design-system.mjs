...
function resolveDesignSidecarPath(
	cwd = process.cwd(),
	contextDir = cwd,
) {
	const candidates = [
...
function parseYamlSubset(yaml) {
	...
	for (const raw of String(yaml || "").split(/\r?\n/)) {
		...
		const rest = stripInlineYamlComment(
...
function primaryFont(stack) {
	if (
		!stack ||
		/var\(/i.test(stack) ||
		!isLiteralFontStack(stack)
	)
		return "";
	return (
		splitFontStack(stack).find(
			(font) => !GENERIC_FONTS.has(font),
		) || ""
	);
}
...
function addDesignColor(out, value, label) {
	...
	if (!out.allowedColorKeys.has(key)) {
		out.allowedColorKeys.set(key, {
			color: parsed,
			labels: [],
		});
	}
	out.allowedColorKeys
		.get(key)
		.labels.push(label || cssColorLabel(value));
}
	...
	for (const [name, meta] of Object.entries(colorMeta)) {
		...
		if (Array.isArray(meta.tonalRamp)) {
			for (const [
				index,
				value,
			] of meta.tonalRamp.entries()) {
				if (typeof value === "string")
					addDesignColor(
						out,
						value,
						`sidecar.${name}.tonalRamp[${index}]`,
					);
			}
		}
	}
...
function addSidecarRadii(out, sidecar) {
	...
	for (const [rawName, meta] of Object.entries(
		roundedMeta,
	)) {
		const name = unquoteYamlKey(rawName).toLowerCase();
		if (
...
function loadDesignSystemForCwd(cwd = process.cwd()) {
	...
	const sidecarPath = resolveDesignSidecarPath(
		cwd,
		md.contextDir,
	);
...
function isProbablyColorLiteral(line, match) {
	...
	if (raw.startsWith("#")) {
		...
		if (prevNonSpace === ">" && nextNonSpace === "<")
			return false; // plain text, e.g. PR #155
	}
	...
	return (
		styleContext || cssFunctionContext || jsColorKeyContext
	);
}
...
function makeDesignFinding(
	...
	return {
		...finding(id, filePath, snippet, line),
		...extras,
	};
}
...
function checkFontStack(
	...
	if (!primary || isAllowedFont(primary, designSystem))
		return [];
	const display = primary.replace(/\b\w/g, (ch) =>
		ch.toUpperCase(),
	);
	return [
		makeDesignFinding(
...
function checkSourceDesignSystem(
	content,
	filePath,
	options = {},
) {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (designSystem.hasFonts) {
			...
			for (const match of line.matchAll(GOOGLE_FONT_RE)) {
				const url = match[0];
				for (const familyMatch of url.matchAll(
					/[?&]family=([^&]+)/g,
				)) {
					const font = normalizeFontName(
						decodeGoogleFamily(familyMatch[1]),
					);
					if (!font || isAllowedFont(font, designSystem))
						continue;
					const display = decodeGoogleFamily(
						familyMatch[1],
					);
					findings.push(
						makeDesignFinding(
	...
	for (const el of document.querySelectorAll("*")) {
		...
		if (designSystem.hasFonts && hasDirectText(el)) {
			...
			if (
				font &&
				!seenFonts.has(font) &&
				!isAllowedFont(font, designSystem)
			) {
				seenFonts.add(font);
				findings.push(
					makeDesignFinding(
...
function canonicalDesignFindingKey(item) {
	...
	if (item.antipattern === "design-system-color") {
		...
		return label
			? `${item.antipattern}:color:${label}`
			: null;
	}
	if (item.antipattern === "design-system-radius") {
		const px = resolveLengthPx(
			String(value || "").trim(),
			16,
		);
