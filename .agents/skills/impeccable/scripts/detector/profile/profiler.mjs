...
function recordProfileEvent(profile, event) {
	...
	const normalized = {
		...
		findings: Number.isFinite(event.findings)
			? event.findings
			: 0,
	};
...
function extractFindingIds(findings) {
	...
	return [
		...new Set(
			findings
				.map((f) => f?.id || f?.type || f?.antipattern)
				.filter(Boolean),
		),
	];
}
...
async function profileFindingsAsync(
	profile,
	meta,
	callback,
) {
...
function summarizeDetectorProfile(profile) {
	...
	for (const event of events) {
		...
		group.calls += 1;
		group.totalMs += ms;
		group.findings += Number.isFinite(event.findings)
			? event.findings
			: 0;
	...
	return [...groups.values()]
		.map((group) => {
			...
			return {
				...
				avgMs: Number(
					(group.totalMs / group.calls).toFixed(3),
				),
