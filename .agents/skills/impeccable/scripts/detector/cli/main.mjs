...
async function detectCli() {
	...
	if (!process.stdin.isTTY && targets.length === 0) {
		...
		const paths =
			targets.length > 0 ? targets : [process.cwd()];
		const urlTargetCount = paths.filter((target) =>
			/^https?:\/\//i.test(target),
		).length;
		const browserDetector =
			urlTargetCount > 1
				? await createBrowserDetector()
				: null;
			...
			for (const target of paths) {
				...
				if (stat.isDirectory()) {
					...
					if (!jsonMode && !quietMode) {
						const fwConfig =
							detectFrameworkConfig(resolved);
						if (fwConfig) {
					...
					for (const file of files) {
						...
						if (importers && importers.size > 0) {
							const importerNames = [...importers].map(
								(f) => path.basename(f),
							);
	...
	if (allFindings.length > 0) {
		if (jsonMode)
			process.stdout.write(
				formatFindings(allFindings, true) + "\n",
			);
		else if (quietMode)
			process.stderr.write(
				formatFindingSummary(allFindings.length) + "\n",
			);
		else
			process.stderr.write(
				formatFindings(allFindings, false) + "\n",
			);
		process.exit(2);
	}
