...
if (IS_BROWSER) {
	...
	const EXTENSION_MODE =
		...
		document.documentElement.dataset.impeccableExtension ===
			"true";
  ...
  `;
	...
	function showSpotlight(target) {
		...
		if (
			window.__IMPECCABLE_CONFIG__?.spotlightBlur === false
		) {
	...
	function resolvePaintedImageRect(
		...
		if (size === "cover" || size === "contain") {
			...
			if (heightToken === "auto")
				paintedHeight =
					paintedWidth * (intrinsicHeight / intrinsicWidth);
		...
		for (const node of nodes) {
			...
			if (tag === "canvas" || tag === "video") {
				...
				const sourcePoint = pointToImageSource(point, {
					...
					intrinsicHeight:
						node.height || node.videoHeight || rect.height,
				});
				if (sourcePoint) {
					const sample = sampleDrawablePixel(
						node,
	...
	async function analyzeVisualContrastCandidate(candidate) {
		...
		const status =
			measuredRatio < candidate.threshold ? "fail" : "pass";
		const method =
	...
	function browserDesignSystemConfig() {
		...
		return {
			...
			allowedFonts,
			hasColors:
				raw.hasColors === true && allowedColors.length > 0,
			allowedColors,
			hasRadii:
	...
	function addVisualContrastResult(
		...
		const existing = groupMap.get(el) || [];
		if (
			existing.some((f) => (f.type || f.id) === findingType)
		)
			return false;
		addBrowserFindings(groupMap, el, [
			{
