...
Output (JSON):
  { ok, file, inserted|removed, config? }`);
	...
	const svelteKit = detectSvelteKitProject(
		process.cwd(),
		config,
	);
	...
	const port =
		portIdx !== -1 ? parseInt(args[portIdx + 1], 10) : NaN;
	if (!Number.isFinite(port)) {
		console.error(
			JSON.stringify({ ok: false, error: "missing_port" }),
		);
		process.exit(1);
	}
	...
	const results = resolvedFiles.map((relFile) => {
		...
		if (!fs.existsSync(absFile))
			return { file: relFile, error: "file_not_found" };
		const content = fs.readFileSync(absFile, "utf-8");
		const withoutOld = revertCspMeta(
			removeTag(content, config.commentSyntax),
		);
		const withTag = insertTag(
			withoutOld,
			config,
			port,
			relFile,
...
export function resolveFiles(rootDir, config) {
	...
	const userExcludes = Array.isArray(config.exclude)
		? config.exclude
		: [];
	...
	for (const pat of patterns) {
		...
		for (const ent of matches) {
			...
			const abs = path.join(
				ent.parentPath || ent.path || rootDir,
				ent.name,
			);
			const rel = path
...
function validateConfig(cfg) {
	...
	if (
		cfg.cspChecked !== undefined &&
		typeof cfg.cspChecked !== "boolean"
	) {
		throw new Error(
...
function insertTag(content, config, port, filePath) {
	...
	if (config.insertBefore) {
		...
		return (
			content.slice(0, idx) + block + content.slice(idx)
		);
	}
	...
	const prefix =
		...
		(existingNewline || lineEnding);
	const rest = content.slice(
		after + existingNewline.length,
	);
...
export function patchCspMeta(content, port) {
	...
	for (let i = tags.length - 1; i >= 0; i--) {
		...
		const attrsBody = attrs.slice(
			0,
			attrs.length - trailingWs.length,
		);
		const newAttrs =
