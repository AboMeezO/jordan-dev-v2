...
function readRawDetectorConfig(cwd, opts = {}) {
	...
	return mergeDetectorConfig(
		detectorSection(unified),
		merged,
	);
}
...
function writeHookConfig(cwd, hookConfig, opts = {}) {
	...
	const existing =
		...
		!Array.isArray(existingRaw)
			? existingRaw
			: {};
	const existingHook = stripDetectorKeys(
		hookSection(existing),
	);
...
function writeDetectorConfig(
	...
	const existingDetector = mergeDetectorConfig(
		detectorSection(existing),
	);
	const next = {
		...existing,
		detector: mergeDetectorConfig(
			detectorConfig,
			existingDetector,
		),
	};
...
function mergeDetectorConfig(existing, seed = null) {
	...
	const out = seed
		? {
				...
				ignoreValues: normalizeIgnoreValueEntries(
					seed.ignoreValues,
				),
			}
		: {
	...
	if (
		...
		out.designSystem = {
			...(out.designSystem || {}),
			enabled:
				base.designSystem.enabled === false ? false : true,
		};
	}
...
function statusReport(cwd) {
	...
	const envState = envKill
		? `IMPECCABLE_HOOK_DISABLED=${envKill}`
		: "unset";
	const cfgPath =
		path.relative(cwd, getConfigPath(cwd)) ||
		".impeccable/config.json";
	const localPath =
		path.relative(cwd, getLocalConfigPath(cwd)) ||
		".impeccable/config.local.json";
	const cachePath =
		path.relative(cwd, getCachePath(cwd)) ||
...
function setEnabled(cwd, value) {
	...
	if (repaired.written.length > 0) {
		...
		parts.push(
			"No installed provider skill folders found to repair.",
		);
	}
...
function repairHookManifests(cwd) {
	...
	for (const target of HOOK_MANIFEST_TARGETS) {
		...
		if (
			sharedDest &&
			fileHasImpeccableHookMarker(sharedDest)
		) {
...
function mergeHookManifests(existing, fresh) {
	...
	for (const event of hookEvents) {
		...
		const added = Array.isArray(freshHooks[event])
			? freshHooks[event]
			: [];
...
function addIgnoreValue(cwd, args) {
	...
	const config = mergeDetectorConfig(
		readRawDetectorConfig(cwd, { local }),
	);
	...
	const target = writeDetectorConfig(cwd, config, {
		local,
	});
	const scope = local
