...
if (args.includes("--help") || args.includes("-h")) {
	console.log(
		"Usage: node live-discard-manual-edits.mjs [--page-url=<url>]",
	);
	process.exit(0);
}
...
if (pageUrlFilter) {
	entries = buffer.entries.filter(
		(entry) => entry.pageUrl === pageUrlFilter,
	);
	discarded = removeEntries(
		cwd,
		(entry) => entry.pageUrl === pageUrlFilter,
	);
} else {
...
const remaining = readBuffer(cwd).entries.reduce(
	(n, e) => n + e.ops.length,
	0,
);
console.log(
