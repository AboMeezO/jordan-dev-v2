...
const UPDATE_HOST = (
	process.env.IMPECCABLE_UPDATE_HOST ||
...
export function loadContext(
	...
	return {
		...
		productPath: productPath
			? path.relative(absCwd, productPath)
			: null,
		hasDesign: !!design,
		design,
		designPath: designPath
			? path.relative(absCwd, designPath)
			: null,
		contextDir: resolved.contextDir,
		productContextDir: productPath
			? path.dirname(productPath)
			: null,
		designContextDir: designPath
...
function resolveContext(cwd = process.cwd(), options = {}) {
	...
	let productPath =
		(projectContextDir
			...
			: null) ||
		(rootContextDir
			? firstExisting(rootContextDir, PRODUCT_NAMES)
			: null);
	let designPath =
		(projectContextDir
			...
			: null) ||
		(rootContextDir
			? firstExisting(rootContextDir, DESIGN_NAMES)
			: null);
	...
	if (!productPath && !designPath) {
		...
		if (envContextDir) {
			productPath = firstExisting(
				envContextDir,
				PRODUCT_NAMES,
			);
			designPath = firstExisting(
				envContextDir,
				DESIGN_NAMES,
			);
		}
...
export function resolveTargetSelection(
	...
	if (
		...
		!project.projectRoot ||
		!project.repoRoot ||
		path.resolve(project.projectRoot) !==
			path.resolve(project.repoRoot)
	) {
		return null;
	}
	const targetCandidates = discoverTargetCandidates(
		project.repoRoot,
	);
...
function resolveProject(cwd = process.cwd(), options = {}) {
	...
	if (!repoRoot && targetDir !== absCwd) {
		...
		if (
			cwdRepoRoot &&
			isPathInside(targetDir, cwdRepoRoot)
		) {
	...
	return {
		targetDir,
		projectRoot:
			resolveWorkspaceProjectRoot(repoRoot, targetDir) ||
			repoRoot,
		repoRoot,
		isMonorepo: true,
	};
...
function resolveLocalContextDir(root) {
	if (
		firstExisting(root, [...PRODUCT_NAMES, ...DESIGN_NAMES])
	) {
		return root;
	}
...
function isMonorepoRoot(dir) {
	if (
		readWorkspacePatterns(dir).some(
			(pattern) =>
				!normalizeWorkspacePattern(pattern).startsWith("!"),
		)
	)
		return true;
	if (
...
function discoverTargetCandidates(repoRoot) {
	...
	for (const pattern of patterns) {
		for (const root of discoverRootsForPattern(
			repoRoot,
			pattern,
		)) {
			roots.set(
	...
	return (
		[...roots.entries()]
			...
			.map(([rel, root]) => {
				const targetExample = findTargetExample(
					repoRoot,
					root,
				);
				return {
...
function resolveCandidateContextSummary(
	repoRoot,
	projectRoot,
	targetPath,
) {
	...
	return {
		productStatus: contextSourceStatus(
			ctx.productPath,
			repoRoot,
			projectRoot,
		),
		productPath: contextSourcePath(
			ctx.productPath,
			repoRoot,
		),
		designStatus: contextSourceStatus(
			ctx.designPath,
			repoRoot,
			projectRoot,
		),
...
function contextSourceStatus(
	...
	if (isPathInsideOrEqual(absPath, absProjectRoot)) {
		return path.dirname(absPath) === absProjectRoot
			? "child"
			: "fallback";
	}
	if (
...
function discoverRootsForPattern(repoRoot, rawPattern) {
	...
	const firstGlobIndex = segments.findIndex((segment) =>
		segment.includes("*"),
	);
	const literalPrefix =
		firstGlobIndex === -1
			? segments
			: segments.slice(0, firstGlobIndex);
...
function isIgnoredWorkspaceDiscoveryDir(name) {
	return (
		name.startsWith(".") ||
		WORKSPACE_DISCOVERY_IGNORED_DIRS.has(name)
	);
}
...
function resolveWorkspaceProjectRoot(repoRoot, targetDir) {
	...
	const patterns = readWorkspacePatterns(repoRoot);
	const excluded = isExcludedByWorkspacePattern(
		relSegments,
		patterns,
	);
	if (!excluded) {
	...
	const nearest = nearestProjectLikeRoot(
		repoRoot,
		targetDir,
	);
...
function isExcludedByWorkspacePattern(
	relSegments,
	patterns,
) {
	return patterns.some((rawPattern) => {
		...
		return workspacePatternMatchesRel(
			pattern.slice(1),
			relSegments,
		);
	});
...
function nearestPackageRootBetween(
	repoRoot,
	targetDir,
	stopDir,
) {
...
function workspacePatternMatchesRel(pattern, relSegments) {
	...
	if (patternSegments.includes("**")) {
		const firstGlobIndex = patternSegments.findIndex(
...
function readLernaWorkspaces(repoRoot) {
	...
	return Array.isArray(lerna?.packages)
		? lerna.packages
		: [];
}
...
function projectRootFromWorkspacePattern(
	repoRoot,
	relSegments,
	rawPattern,
) {
...
function projectRootFromDoubleStarPattern(
	...
	const packageRoot = nearestPackageRootBetween(
		repoRoot,
		targetDir,
		prefixDir,
	);
	...
	return path.join(
		repoRoot,
		...relSegments.slice(0, literalPrefix.length + 1),
	);
}
...
function readUpdateCache() {
	try {
		return JSON.parse(
			fs.readFileSync(UPDATE_CACHE_PATH, "utf-8"),
		);
	} catch {
		return {};
	}
...
function writeUpdateCache(cache) {
	try {
		fs.mkdirSync(path.dirname(UPDATE_CACHE_PATH), {
			recursive: true,
		});
		fs.writeFileSync(
			UPDATE_CACHE_PATH,
...
async function cli() {
	...
	if (selection) {
		process.stdout.write(
			buildTargetSelectionDirective(selection) + "\n",
		);
		process.exit(0);
	}
	...
	if (!ctx.hasProduct) {
		...
		parts.push(
			buildResolvedContextDirective(ctx, cliOptions, {
				targetExists,
			}),
		);
		if (
			shouldWarnMissingTarget(
				ctx,
				targetProvided,
				targetExists,
			)
...
function buildResolvedContextDirective(
	...
	const targetPath = hasTargetOption(options)
		? options.targetPath
		: null;
...
function shouldWarnMissingTarget(
	ctx,
	targetProvided,
	targetExists = null,
) {
