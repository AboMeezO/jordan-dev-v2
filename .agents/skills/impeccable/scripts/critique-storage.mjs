 ...
 * phrase.
 */
export function slugFromTarget(
	resolved,
	{ cwd = process.cwd() } = {},
) {
...
function serializeFrontmatter(obj) {
	...
	for (const [key, value] of Object.entries(obj)) {
		...
		const needsQuotes =
			typeof value === "string" && /[:#]/.test(str);
		lines.push(
			`${key}: ${needsQuotes ? JSON.stringify(str) : str}`,
		);
	}
	lines.push("---");
	return lines.join("\n");
...
export function readTrend(
	slug,
	{ limit = 5, cwd = process.cwd() } = {},
) {
	const all = listSnapshotsForSlug(slug, cwd);
	const slice = all.slice(-limit);
	return slice.map((file) =>
		parseFrontmatter(fs.readFileSync(file, "utf-8")),
	);
}
