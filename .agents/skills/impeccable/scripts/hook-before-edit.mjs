...
function proposedContent(event, cwd, filePath) {
	...
	const editProjection = projectedEditContent(
		input,
		filePath,
		cwd,
	);
...
function shellTeeDestination(command) {
	...
	const teeIndex = words.findIndex(
		(word) => path.basename(word) === "tee",
	);
...
function shellCopiedFileContent(command, cwd) {
	...
	if (
		SENSITIVE_PATH.test(sourcePath) ||
		GENERATED_PATH.test(sourcePath)
	)
		return "";
	try {
...
function shellHereDocContent(command) {
	...
	const start =
		(markerMatch.index || 0) + markerMatch[0].length;
...
function findingSignature(findings) {
	return findings
		.map(
			(finding) =>
				`${finding.antipattern || "unknown"}:${finding.line || 0}`,
		)
		.sort()
		.join("|");
}

function bumpCursorDenial(
	cache,
	sessionId,
	filePath,
	findings,
	...
	const fileEntry = session.files[filePath] || {
		editCount: 0,
		findings: [],
	};
	...
	fileEntry.cursorDenials =
		fileEntry.cursorDenials &&
		typeof fileEntry.cursorDenials === "object"
			? fileEntry.cursorDenials
			: {};
	fileEntry.cursorDenials[key] =
		(fileEntry.cursorDenials[key] || 0) + 1;
...
async function main() {
	...
	const contentResult = proposedContent(
		event,
		cwd,
		filePath,
	);
	...
	const content =
		typeof contentResult === "string" ? contentResult : "";
	if (!content)
		return allow({
			...audit,
	...
	if (
		!detector ||
		typeof detector.detectText !== "function"
	) {
		return allow({
			...audit,
	...
	let findings = [];
	try {
		findings = await detector.detectText(
			content,
			filePath,
			scanOptions,
		);
...
main().catch((err) => {
	if (process.env.IMPECCABLE_HOOK_DEBUG) {
		process.stderr.write(
			`[impeccable-hook-before-edit] ${err}\n`,
		);
	}
