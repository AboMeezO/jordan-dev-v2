...
export function getImpeccableDir(
	cwd = process.cwd(),
	options = {},
) {
	return path.join(
...
export function getDesignSidecarPath(
	cwd = process.cwd(),
	options = {},
) {
	return path.join(
		getImpeccableDir(cwd, options),
		"design.json",
	);
}
...
export function getLiveConfigPath(
	cwd = process.cwd(),
	options = {},
) {
...
export function resolveLiveConfigPath({
	...
	if (
		...
		env.IMPECCABLE_LIVE_CONFIG.trim()
	) {
...
export function getLiveServerPath(
	cwd = process.cwd(),
	options = {},
) {
...
export function getLegacyLiveServerPath(
	cwd = process.cwd(),
	options = {},
) {
	return path.join(
...
export function readLiveServerInfo(
	cwd = process.cwd(),
	options = {},
) {
...
export function writeLiveServerInfo(
	cwd = process.cwd(),
	info,
	options = {},
) {
...
export function removeLiveServerInfo(
	cwd = process.cwd(),
	options = {},
) {
	for (const filePath of [
...
export function getLiveSessionsDir(
	cwd = process.cwd(),
	options = {},
) {
...
export function getLegacyLiveSessionsDir(
	cwd = process.cwd(),
	options = {},
) {
	return path.join(
...
export function getLiveAnnotationsDir(
	cwd = process.cwd(),
	options = {},
) {
...
export function getCritiqueDir(
	cwd = process.cwd(),
	options = {},
) {
	return path.join(
...
export function getLegacyLiveAnnotationsDir(
	cwd = process.cwd(),
	options = {},
) {
	return path.join(
...
function firstExisting(paths) {
	return (
		paths.find((filePath) => fs.existsSync(filePath)) ||
		null
	);
}
