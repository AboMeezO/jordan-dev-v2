...
function cloneDetectionConfig() {
	return {
		...
		ignoreValues: [],
		designSystem: {
			...DEFAULT_DETECTION_CONFIG.designSystem,
		},
	};
}
...
function normalizeDetectionConfigForWrite(config) {
	...
	out.ignoreValues = normalizeIgnoreValueEntries(
		config?.ignoreValues || [],
	);
	if (
		config?.designSystem &&
		...
		out.designSystem = {
			enabled:
				config.designSystem.enabled === false
					? false
					: true,
		};
	}
...
export function filterDetectionFindings(findings, config) {
	...
	return findings.filter((finding) => {
		...
		if (
			ignoreRules.has(
				normalizeIgnoreRule(finding.antipattern),
			)
		)
			return false;
...
function resolveGitDir(root) {
	...
	try {
		...
		const match = readFileSync(dotGit, "utf-8").match(
			/gitdir:\s*(.+)/,
		);
		if (match) {
