...
function matchCanonicalSection(name) {
	const normalized =
		normalizeApostrophes(name).toLowerCase();
	// Exact match first
	for (const c of CANONICAL_SECTIONS) {
		if (
			normalizeApostrophes(c).toLowerCase() === normalized
		)
			return c;
	}
...
function extractNamedRules(lines) {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		const headerName = stripBold(h3[1])
			...
			.trim();
		if (
			!/^The\b.*\b(Rule|Fallback|Principle)\b/i.test(
				headerName,
			)
		)
			continue;
	...
	for (const b of collectBullets(lines)) {
		...
		const nameRaw = mm[1]
			...
			.trim();
		if (
			!/^The\b.+\b(Rule|Fallback|Principle)$/i.test(nameRaw)
		)
			continue;
...
function extractColors(section) {
	...
	const groups = [];
	const ROLE_KEYWORDS =
		/^(primary|secondary|tertiary|neutral|accent)\b/i;

	for (const sub of subs.slice(1)) {
		if (
			!sub.name ||
			/Named Rules?/i.test(sub.name) ||
			/^The\s/i.test(sub.name)
		)
			continue;
...
function buildColor(name, rawValue, description) {
	...
	return {
		...
		description:
			stripBold(description || "").trim() || null,
	};
}
...
function extractTypography(section) {
	...
	const subs = splitSubsections(section.lines);
	let hierarchy = [];
	const hierSub = subs.find(
		(s) => s.name && /hierarch/i.test(s.name),
	);
	if (hierSub) {
...
function extractElevation(section) {
	...
	const description =
		collectParagraphs(subs[0].lines).join(" ") || null;
	...
	while ((m = re.exec(text)) !== null) {
		...
		if (nameMatch) {
			const stripped = nameMatch[1]
				.replace(
					/^(?:use|using|apply|applying|is|are|looks? like)\s+/i,
					"",
				)
...
function parseShadowBullet(bullet) {
	...
	const looksLikeShadow =
		/box-shadow|rgba?\(|\bpx\b|\brem\b|^-?\d+\s/i.test(
			rawValue,
		) && /\d/.test(rawValue);
...
function extractDosDonts(section) {
	...
	for (const sub of subs.slice(1)) {
		...
		const bullets = collectBullets(sub.lines).map((b) =>
			stripBold(b).trim(),
		);
		if (
			/^do'?t?:?$/i.test(subName) ||
	...
	for (const b of collectBullets(section.lines)) {
		const stripped = normalizeApostrophes(
			stripBold(b).trim(),
		);
		if (/^don'?t\b/i.test(stripped)) {
			if (
				!donts.some(
					(d) => normalizeApostrophes(d) === stripped,
				)
			)
				donts.push(stripped);
...
function assessCoverage(model) {
	...
	report.overview = model.overview
		? {
				northStar: Boolean(
					model.overview.creativeNorthStar,
				),
				philosophy: model.overview.philosophy.length > 0,
				keyCharacteristics:
					model.overview.keyCharacteristics.length,
			}
		: "missing";
