...
The agent should insert variant HTML at insertLine.`);
	...
	if (!elementId && !classes && !query) {
		console.error(
			"Need at least one of: --element-id, --classes, --query",
		);
		process.exit(1);
	}
	...
	const reindentOriginal = (extra) =>
		originalLines
			.map((l) =>
				l.trim() === ""
					? ""
					: indent + extra + l.slice(originalBaseIndent),
			)
			.join("\n");
	...
	if (useSvelteComponent) {
		...
		// extra line count.
		insertLine =
			startLine + 6 + (originalLines.length - 1) + 1;
	}
	...
	console.log(
		JSON.stringify({
			file: outputRelFile,
			sourceFile: useSvelteComponent
				? relTargetFile
				: undefined,
			previewMode: useSvelteComponent
				? "svelte-component"
				: undefined,
			componentDir: svelteSession?.componentDir,
			propContract: svelteSession?.propContract,
			sourceStartLine: useSvelteComponent
				? startLine + 1
				: undefined,
			...
			styleMode: useSvelteComponent
				? "svelte-component"
				: styleMode.mode,
			styleTag: useSvelteComponent
				? null
				: styleMode.styleTag,
...
function manualEditLocatorMatchesSelection(
	op,
	originalLines,
) {
	if (
	...
	return originalLines.some(
		(line) =>
			line.includes(op.originalText) &&
			lineMatchesManualEditLocator(line, op),
	);
}

function applyBufferedManualEditToLines(
	originalLines,
	selectionStartLine,
	op,
) {
	...
	if (
		countOccurrences(originalBlock, op.originalText) === 1
	) {
		return {
			lines: replaceOnce(
				originalBlock,
...
function buildCssAuthoring(styleMode, count) {
	...
	if (styleMode.mode === "astro-global-prefixed") {
		return {
			...
			rulePattern:
				'[data-impeccable-variant="N"] > .variant-class { ... }',
...
function searchDir(dir, query, seen, depth, genOpts) {
	...
	for (const entry of entries) {
		...
		if (
			!genOpts.includeGenerated &&
			isGeneratedFile(filePath, genOpts)
		)
			continue;
		try {
...
function findOpenerLine(lines, matchLine, tag) {
	...
	for (
		let i = matchLine - 1;
		i >= Math.max(0, matchLine - MAX_BACKWALK);
		i--
	) {
...
function findClosingLine(lines, start) {
	...
	for (let i = start; i < lines.length; i++) {
		...
		const selfCloses = (line.match(selfCloseRe) || [])
			.length;
