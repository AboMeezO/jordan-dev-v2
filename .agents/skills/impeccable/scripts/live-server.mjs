...
function chatAgentLikelyActive() {
	...
	return (
		Date.now() - state.lastPollAt < CHAT_POLL_FRESHNESS_MS
	);
}
...
function summarizePendingEventForStatus(entry) {
	...
	const summary = {
		...
		type: event.type,
		leased: !!(
			entry.leaseUntil && entry.leaseUntil > Date.now()
		),
	...
	if (event.type === "manual_edit_apply") {
		...
		summary.agentAction =
			event.agentAction ||
			manualApply.buildAgentAction(event);
		summary.manualApplySummary = manualApply.summarizeEvent(
			event,
			manualApply.getDeferred(event.id)?.batch ||
				event.batch,
		);
	}
	return summary;
...
function activeSessionSummaries() {
	...
	return state.sessionStore
		.listActiveSessions()
		.map((snapshot) =>
			summarizeActiveSessionForClient(snapshot),
		);
}
...
function cancelQueuedAnonymousExitEvents() {
	let removed = 0;
	for (
		let i = state.pendingEvents.length - 1;
		i >= 0;
		i -= 1
	) {
...
function scheduleLeaseFlush() {
	...
	state.leaseTimer = setTimeout(
		...
		Math.max(
			...
			nextLeaseUntil -
				now +
				POLL_LEASE_EXPIRY_TIMER_GRACE_MS,
		),
	);
}
...
function getManualEditStatus() {
	try {
		const { totalCount, perPage } = countPendingByPage(
			process.cwd(),
		);
		return {
			totalCount,
...
function loadBrowserScripts() {
	...
	const liveScriptParts =
		resolveLiveBrowserScriptParts(__dirname);
	try {
	...
	return (req, res) => {
		...
		if (p === "/live.js") {
			...
			try {
				...
				res.end(
					"Error reading live browser scripts: " +
						err.message,
				);
				return;
			}
		...
		if (p === "/annotation" && req.method === "POST") {
			...
			req.on("end", () => {
				...
				res.end(
					JSON.stringify({ ok: true, path: absPath }),
				);
			});
			req.on("error", () => {
				if (!aborted) {
		...
		if (
			...
			if (jsonStat) {
				try {
					response.sidecar = JSON.parse(
						fs.readFileSync(jsonPath, "utf-8"),
					);
				} catch (err) {
					response.sidecarError =
						"Failed to parse .impeccable/design.json: " +
						err.message;
				}
			}
...
function sessionFileMetadataFromPollReply(file) {
	...
	try {
		...
		if (
			...
			return base;
		return {
			file: String(manifest.sourceFile)
				.split(path.sep)
				.join("/"),
			sourceFile: String(manifest.sourceFile)
				.split(path.sep)
				.join("/"),
...
function handlePollPost(req, res) {
	...
	req.on("end", () => {
		...
		const pendingApplyDeferred = manualApply.getDeferred(
			msg.id,
		);
		if (pendingApplyDeferred) {
			...
			if (!validation.ok) {
				recordManualEditActivity(
					"manual_edit_apply_reply_invalid",
					{
						id: msg.id,
						pageUrl: pendingApplyDeferred.pageUrl,
						chunk:
							pendingApplyDeferred.event?.chunk || null,
						repair:
							pendingApplyDeferred.event?.repair || null,
						reason:
			...
			recordManualEditActivity(
				"manual_edit_apply_reply_received",
				{
					id: msg.id,
					pageUrl: pendingApplyDeferred.pageUrl,
					...
					appliedCount:
						validation.result.appliedEntryIds.length,
					failed: summarizeManualApplyFailures(
		...
		if (manualApply.hasTimedOutId(msg.id)) {
			const rollback =
				manualApply.rollbackTimedOutReply(msg);
			recordManualEditActivity(
				"manual_edit_apply_stale_reply_rejected",
				{
					id: msg.id,
					rolledBackFileCount:
						rollback.rolledBackFiles?.length || 0,
			...
			res.end(
				JSON.stringify({
					error: "stale_manual_edit_apply_reply",
					...rollback,
				}),
			);
			return;
		}
		const pendingEventBeforeAck = findPendingEventById(
			msg.id,
		);
		if (
		...
		const acknowledgedEvent = acknowledgePendingEvent(
			msg.id,
		);
		...
		if (
			...
			try {
				existingSession = state.sessionStore.getSnapshot(
					msg.id,
		...
		const replyFileMeta = sessionFileMetadataFromPollReply(
			msg.file,
		);
...
function applyLegacyDeferredAcceptsOnStartup() {
	try {
		const result = applyDeferredSvelteComponentAccepts(
			process.cwd(),
		);
...
if (args.includes("stop")) {
	...
	try {
		const { info } =
			readLiveServerInfo(process.cwd()) || {};
		const res = await fetch(
			`http://localhost:${info.port}/stop?token=${info.token}`,
		);
		if (res.ok)
			console.log(
				`Stopped live server on port ${info.port}.`,
			);
	} catch {
		...
		try {
			...
			if (line) {
				try {
					...
					if (j.removed === true) {
						console.log(
							`Removed live script tag from ${j.file}.`,
						);
					}
				} catch {
...
state.sessionStore = createLiveSessionStore({
	cwd: process.cwd(),
});
...
state.sessionDir = fs.mkdtempSync(
	path.join(annotRoot, "session-"),
);

const { detectScript, liveScriptParts } =
	loadBrowserScripts();
