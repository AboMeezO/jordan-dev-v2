...
Output (JSON):
  { handled, file, carbonize }`);
		...
		if (isDiscard) {
			...
			console.log(
				JSON.stringify({
					...
					componentDir:
						svelteComponentManifest.componentDir,
				}),
			);
			return;
		}
	...
	if (isDiscard) {
		...
		const acceptedOriginalText =
			result.acceptedOriginalText || "";
...
function originalBlockHasExactManualText(
	originalBlock,
	text,
) {
...
function scrubManualEditsAgainstFile(
	...
	return scrubManualEditsAgainstOriginalBlock(
		originalBlockText,
		cwd,
		pageUrl,
	);
...
function handleDiscard(id, lines, targetFile) {
	...
	const isJsx =
		detectCommentSyntax(targetFile).open === "{/*";
	const replaceRange = expandReplaceRange(
		block,
		lines,
		isJsx,
	);
	...
	const indent =
		lines[replaceRange.start].match(/^(\s*)/)[1];
...
function buildCarbonizeReplacement({
	...
	const pushCarbonizeBody = (bodyIndent) => {
		const bodyRestored = reindentContent(
			restored,
			indent,
			bodyIndent + "  ",
		);
...
function handleAccept(
	...
	const variantContent = extractVariant(
		lines,
		block,
		variantNum,
	);
	...
	const hasHelperAttrs = variantText.includes(
...
function readSourceShadowPreviewMeta(content, id) {
	...
	const tag = match[0];
	if (
		readHtmlAttr(tag, "data-impeccable-preview") !==
		"source-shadow"
	)
		return null;
	const sourceFile = readHtmlAttr(
		tag,
		"data-impeccable-source-file",
	);
	const sourceStartLine = Number(
		readHtmlAttr(tag, "data-impeccable-source-start"),
	);
	const sourceEndLine = Number(
		readHtmlAttr(tag, "data-impeccable-source-end"),
	);
	if (
		!sourceFile ||
	...
	for (let i = block.start; i <= block.end; i++) {
		...
		if (!inStyle) {
			...
			if (closeIdx !== -1) {
				inStyle = false;
				out.push(
					line.slice(closeIdx).replace(/<\/style\s*>/, ""),
				);
			}
