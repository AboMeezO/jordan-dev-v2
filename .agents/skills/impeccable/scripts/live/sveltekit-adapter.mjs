...
export function detectSvelteKitProject(
	cwd = process.cwd(),
	config = null,
) {
...
export function applySvelteKitLiveAdapter({
	...
	fs.mkdirSync(path.dirname(layoutAbs), {
		recursive: true,
	});
...
export function unpatchSvelteLayout(content) {
	...
	out = out.replace(
		new RegExp(
			"^\\s*" +
				escapeRegExp(SVELTE_ROOT_IMPORT) +
				"\\s*\\n?",
			"gm",
		),
		"",
...
export function ensureSvelteLiveRootComponent(cwd, port) {
	...
	fs.writeFileSync(
		file,
		buildSvelteLiveRootComponent(port),
		"utf-8",
	);
	return file;
}
...
function findSvelteKitAppHtml(cwd, config) {
	...
	return fs.existsSync(path.join(cwd, fallback))
		? fallback
		: null;
}
