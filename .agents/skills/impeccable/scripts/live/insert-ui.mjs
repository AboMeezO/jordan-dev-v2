 ...
 */
export function canCreateInsert({
	prompt,
	comments,
	strokes,
}) {
...
export function insertCreateDisabledReason({
	prompt,
	comments,
	strokes,
}) {
 ...
 * @param {InsertAxis} [axis]
 */
export function insertLineCoords(
	rect,
	position,
	axis = "column",
) {
	if (axis === "row") {
		...
		const x =
			position === "before" ? rect.left - 2 : right + 2;
		return {
			axis: "row",
			top: rect.top,
			left: x,
	...
	const y =
		position === "before" ? rect.top - 2 : bottom + 2;
	return {
		axis: "column",
		top: y,
		left: rect.left,
 ...
 */
export function hitSiblingInsertGap(
	clientX,
	clientY,
	siblings,
	opts = {},
	...
	for (const row of groupSiblingRows(siblings)) {
		...
		for (let i = 0; i < sorted.length - 1; i++) {
			...
			const inX =
				clientX >= aRight - slop && clientX <= bLeft + slop;
			const inY =
 ...
 */
export function clampPlaceholderSize(
	width,
	height,
	parentWidth,
	opts = {},
	...
	const maxW =
		opts.maxWidth ?? Math.max(minW, parentWidth || minW);
	return {
		width: Math.min(
			maxW,
			Math.max(minW, Math.round(width)),
		),
...
export function applyInsertToggle(
	pickActive,
	insertActive,
) {
 ...
 */
export function findInsertAnchorInDom(
	doc,
	snapshot,
	liveAnchor = null,
) {
	if (liveAnchor && doc.body.contains(liveAnchor))
		return liveAnchor;
	if (!snapshot) return null;
	const tag = (snapshot.anchorTag || "div").toLowerCase();
	const cls = (snapshot.anchorClasses || "")
		.split(/\s+/)
		.filter(Boolean)[0];
