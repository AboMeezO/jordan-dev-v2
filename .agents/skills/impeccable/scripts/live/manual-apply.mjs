...
const APPLY_EVENT_HARD_TIMEOUT_MS = Number(
	process.env.IMPECCABLE_LIVE_APPLY_EVENT_HARD_TIMEOUT_MS ||
		150_000,
);
...
export function createManualApplyController({
	...
	function pushApplyEventAndWait(
		batch,
		pageUrl,
		chunk = null,
		repair = null,
		...
		const eventId = randomUUID()
			.replace(/-/g, "")
			.slice(0, 8);
		const evidencePath = writeManualApplyEvidence(
			eventId,
			batch,
			cwdValue,
		);
		...
		const rollbackSnapshot = snapshotApplyEventFiles(
			batch,
			cwdValue,
		);
		recordManualEditActivity(
			"manual_edit_apply_dispatched",
			{
				...
				chunk,
				repair,
				entryCount: Array.isArray(batch.entries)
					? batch.entries.length
					: 0,
				opCount: countManualApplyOps(batch),
				fileCount: collectManualApplyFiles(
					batch,
					[],
					cwdValue,
				).length,
	...
	async function pushBatchInChunksAndWait(
		batch,
		pageUrl,
		context = {},
	) {
		...
		if (repair)
			return pushApplyEventAndWait(
				batch,
				pageUrl,
				null,
				repair,
			);
		const chunks = splitManualApplyBatch(
			batch,
		...
		for (const chunk of chunks) {
			...
			const reportedAppliedIds = new Set(
				result.appliedEntryIds,
			);
			for (const entryId of reportedAppliedIds) {
				if (
					!chunk.entryIds.has(entryId) ||
					chunkFailedIds.has(entryId)
				)
					continue;
			...
			for (const entryId of chunk.entryIds) {
				if (
					reportedAppliedIds.has(entryId) ||
					chunkFailedIds.has(entryId)
				)
					continue;
				if (!failedByEntry.has(entryId)) {
	...
	function rejectDeferred(eventId, reason) {
		...
		deferred.reject(
			new Error(reason || "chat_agent_error"),
		);
		return true;
	}

	function referencedManualApplyEvidencePaths(
		cwdValue = projectCwd(),
	) {
	...
	function pruneStaleEvidence(cwdValue = projectCwd()) {
		...
		const referenced =
			referencedManualApplyEvidencePaths(cwdValue);
		const removed = [];
	...
	return {
		...
		readTransaction: () =>
			readManualApplyTransaction(projectCwd()),
		rejectDeferred,
		resolveDeferred,
...
export function manualEditApplyChunkSize(
	env = process.env,
) {
	const raw = Number(
		env.IMPECCABLE_LIVE_MANUAL_EDIT_CHUNK_SIZE,
	);
...
export function writeManualApplyEvidence(
	eventId,
	batch,
	cwd = process.cwd(),
) {
...
export function manualApplyEvidenceDir(
	cwd = process.cwd(),
) {
...
export function removeManualApplyEvidence(
	evidencePath,
	cwd = process.cwd(),
) {
	const fullPath = normalizeManualApplyEvidencePath(
...
export function compactManualApplyBatch(
	batch = {},
	cwd = process.cwd(),
) {
	const entries = (batch.entries || []).map(
		compactManualApplyEntry,
	);
	const candidates = compactManualApplyCandidates(
		batch.candidates || [],
		cwd,
	);
	return {
...
export function compactManualApplyCandidates(
	candidates,
	cwd = process.cwd(),
) {
	return (Array.isArray(candidates) ? candidates : [])
		...
		.map((candidate) => ({
			...
			ref: candidate.ref,
			sourceHint: compactManualApplySourceMatch(
...
function compactManualApplySourceMatches(
	matches,
	limit,
	cwd,
) {
	return (Array.isArray(matches) ? matches : [])
		.slice(0, limit)
		.map((match) =>
			compactManualApplySourceMatch(match, cwd),
		)
		.filter(Boolean);
}
...
function compactManualApplyOp(op = {}) {
	return {
		...
		nearbyEditableTexts: compactNearbyManualEditTexts(
...
function normalizeApplyChunkResult(result) {
	...
	return {
		...
		appliedEntryIds: Array.isArray(result?.appliedEntryIds)
			? result.appliedEntryIds.filter(
					(id) => typeof id === "string",
				)
			: [],
		failed: Array.isArray(result?.failed)
			? result.failed.filter(Boolean)
			: [],
...
function invalidManualApplyResult(
	reason,
	eventId,
	extra = {},
) {
...
export function validateManualApplyResultMessage(
	msg,
	deferred,
) {
	let data = msg?.data;
	const eventId =
		msg?.id || deferred?.event?.id || "EVENT_ID";
	if (
		!data ||
		typeof data !== "object" ||
		Array.isArray(data)
	...
	for (const [
		index,
		value,
	] of data.appliedEntryIds.entries()) {
	...
	for (const entryId of data.appliedEntryIds) {
		if (
			eventEntryIds.size > 0 &&
			!eventEntryIds.has(entryId)
		) {
			return invalidManualApplyResult(
	...
	for (const item of data.failed) {
		if (
			eventEntryIds.size > 0 &&
			!eventEntryIds.has(item.entryId)
		) {
			return invalidManualApplyResult(
...
function markChunkEntriesFailed(
	failedByEntry,
	chunk,
	reason,
) {
...
export function splitManualApplyBatch(batch, maxOps) {
	...
	for (const entry of batch?.entries || []) {
		...
		if (ops.length <= maxOps) {
			if (
				current.opCount > 0 &&
				current.opCount + ops.length > maxOps
			) {
	...
	return rawChunks.map((chunk, index) => ({
		batch: {
			...
			ops: chunk.ops,
			candidates: filterManualApplyChunkCandidates(
				batch,
				chunk.refsByEntry,
			),
			context: {
...
function filterManualApplyChunkCandidates(
	batch,
	refsByEntry,
) {
...
export function snapshotApplyEventFiles(
	batch,
	cwd = process.cwd(),
) {
	const snapshot = new Map();
	for (const relativeFile of collectManualApplyFiles(
		batch,
		[],
		cwd,
	)) {
...
export function manualApplyTransactionPath(
	cwd = process.cwd(),
) {
	return path.join(
		getLiveDir(cwd),
		"manual-edit-apply-transaction.json",
	);
}

export function readManualApplyTransaction(
	cwd = process.cwd(),
) {
...
export function rollbackManualApplyTransaction({
	...
	const shouldRollback = (transaction.entryIds || []).some(
	...
	for (const item of transaction.files || []) {
		const relativeFile = normalizeProjectFile(
			item.file,
			cwd,
		);
...
export function rollbackApplySnapshot(
	...
	const scope = collectManualApplyFiles(
		batch,
		extraFiles,
		cwd,
	);
...
function manualApplyReplyCommand(eventOrId = "EVENT_ID") {
	const id =
		typeof eventOrId === "string"
			? eventOrId
			: eventOrId?.id || "EVENT_ID";
	return `live-poll.mjs --reply ${id} done --data '<json>'`;
}

export function buildManualApplyAgentAction(
	eventOrId = "EVENT_ID",
) {
	return {
...
export function summarizeManualApplyFailures(
	failed,
	cwd = process.cwd(),
) {
	...
	return failed.slice(0, 20).map((item) => ({
		...
		failures: summarizeManualDiagnostics(
			item.failures,
			cwd,
		),
		candidates: summarizeManualDiagnostics(
			item.candidates,
			cwd,
		),
	}));
...
export function summarizeManualDiagnostics(
	items,
	cwd = process.cwd(),
) {
	...
	return items.slice(0, 12).map((item) => ({
		...
		message: compactManualLogText(item.message, 300),
		file: summarizeManualLogFile(
			item.file || item.relativeFile,
			cwd,
		),
...
export function summarizeManualLogFile(
	file,
	cwd = process.cwd(),
) {
