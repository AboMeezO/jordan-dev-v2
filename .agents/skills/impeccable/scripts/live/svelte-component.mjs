...
export const SVELTE_COMPONENT_ROOT =
...
export function shouldUseSvelteComponentInjection(
	filePath,
) {
	if (
		/^(0|false|no)$/i.test(
			process.env.IMPECCABLE_LIVE_SVELTE_COMPONENT || "",
		)
	)
		return false;
...
export function componentSessionDir(
	id,
	cwd = process.cwd(),
) {
...
export function parseSvelteComponentFile(content) {
	...
	const withoutScript = scriptMatch
		? text.slice(scriptMatch[0].length)
		: text;
	const styleMatch = withoutScript.match(
	...
	while (
		cssLines.length > 0 &&
		cssLines[cssLines.length - 1].trim() === ""
	)
		cssLines.pop();
...
export function scaffoldSvelteComponentSession({
	...
	const originalWithProps = substituteExprsWithProps(
		originalMarkup,
		contract,
	);
...
export function findSvelteComponentManifest(
	id,
	cwd = process.cwd(),
) {
...
export function resolveSourceFile(
	...
	if (!fs.existsSync(full)) {
		throw new Error(
			"Svelte-component source file not found: " +
				sourceFile,
		);
	}
	return full;
...
function sanitizeAcceptedSvelteCss(
	...
	if (
		!/data-impeccable-variant|impeccable-variant-ready/.test(
			css,
		)
	)
		return cssLines;
...
function appendSanitizedCssRule(
	...
	if (
		!prelude ||
		!body ||
		/--impeccable-variant-ready\s*:/.test(body)
	)
		return;
	...
	if (/^@scope\b/i.test(prelude)) {
		...
		for (const innerRule of inner) {
			const rewrittenPrelude =
				rewriteAcceptedSvelteSelector(
					innerRule.prelude,
					variantNum,
					paramValues,
					rootTag,
			...
			if (
				!rewrittenPrelude ||
				/--impeccable-variant-ready\s*:/.test(
					innerRule.body,
				)
			)
				continue;
...
function rewriteAcceptedSvelteSelectorPart(
	...
	if (hasVariant) {
		...
		out = out.replace(
			/\[data-impeccable-variant=(["']).*?\1\]/g,
			"",
		);
	}

	const paramResult = rewriteParamSelectors(
		out,
		paramValues,
	);
...
export function inlineSvelteComponentAccept(
	...
	const restoredMarkup = substitutePropsWithExprs(
		mergedMarkup,
		contract,
	)
		.split("\n")
	...
	const bakedCss = bakeParamValuesInCss(
		sanitizedCss,
		paramValues,
	);
	...
	try {
		...
		return {
			handled: false,
			error:
				"Failed to write Svelte source: " + err.message,
			...resultBase,
		};
	}
...
function inlineSvelteComponentInsertAccept({
	...
	if (
		...
		return {
			handled: false,
			error:
				"Invalid insert line for " + manifest.sourceFile,
			...resultBase,
		};
	}

	const nearbyLine =
		sourceLines[insertIndex] ??
		sourceLines[insertIndex - 1] ??
		"";
...
function mergeOriginalTopLevelAttrs(
	markup,
	originalMarkup,
) {
	...
	if (originalClass && variantClass) {
		const merged = mergeStaticClassAttr(
			originalClass,
			variantClass,
		);
		if (merged) {
...
function mergeStaticClassAttr(originalClass, variantClass) {
	const originalValue = originalClass.raw.match(
		/class\s*=\s*(["'])(.*?)\1/,
	);
	const variantValue = variantClass.raw.match(
		/class\s*=\s*(["'])(.*?)\1/,
	);
...
export function removeSvelteComponentSession(
	id,
	cwd = process.cwd(),
) {
	const dir = componentSessionDir(id, cwd);
...
export function removeAllSvelteComponentSessions(
	cwd = process.cwd(),
) {
	const root = path.join(cwd, SVELTE_COMPONENT_ROOT);
...
export function writeDeferredAccept(
	entry,
	cwd = process.cwd(),
) {
	const file = deferredAcceptsPath(cwd);
	...
	data.accepts = (data.accepts || []).filter(
...
export function applyDeferredSvelteComponentAccepts(
	cwd = process.cwd(),
) {
	const file = deferredAcceptsPath(cwd);
	const data = readDeferredAccepts(cwd);
	const pending = Array.isArray(data.accepts)
		? data.accepts
		: [];
	const results = [];
	const remaining = [];
	for (const entry of pending) {
		try {
			const manifest = findSvelteComponentManifest(
				entry.id,
				cwd,
			);
			if (!manifest) {
