	...
	return function handleManualEditRoute(req, res, url) {
		...
		if (
			...
			const existingTransaction =
				manualApply.readTransaction();
			if (repairOnly && !existingTransaction) {
				sendJson(res, 409, {
					error: "manual_edit_repair_transaction_missing",
				});
				return true;
			}
				...
				"manual_edit_commit_started",
				{
					...
					recoveredTransaction: recoveredTransaction
						? {
								...
								rolledBackFiles:
									recoveredTransaction.rolledBackFiles,
								rollbackFailures:
									summarizeManualDiagnostics(
						...
						: null,
					...summarizePendingManualEditBatch(
						projectCwd(),
						pageUrl,
					),
				},
			...
			(async () => {
				...
				try {
					if (pendingCount > 0) {
						...
						if (
							!repairOnly &&
							manualApply.countOps(transactionBatch) > 0
						) {
					...
					const requestedMode = (
						envValue.IMPECCABLE_LIVE_COPY_AGENT || "auto"
					)
						.trim()
						.toLowerCase();
					const useChatRoute =
					...
					if (useChatRoute) {
						...
						const timeoutMs = Number(
							envValue.IMPECCABLE_LIVE_COPY_AGENT_TIMEOUT_MS ||
								120000,
						);
					...
					if (transaction) {
						const shouldKeepTransaction =
							result?.needsManualDecision === true;
						...
						"manual_edit_commit_done",
						{
							...
							files: Array.isArray(result.files)
								? result.files
										...
										.filter(Boolean)
								: [],
							warnings: summarizeManualDiagnostics(
								result.warnings,
								projectCwd(),
							),
							rolledBackFiles: Array.isArray(
							...
							unreportedFiles: Array.isArray(
								...
								? result.unreportedFiles
										...
										.filter(Boolean)
								: undefined,
							noteCount: Array.isArray(result.notes)
								? result.notes.length
								: 0,
		...
		if (
			...
			req.on("end", () => {
				...
				if (action !== "rollback") {
					sendJson(res, 400, {
						error:
							"unsupported_manual_edit_repair_decision",
						action,
					});
					return;
				}
				...
				recordManualEditActivity(
					"manual_edit_repair_rollback_done",
					response,
				);
		...
		if (
			...
			try {
				...
				if (pageUrl) {
					...
					discarded =
						truncateManualEditsBuffer(projectCwd());
				}
				canceledApplyEvents =
					manualApply.cancelPendingEvents(pageUrl);
			} catch (err) {
				sendJson(res, 500, {
			...
			recordManualEditActivity("manual_edit_discarded", {
				pageUrl,
				discarded,
				canceledApplyIds: canceledApplyEvents.map(
					(event) => event.id,
				),
...
function summarizePendingManualEditBatch(
	cwd,
	pageUrl = null,
) {
	try {
		...
		return {
			pendingSummaryError: err.message || String(err),
		};
	}
}
