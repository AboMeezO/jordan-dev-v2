...
function validateManualEditText(newText) {
	...
	const hits = FORBIDDEN_MANUAL_EDIT_TEXT_CHARS.filter(
		(char) => newText.includes(char),
	);
...
export function validateEvent(msg) {
	...
	switch (msg.type) {
		...
		case "accept":
			...
			return null;
		case "discard":
			return isValidId(msg.id)
				? null
				: "discard: missing or malformed id";
		case "checkpoint":
			if (!isValidId(msg.id))
