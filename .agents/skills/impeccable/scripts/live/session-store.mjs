...
export function createLiveSessionStore({
	...
	function loadCachedOrRebuild(id) {
		...
		const rebuilt = rebuildSnapshotFromJournal(
			journalPath,
			id,
		);
	...
	return {
		...
		appendEvent(event) {
			...
			const legacyJournalPath = getJournalPath(
				legacyRootDir,
				normalized.id,
			);
			if (
...
function getSnapshotPath(rootDir, id) {
	return path.join(
		rootDir,
		safeSessionId(id) + ".snapshot.json",
	);
}
...
function applyEvent(
	...
	switch (event.type) {
		case "generate":
			...
			next.expectedVariants =
				event.count ?? next.expectedVariants;
			next.pendingEventSeq =
				entry.seq ?? next.pendingEventSeq;
		...
		case "agent_done":
			next.phase =
				event.carbonize === true
					...
					: "variants_ready";
			next.sourceFile =
				event.sourceFile ?? event.file ?? next.sourceFile;
			next.previewFile =
				event.previewFile ?? next.previewFile;
			next.previewMode =
				event.previewMode ?? next.previewMode;
		...
		case "checkpoint":
			...
			if (
				(event.revision ?? 0) >=
				(next.checkpointRevision ?? 0)
			) {
				next.phase = event.phase ?? next.phase;
				next.checkpointRevision =
					event.revision ?? next.checkpointRevision;
				next.activeOwner = event.owner ?? next.activeOwner;
				next.arrivedVariants =
					event.arrivedVariants ?? next.arrivedVariants;
				next.visibleVariant =
					event.visibleVariant ?? next.visibleVariant;
				next.sourceFile =
					event.sourceFile ?? next.sourceFile;
				next.previewFile =
					event.previewFile ?? next.previewFile;
				next.previewMode =
					event.previewMode ?? next.previewMode;
		...
		case "accept_intent":
			...
			next.visibleVariant = Number(
				event.variantId ?? next.visibleVariant,
			);
			if (event.paramValues)
				next.paramValues = { ...event.paramValues };
			next.pendingEventSeq =
