...
const HOOK_IGNORE_MARKER_OPEN =
	"# impeccable-hook-ignore-start";
const HOOK_IGNORE_MARKER_CLOSE =
...
function applyDetectorConfigSource(config, raw) {
	...
	if (
		...
		config.designSystem = {
			...config.designSystem,
			enabled:
				raw.designSystem.enabled === false ? false : true,
		};
	}
...
function applyConfigSource(config, raw) {
	...
	if (raw.limits && typeof raw.limits === "object") {
		config.limits = {
			maxFindings: numberOr(
				raw.limits.maxFindings,
				config.limits.maxFindings,
			),
			maxChars: numberOr(
				raw.limits.maxChars,
				config.limits.maxChars,
			),
		};
	}
	return config;
...
function ignoreValueMatches(
	...
	return Boolean(
		entryColor &&
		entryColor === colorIgnoreKey(findingValue),
	);
}
...
function mergeIgnoreValues(existing, incoming) {
	const map = new Map();
	for (const entry of normalizeIgnoreValueEntries(
		existing,
	)) {
		map.set(
...
export function ensureHookGitExcludes(cwd = process.cwd()) {
	try {
		...
		if (updated !== existing) {
			fs.mkdirSync(path.dirname(target.path), {
				recursive: true,
			});
...
export function filterFindings(
	...
	const ignoreRules = new Set(
		(config.ignoreRules || []).map((rule) =>
			normalizeIgnoreRule(rule),
		),
	);
	const ignoreValues = normalizeIgnoreValueEntries(
		config.ignoreValues || [],
	);
...
function isIgnoredFindingValue(finding, ignoreValues) {
	...
	return ignoreValues.some((entry) => {
		...
		if (
			entry.rule !== rule ||
			(!wildcardValue &&
				!ignoreValueMatches(rule, entry.value, value))
		)
			return false;
		if (
			!Array.isArray(entry.files) ||
		...
		return findingMatchesScopedIgnoreFile(
			finding,
			entry.files,
		);
	});
...
export function extractFindingIgnoreValue(finding) {
	...
	return normalizeIgnoreValue(
		extractFindingIgnoreValueRaw(finding, rule),
	);
}
...
function extractFindingIgnoreValueRaw(
	...
	for (const text of candidates) {
		...
		if (google) {
			try {
				return cleanIgnoreValueDisplay(
...
export function dedupeAgainstCache(
	findings,
	cache,
	sessionId,
	filePath,
...
function renderGroupedTemplate(groups, config, opts = {}) {
	...
	for (const group of realGroups) {
		...
		lines.push(
			`${display} (${group.findings.length} issue(s)):`,
		);
		...
		if (hidden > 0) {
			lines.push(
				`- ... ${hidden} more in ${display} (see /impeccable audit).`,
			);
		}
	}

	const footer = directiveFooter("the affected files", {
		grouped: true,
	});
	let text = [header, ...lines, "", footer].join("\n");
	if (text.length > maxChars) {
		text = clampGroupedToBudget(
			header,
			lines,
			footer,
			maxChars,
...
export function resolveTargetFiles(event, projectCwd) {
	...
	if (
		...
		for (const filePath of parseApplyPatchPaths(
			ti.command,
			projectCwd,
		))
			add(filePath);
...
function looksLikeApplyPatch(rawArgs) {
	if (
		...
		!APPLY_PATCH_MARKER.test(rawArgs)
	)
		return false;
...
function applyPatchText(rawArgs) {
	if (typeof rawArgs === "string") {
		...
		return (
			parsed.patch || parsed.input || parsed.command || ""
		);
	}
	if (
		rawArgs &&
...
function normalizeGitHubEvent(event, projectCwd) {
	const cwd =
		event.cwd || envProjectDir(projectCwd) || projectCwd;
	const sessionId =
		event.sessionId || event.session_id || "unknown";
	const toolName =
		event.toolName || event.tool_name || null;
	const toolInput =
...
export function normalizeHookEvent(
	...
	const cwd =
		...
		projectCwd;
	const sessionId =
		event.session_id || event.conversation_id || "unknown";
	...
	const ti =
		event.tool_input && typeof event.tool_input === "object"
			...
			: {};
	const filePath =
		ti.file_path || ti.path || event.file_path;
	if (filePath) {
		return {
			...event,
			cwd,
...
export function parseStaticStyleImports(
	content,
	fromFile,
	projectCwd,
) {
...
export function normalizeScanTargets(
	primaryTargets,
	projectCwd,
) {
	if (
		!Array.isArray(primaryTargets) ||
...
export function expandScanTargets(
	...
	const ordered = normalizeScanTargets(
		primaryTargets,
		projectCwd,
	);
	...
	for (const p of normalizedPrimaries) {
		...
		for (const imp of parseStaticStyleImports(
			content,
			p,
			projectCwd,
		)) {
...
export function writeAuditLog(
	...
	try {
		...
		fs.mkdirSync(path.dirname(expanded), {
			recursive: true,
		});
		const line =
			JSON.stringify({
...
export function renderPendingAck(
	filePath,
	knownFindings,
	opts = {},
) {
...
export function designSystemOptions(
	...
	if (
		!detector ||
		typeof detector.loadDesignSystemForCwd !== "function"
	)
		return {};
	try {
		const designSystem =
			detector.loadDesignSystemForCwd(projectCwd);
...
export async function runHook({
	...
	try {
		...
		const targetFiles = expandScanTargets(
			primaryFiles,
			projectCwd,
		);
		...
		const scanOptions = designSystemOptions(
			config,
			det,
			projectCwd,
		);
		...
		for (const filePath of targetFiles) {
			...
			if (primaryFileSet.has(filePath)) {
				...
				if (editCount > EDIT_COUNT_THRESHOLD) {
					const wasJustCrossed =
						editCount === EDIT_COUNT_THRESHOLD + 1;
			...
			const fresh = dedupeAgainstCache(
				filtered,
				cache,
				sessionId,
				filePath,
		...
		if (freshGroups.length > 0) {
			...
			const allFindings = freshGroups.flatMap(
		...
		if (
			truthy(env.IMPECCABLE_HOOK_QUIET) ||
			config.quiet === true
		) {
			return result({
				emitted: false,
		...
		if (
			pendingWinner &&
			shouldEmitAckForFile(pendingWinner.filePath)
		) {
			const text = appendDesignSystemNote(
				renderPendingAck(
					pendingWinner.filePath,
					pendingWinner.known,
					{ cwd: projectCwd },
				),
				scanOptions,
		...
		if (
			cleanWinner &&
			shouldEmitAckForFile(cleanWinner.filePath)
		) {
			const text = appendDesignSystemNote(
				renderCleanAck(cleanWinner.filePath, {
					cwd: projectCwd,
				}),
				scanOptions,
			);
