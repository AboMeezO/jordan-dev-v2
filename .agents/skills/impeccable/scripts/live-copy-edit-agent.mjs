...
export function buildCopyEditBatchPrompt(
	batch,
	{ cwd = process.cwd() } = {},
) {
...
export async function runCopyEditBatchAgent(
	batch,
	opts = {},
) {
	...
	if (provider === "mock") {
		const delayMs = Number(
			env.IMPECCABLE_LIVE_COPY_AGENT_MOCK_DELAY_MS || 0,
		);
		if (delayMs > 0)
	...
	if (provider === "chat") {
		if (typeof opts.applyBatchToSource !== "function") {
			throw new Error(
				"chat provider requires applyBatchToSource callback",
			);
		}
	...
	const outDir =
		opts.outDir ||
		fs.mkdtempSync(
			path.join(os.tmpdir(), "impeccable-copy-batch-"),
		);
...
export function runCopyEditPostApplyChecks({
	...
	for (const relativeFile of uniqueFiles) {
		...
		const markerMatch =
			findLeftoverImpeccableMarker(content);
		if (markerMatch)
			failures.push({
				file: relativeFile,
		...
		const syntaxCheck = checkFrameworkSourceSyntax(
			relativeFile,
			content,
		);
...
function readManualEditValidationScript(cwd) {
	...
	try {
		...
		const script =
			pkg?.scripts?.["impeccable:manual-edit-validate"];
...
function compactBatchOp(op) {
	return {
		...
		nearbyEditableTexts: Array.isArray(
...
function compactContextForBatch(value) {
	...
	return {
		...
		outerHTML: truncate(
			stripLiveRuntimeHtml(value.outerHTML),
			1800,
		),
	};
}
	...
	const status =
		result.status === "partial"
			...
			: result.status === "error"
				? "error"
				: "done";
	const appliedEntryIds = Array.isArray(
...
export function chooseCopyEditAgent({
	...
	const mode = (env.IMPECCABLE_LIVE_COPY_AGENT || "auto")
		.trim()
		.toLowerCase();
	if (
		mode === "0" ||
...
function runCodex(
	...
	if (env.IMPECCABLE_LIVE_COPY_AGENT_MODEL) {
		args.push(
			"--model",
			env.IMPECCABLE_LIVE_COPY_AGENT_MODEL,
		);
	}
	args.push("-");
...
function runAgentProcess(
	...
	return new Promise((resolve, reject) => {
		...
		const timer = setTimeout(() => {
			...
			rejectOnce(
				new Error(
					`AI copy-edit worker timed out after ${timeoutMs}ms`,
				),
			);
		}, timeoutMs);
		...
		child.on("exit", (code, signal) => {
			if (code === 0) {
				...
				const hint = extractRunnerErrorMessage(
					output,
					command,
				);
				rejectOnce(
