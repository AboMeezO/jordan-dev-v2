...
function mergeFailedEntries(...groups) {
	...
	for (const item of groups.flatMap((group) =>
		...
		out[existingIndex] = {
			...
			...item,
			candidates:
				item.candidates || out[existingIndex].candidates,
...
function repairAttemptLimit(env = process.env) {
	const value = Number(
		env.IMPECCABLE_LIVE_MANUAL_EDIT_REPAIR_ATTEMPTS ||
	...
	return failures
		.map((failure) => {
			...
			if (Array.isArray(failure.candidates)) {
				out.candidates = failure.candidates
					.slice(0, 8)
					.map((candidate) => ({
						file: candidate.file,
						line: candidate.line,
...
function sourceHintWindowFailure(cwd, op) {
	...
	if (
		lines
			.slice(start, end)
			.some((candidateLine) =>
				lineShowsAppliedOp(candidateLine, op),
			)
	)
		return null;
	return null;
...
function verificationTargetsForOp(
	batch,
	op,
	reportedFiles,
	cwd,
	...
	const candidate = (batch.candidates || []).find(
		(item) =>
			item.entryId === op.entryId && item.ref === op.ref,
	);
	const out = [];
	...
	for (const siblingCandidate of siblingCandidatesForEntry(
		batch,
		op,
	)) {
		add(
	...
	for (const relativeFile of reportedFiles || []) {
		for (const target of locatorTargetsInFile(
			cwd,
			relativeFile,
			op,
		)) {
	...
	return out.filter((target) => {
		const key =
			target.file + ":" + target.line + ":" + target.kind;
...
function objectKeyCandidatesForOp(batch, op) {
	...
	return candidates.flatMap(
		(candidate) => candidate.objectKeyMatches || [],
	);
}
...
function objectKeyMatchStillUsesOriginal(cwd, match, op) {
	...
	try {
		lines = fs
			.readFileSync(path.resolve(cwd, relative), "utf-8")
			.split("\n");
	} catch {
		return false;
	}
	...
	if (
		windowLines.some((line) =>
			lineHasObjectKey(line, op.newText),
		)
	)
		return false;
	return windowLines.some((line) =>
		lineHasObjectKey(line, op.originalText),
	);
}
...
function coupledObjectKeyFailuresForOp(batch, op, cwd) {
	...
	return objectKeyCandidatesForOp(batch, op)
		.filter((match) =>
			objectKeyMatchStillUsesOriginal(cwd, match, op),
		)
		.map((match) => ({
			ref: op.ref,
			...
			candidates: [
				{
					file:
						normalizeRelativeFile(cwd, match.file) ||
						match.file,
					line: match.line,
...
function verificationTargetPassesLines(lines, target, op) {
	...
	const radius = kind.includes("context_text_match")
		? 20
		: 4;
	...
	const windowLines = lines.slice(start, end);
	if (
		windowLines.some((candidateLine) =>
			lineShowsAppliedOp(candidateLine, op),
		)
	)
		return true;
...
function windowShowsAppliedOp(lines, op) {
	const newText =
		typeof op?.newText === "string" ? op.newText : "";
	...
	const normalizedOriginal =
		normalizeVerificationText(originalText);
	const normalizedWindow = normalizeVerificationText(
		lines.join("\n"),
	);
	if (
		!normalizedNew ||
		!normalizedWindow.includes(normalizedNew)
	)
		return false;
	if (
		normalizedOriginal &&
...
function lineShowsAppliedOp(line, op) {
	...
	const newText =
		typeof op?.newText === "string" ? op.newText : "";
	const deletion =
		op?.deleted === true || newText.length === 0;
	if (deletion)
...
function opHasLocator(op) {
	return !!(
		op?.tag ||
		op?.elementId ||
		(Array.isArray(op?.classes) &&
			op.classes.filter(Boolean).length > 0)
	);
}
...
function verifyAppliedEntry({
	...
	for (const rawOp of entry.ops || []) {
		...
		const targets = verificationTargetsForOp(
			batch,
			op,
			reportedFiles,
			cwd,
		);
		const coupledObjectKeyFailures =
			coupledObjectKeyFailuresForOp(batch, op, cwd);
		if (
...
function snapshotTargetPasses(snapshot, target, op) {
	...
	return verificationTargetPassesLines(
		before.split("\n"),
		target,
		op,
	);
...
function verificationFailuresForEntries(
	batch,
	entries,
	reason,
	extra = {},
...
function clearAppliedEntries(cwd, appliedEntryIds) {
	...
	for (const entry of buffer.entries || []) {
		if (ids.has(entry.id)) {
			cleared += Array.isArray(entry.ops)
				? entry.ops.length
				: 0;
		} else {
			kept.push(entry);
...
function changedFilesSinceSnapshot(
	cwd,
	snapshot,
	scopeFiles = null,
) {
	...
	const currentFiles = new Set(
		scopedFiles || collectRollbackFiles(cwd),
	);
...
function rollbackChangedFiles(
	...
	const byFile = new Map(
		changed.map((item) => [item.file, item]),
	);
...
function collectApplyOwnedFiles(
	batch,
	cwd,
	extraFiles = [],
) {
...
function unreportedChangedFiles(
	cwd,
	snapshot,
	reportedFiles,
	scopeFiles = [],
...
function verifyEntriesAfterRepair({
	batch,
	appliedEntryIds,
	files,
	cwd,
	...
	const failed = [];
	for (const entry of entries) {
		const failures = verifyAppliedEntry({
			batch,
			entry,
			reportedFiles,
			cwd,
...
async function repairPostApplyValidation({
	...
	for (
		...
		currentFiles = mergeUniqueStrings(
			currentFiles,
			repairResult.files || [],
		);
		currentNotes = [
		...
		const cleared = clearAppliedEntries(
			cwd,
		...
		return {
			applied: summarizeAppliedEntries(
				batch.entries,
				verified.verifiedIds,
			),
	...
	const decisionFailedEntries =
		currentAppliedIds.length > 0
			? (batch.entries || [])
					.filter((entry) =>
						currentAppliedIds.includes(entry.id),
					)
					.map((entry) => ({
						id: entry.id,
						reason: repairReason,
...
export async function commitManualEdits({
	...
	const batch =
		providedBatch ||
		buildManualEditEvidence({ cwd, pageUrl });
	...
	const baseRollbackScope = collectApplyOwnedFiles(
		batch,
		cwd,
	);
	const rollbackSnapshot = snapshotRollbackFiles(
		cwd,
		baseRollbackScope,
	);
	let result;
	...
	const reportedAppliedIds = uniqueStrings(
		result.appliedEntryIds || [],
	);
	const reportedFiles = uniqueStrings(result.files || [])
		.map((file) => normalizeRelativeFile(cwd, file))
		.filter(Boolean);
	const aiFailed = normalizeFailedEntries(
		batch,
		result,
		"AI copy edit failed",
	);
	...
	const failedIds = new Set(
		aiFailed.map((item) => item.id).filter(Boolean),
	);
	const conflictingAppliedIds = reportedAppliedIds.filter(
		(id) => failedIds.has(id),
	);
	...
	if (conflictingAppliedIds.length > 0) {
		...
		const conflictingEntries = batch.entries.filter(
			(entry) => conflictingAppliedIds.includes(entry.id),
		);
		return {
			applied: [],
			failed: [
				...
				...aiFailed.filter(
					(item) =>
						!conflictingAppliedIds.includes(item.id),
				),
			],
			files: result.files || [],
			cleared: 0,
	...
	const reportedAppliedEntries = batch.entries.filter(
	...
	if (leakedUnapplied.length > 0) {
		...
		const rolledBackVerified = reportedAppliedEntries
			...
			.map((entry) => ({
				id: entry.id,
				reason:
					"rolled_back_due_to_failed_entry_source_changed",
				candidates: candidatesForEntry(batch, entry.id),
			}));
		const rollback = rollbackChangedFiles(
			cwd,
...
async function main() {
	...
	const result = await commitManualEdits({
		...
		timeoutMs: Number(
			process.env.IMPECCABLE_LIVE_COPY_AGENT_TIMEOUT_MS ||
				120000,
		),
	});
...
if (
	process.argv[1]?.endsWith("live-commit-manual-edits.mjs")
) {
	main().catch((err) => {
		console.error(
			JSON.stringify({
