...
(function () {
	...
	const PICK_CURSOR_STYLE_ID =
		PREFIX + "-pick-cursor-style";
	...
	const LIVE_CHROME_MOUNT_CONTRACT = [
		"root",
		"transport",
		"state",
		"actions",
	...
	const SCROLL_ANCHOR_LOCK_ID =
		"impeccable-scroll-anchor-lock";
	...
	function initAnnotOverlay() {
		...
		annotSvgEl = document.createElementNS(
		...
		annotOverlayEl.addEventListener(
			"pointerdown",
			onAnnotDown,
		);
		annotOverlayEl.addEventListener(
		...
		defangOutsideHandlers(annotOverlayEl, {
	...
	function updateClearChip() {
		...
		const hasAny =
			annotState.comments.length > 0 ||
			annotState.strokes.length > 0;
		annotClearChipEl.style.display = hasAny
			? "block"
			: "none";
	}
	...
	function onAnnotDown(e) {
		...
		const strokeHit = e.target.closest?.(
			"[data-annot-stroke]",
		);
		if (strokeHit) {
			const idx = parseInt(
				strokeHit.dataset.annotStroke,
				10,
			);
	...
	function onAnnotMove(e) {
		...
		if (!annotPointer.moved) {
			...
			annotPointer.strokePoints = [
				[annotPointer.x0, annotPointer.y0],
			];
		}
	...
	function onAnnotUp(e) {
		...
		const wasDrag = annotPointer.moved;
		if (wasDrag) {
			annotState.strokes.push({
				points: annotPointer.strokePoints,
			});
	...
	function contextElementForManualEdit(
		selectedEl,
		rows,
		ops,
	) {
	...
	function isUsefulManualEditContext(
		candidate,
		leafEl,
		editedTexts,
	) {
		...
		if (
			...
			return false;
		return (
			collectManualContextPieces(candidate, editedTexts)
				.length > 0
		);
	}
	...
	function collectManualContextPieces(rootEl, editedTexts) {
		...
		function walk(node) {
			if (!node) return;
			if (node.nodeType === 3) {
				const text = normalizeManualContextText(
					node.nodeValue,
				);
				if (
					isMeaningfulManualContextPiece(text, editedTexts)
				)
					pieces.push(text);
				return;
			}
	...
	function isMeaningfulManualContextPiece(
		text,
		editedTexts,
	) {
	...
	function positionBar() {
		...
		if (
			belowTop + barH + GAP <=
			window.innerHeight - GLOBAL_BAR_RESERVE
		) {
			top = belowTop;
	...
	function showBar(mode) {
		...
		if (
			mode === "cycling" &&
			!ensureCyclingRenderable("show-bar")
		)
			return;
		barEl.innerHTML = "";
		if (mode === "configure") {
	...
	function updateBarContent(mode) {
		...
		if (
			mode === "cycling" &&
			!ensureCyclingRenderable("update-bar")
		)
			return;
		barEl.innerHTML = "";
		if (mode === "configure") {
			...
			if (mode === "generating")
				...
				barEl.style.border =
					"1px solid oklch(75% 0.12 145 / 0.4)";
			}
		}
	...
	const CONFIGURE_SELECTION_PILL_BORDER =
		"1px solid oklch(70% 0.12 188)";
	...
	function syncConfigureInputChrome() {
		const input =
			uiGetById(PREFIX + "-input") ||
			uiGetById(PREFIX + "-insert-input");
		const surface =
			barEl?.dataset.configureSurface === "true"
				? barEl
				: null;
		...
		surface.dataset.inputFocused = focused
			? "true"
			: "false";
		surface.dataset.voiceListening = listening
			? "true"
			: "false";
	...
	function bindConfigureInlineControlHover(
		btn,
		controlsLocked,
	) {
	...
	function bindConfigureModifierPillHover(
		btn,
		controlsLocked,
	) {
		btn.addEventListener("mouseenter", () => {
	...
	function removeConfigureSelection() {
		...
		exitConfigureToPicking("selection-pill-remove", {
			clearHover: true,
		});
	}
	...
	function buildSelectionPill({
		...
		pill.style.cursor = controlsLocked
			? "not-allowed"
			: "pointer";
	...
	function bindConfigureCountPillTooltip(
		count,
		controlsLocked,
	) {
	...
	function buildConfigureActionControl({
		...
		control.style.cursor = controlsLocked
			? "not-allowed"
			: "pointer";
		control.style.opacity = controlsLocked ? "0.58" : "1";
		bindConfigureInlineControlHover(
			control,
			controlsLocked,
		);
	...
	function buildConfigureCountControl({
		controlsLocked,
		onClick,
	}) {
		const count = el(
		...
		count.style.cursor = controlsLocked
			? "not-allowed"
			: "pointer";
	...
	function buildConfigureVoiceButton({
		id,
		controlsLocked,
		onClick,
	}) {
		...
		voiceBtn.style.cursor = controlsLocked
			? "not-allowed"
			: "pointer";
	...
	function buildConfigureTrailingCluster(
		controls,
		voiceBtn,
		submitBtn,
	) {
	...
	function buildConfigureSubmitButton({
		controlsLocked,
		onClick,
		ariaLabel,
	}) {
	...
	function groupSiblingRows(siblings, rowThreshold) {
		...
		for (const entry of sorted) {
			...
			for (const row of rows) {
				if (
					Math.abs(entry.rect.top - row[0].rect.top) <=
					rowThreshold
				) {
					row.push(entry);
					placed = true;
	...
	function hitSiblingInsertGap(
		...
		for (const row of groupSiblingRows(siblings)) {
			...
			for (let i = 0; i < sorted.length - 1; i++) {
				...
				const inX =
					...
					clientX <= bLeft + slop;
				const inY =
					clientY >= top - slop && clientY <= bottom + slop;
				if (!inX || !inY) continue;
				return {
					anchor: b.el,
					position: "before",
		...
		for (let i = 0; i < sortedCol.length - 1; i++) {
			...
			const inY =
				clientY >= gapTop - slop &&
				clientY <= gapBottom + slop;
			const inX =
				clientX >= overlapLeft - slop &&
				clientX <= overlapRight + slop;
			if (!inY || !inX) continue;
			return {
				anchor: b.el,
				position: "before",
	...
	function placeholderSizing({
		...
		const w = Number.isFinite(parentWidth)
			? parentWidth
			: 0;
		if (axis === "row") {
			if (display.includes("flex")) {
	...
	function applyPlaceholderSizingStyles(
		placeholder,
		sizing,
	) {
		placeholder.dataset.impeccablePlaceholderWidth =
			sizing.kind;
	...
	function insertCreateDisabledReason({
		prompt,
		comments,
		strokes,
	}) {
	...
	function clampPlaceholderSize(
		...
		return {
			width: Math.min(
				maxW,
				Math.max(PLACEHOLDER_MIN_WIDTH, Math.round(width)),
			),
			height: Math.max(
	...
	function resizePlaceholderFromEdge(
		start,
		edge,
		dx,
		dy,
		...
		if (edge === "w")
			base.marginLeft =
				start.marginLeft + start.width - clamped.width;
		else if (edge === "n")
			base.marginTop =
				start.marginTop + start.height - clamped.height;
		return {
			width: clamped.width,
	...
	function syncPageInteractionCursor() {
		...
		if (
			...
			cursor = cursorForInsertAxis(
				insertHoverAxis || "column",
			);
		}
	...
	function resolveBarAnchor() {
		if (
			svelteComponentSession?.sessionId ===
				currentSessionId &&
		...
		if (
			...
			if (wrapper) {
				...
				if (variantCount > 0 && visibleVariant > 0) {
					const visEl = pickVariantContent(
						wrapper,
						visibleVariant,
					);
		...
		if (
			selectedElement &&
			document.body.contains(selectedElement)
		)
			return selectedElement;
		if (
			placeholderElement &&
			document.body.contains(placeholderElement)
		)
	...
	function buildInsertPlaceholderSnapshotFromDom(
		anchor,
		placeholder,
	) {
		return {
			...
			marginLeft:
				parseFloat(placeholder.style.marginLeft) || 0,
			marginTop:
				parseFloat(placeholder.style.marginTop) || 0,
	...
	function isInsertGeneratingSession() {
		...
		return (
			!!wrapper &&
			wrapper.dataset.impeccableMode === "insert"
		);
	}
	...
	function ensureInsertPlaceholder() {
		...
		const position =
			insertPlaceholderSnapshot?.position ||
			insertAnchorPosition ||
			"before";
		const axis =
			insertPlaceholderSnapshot?.layoutAxis ||
			insertAnchorLayoutAxis;
	...
	function applyPlaceholderDimensions({
		...
		ph.style.marginLeft = marginLeft
			? marginLeft + "px"
			: "";
	...
	function buildPlaceholderResizeHandles() {
		...
		for (const spec of specs) {
			...
			handle.dataset.impeccablePlaceholderResize =
				spec.edge;
			handle.setAttribute(
				"aria-label",
				"Resize placeholder",
	...
	function syncPlaceholderResizeHandles() {
		...
		const show =
			...
			!!placeholderElement &&
			state === "CONFIGURING";
		placeholderResizeLayerEl.style.display = show
			? "block"
			: "none";
		if (!show) {
			placeholderResizeLayerEl.innerHTML = "";
	...
	function startPlaceholderEdgeResize(edge, e) {
		...
		placeholderResizeDrag = {
			...
			parentWidth:
				ph.parentNode?.getBoundingClientRect().width ||
				PLACEHOLDER_MIN_WIDTH,
	...
	function syncInsertCreateButton(btn, input) {
		...
		const reason = ok
			? "Create variants"
			: insertCreateDisabledReason(gate);
		btn.setAttribute(
			"aria-disabled",
			ok ? "false" : "true",
		);
	...
	function buildGeneratingRow() {
		...
		label.textContent =
			configureKind === "insert" ? "Insert" : actionLabel();
	...
	function buildCyclingRow() {
		...
		counter.textContent =
			visibleVariant + "/" + arrivedVariants;
		...
		const visParams = parseVariantParams(
			getVisibleVariantEl(),
		);
		...
		for (let i = 1; i <= expectedVariants; i++) {
			...
			const dotBg = active
				...
				: arrived
					? BP.textDim
					: "transparent";
			const dotBorder = arrived
				? "none"
				: "1.5px solid " + BP.hairline;
			const dot = el("div", {
				...
				cursor:
					clickable && arrived ? "pointer" : "default",
	...
	function toggleActionPicker() {
		...
		const P =
			pickerEl.__iceq_palette ||
			barPaletteForTheme(detectPageTheme());
		pickerEl.querySelectorAll("button").forEach((chip) => {
			const isActive =
				chip.dataset.action === selectedAction;
		...
		left = Math.max(
			8,
			Math.min(left, window.innerWidth - pickerW - 8),
		);
		Object.assign(pickerEl.style, {
			top: top + "px",
			left: left + "px",
	...
	function recoverEmptyCycling(reason) {
		...
		try {
			...
			const message =
				"No variants were mounted. Please try again.";
			if (
				svelteComponentSession?.sessionId ===
				currentSessionId
			) {
	...
	function initParamsPanel() {
		paramsPanelPalette = barPaletteForTheme(
			detectPageTheme(),
		);
		...
		defangOutsideHandlers(paramsPanelEl, {
			setPointerEvents: false,
		});
		paramsPanelInner = paramsPanelEl; // compatibility alias for the rest of the code
	}

	function getMountedSvelteComponentAnchor(
		session = svelteComponentSession,
	) {
		const el =
			session?.mountTargetEl?.firstElementChild || null;
		if (!el || !document.body.contains(el)) return null;
		return rectIsUsableAnchor(el.getBoundingClientRect())
			? el
			: null;
	}

	function resolveSvelteComponentAnchor(
		session = svelteComponentSession,
	) {
		return (
			getMountedSvelteComponentAnchor(session) ||
			session?.swapAnchor ||
			null
		);
	}
	...
	function parseVariantParams(variantEl) {
		...
		if (
			...
			const byVariant =
				svelteComponentSession.paramsByVariant || {};
			const params =
				byVariant[String(visibleVariant)] ||
				byVariant[visibleVariant];
			return Array.isArray(params) ? params : [];
		}
		if (!variantEl) return [];
		const raw = variantEl.getAttribute(
	...
	function buildParamsPanel(variantEl, params) {
		...
		for (const p of params) {
			...
			if (p.kind === "range") {
				...
				const activeOpt =
					opts.find((o) => o.value === p.default) ||
					opts[0];
				readout.textContent = activeOpt
					? activeOpt.label
					: String(p.default);
				const segRow = el("div", {
					display: "grid",
	...
	function wrapMixedContentTextNodes(rootEl) {
		...
		for (const child of Array.from(rootEl.children)) {
			if (
				!child.dataset ||
				!child.dataset.impeccableTextWrap
			) {
	...
	function unwrapMixedContentTextNodes(rootEl) {
		...
		const wraps = rootEl.querySelectorAll(
			'[data-impeccable-text-wrap="true"]',
		);
	...
	function enableInlineEdit(targetEl) {
		...
		const rows = collectEditableTextRows(targetEl, {
			isOwn: own,
		});
		inlineEditRows = rows;
		...
		for (const row of rows) {
			...
			row.el.style.whiteSpace = getComputedStyle(
				row.el,
			).whiteSpace;
	...
	function enterEditingMode() {
		...
		if (inlineEditRows.length > 0) {
			const firstEditable =
				inlineEditRows[0] && inlineEditRows[0].el;
			setTimeout(() => {
	...
	function restoreInlineEditDrafts() {
		for (const row of inlineEditRows) {
			if (inlineEditDrafts.has(row.el)) {
				row.el.textContent =
					row.el.dataset.impeccableOriginalText;
			}
		}
	}
	...
	function buildLocatorForLeaf(leafEl, fallbackEl) {
		if (
			leafEl &&
			(leafEl.id || leafEl.classList.length > 0)
		) {
			return {
	...
	function documentRefIdSuffix(el) {
		return el.id
			? "#" + normalizeDocumentRefToken(el.id)
			: "";
	}
	...
	function copyEditLeafContext(el, originalText, newText) {
		...
		return {
			...
			textContent: (el.textContent || "").slice(0, 500),
			outerHTML:
				sanitizedContextOuterHTML(el, 3000) || null,
		};
	}
	...
	function copyEditContainerContext(el) {
		...
		return {
			...
			outerHTML:
				sanitizedContextOuterHTML(el, 10000) || null,
		};
	}
	...
	async function applyEditing() {
		...
		for (const row of inlineEditRows) {
			...
			if (newText !== undefined && newText !== row.text) {
				if (String(newText || "").trim() === "") {
					showToast(
						"Save rejected: copy edits cannot be empty.",
						5500,
					);
					return;
				}
				...
				const locator = buildLocatorForLeaf(
					row.el,
					selectedElement,
				);
				const op = {
				...
				const restoreHint = mixedTextWrapRestoreHint(
					row.el,
				);
		...
		const contextRef =
			documentRefForElement(contextElement);
		if (contextRef)
			for (const op of ops) op.contextRef = contextRef;
		const container =
			copyEditContainerContext(contextElement);
		if (container)
		...
		try {
			...
			console.error(
				"[impeccable] manual edit stash failed:",
				err,
			);
	...
	function positionPendingDock() {
		...
		pendingDockEl.style.left =
			Math.round(window.innerWidth / 2 - width / 2 - 18) +
			"px";
		pendingDockEl.style.top = "auto";
		pendingDockEl.style.bottom =
			Math.round(14 + height / 2) + "px";
	}
	...
	function showManualApplyBusyToast() {
		showToast(
			"Apply is still running. Wait for it to finish.",
			2800,
		);
	}
	...
	function shouldResumeManualApplyLoading(count) {
		return (
			Number(count) > 0 &&
			readStoredManualApplyState() !== null
		);
	}
	...
	function manualApplyLoadingText(fallbackCount) {
		...
		if (stored?.phase === "repairing") {
			...
			return (
				"Fixing apply issue, attempt " + attempt + "/" + max
			);
		}
	...
	function updateManualApplyProgressFromChunk(chunk) {
		...
		const completedOps = Math.min(
			totalOps,
			(Number(stored.completedOps) || 0) +
				(Number(chunk.opCount) || 0),
		);
		const remainingCount = Math.max(
			0,
			totalOps - completedOps,
		);
		storeManualApplyState(
			Number(stored.count) || totalOps,
			{
				totalOps,
				completedOps,
				remainingCount,
				phase:
					remainingCount > 0 ? "applying" : "verifying",
			},
		);
	...
	function updateManualApplyRepairState(repair, phase) {
		...
		storeManualApplyState(count, {
			phase,
			repairAttempt:
				Number(repair?.attempt || repair?.attempts) || 1,
	...
	function setPendingApplyLoading(loading, count) {
		...
		if (pendingPillSpinnerEl)
			pendingPillSpinnerEl.style.display =
				pendingApplyInFlight ? "inline-block" : "none";
		...
		pendingPillEl.style.cursor = pendingApplyInFlight
			? "wait"
			: "pointer";
		...
		pendingTrashBtn.style.cursor = pendingApplyInFlight
			? "not-allowed"
			: "pointer";
		pendingTrashBtn.style.opacity = pendingApplyInFlight
			? "0.58"
			: "1";
		if (pendingApplyInFlight) {
			if (pendingKeepFixingBtn)
	...
	function updatePendingCounter(currentPageCount) {
		...
		pendingPillCountEl.textContent = String(
			currentPageCount,
		);
		pendingPillEl.setAttribute(
			"aria-label",
	...
	function maybeShowFirstSaveToast() {
		...
		showToast(
			'Saved. Click "Apply copy edits" to write changes.',
			4500,
		);
	}
	...
	async function onPendingTrashClick() {
		...
		try {
			...
			const restoreFailures = restoreDiscardedManualEdits(
				result.entries || [],
			);
	...
	function showManualApplyDecision(msg) {
		...
		storeManualApplyState(count, {
			...
			repairAttempt:
				...
				3,
			repairMaxAttempts:
				numberOrNull(msg?.repair?.maxAttempts) || 3,
		});
		if (pendingPillSpinnerEl)
			pendingPillSpinnerEl.style.display = "none";
		if (pendingPillLabelEl)
			pendingPillLabelEl.textContent =
	...
	async function onPendingKeepFixingClick() {
		...
		try {
			...
			console.error(
				"[impeccable] repair retry failed:",
				err,
			);
	...
	async function onPendingRollbackClick() {
		...
		try {
			...
			updatePendingCounter(
				numberOrNull(result.remainingCount) || 0,
			);
			showToast(
				"Rolled back source; copy edits are still staged.",
				3500,
			);
		} catch (err) {
			console.error(
				"[impeccable] manual Apply rollback failed:",
				err,
			);
	...
	function manualEditEventForCurrentPage(msg) {
		return (
			!msg?.pageUrl || msg.pageUrl === location.pathname
		);
	}
	...
	function remainingManualEditCount(payload) {
		const perPageCount = numberOrNull(
			payload?.perPage?.[location.pathname],
		);
		if (perPageCount !== null) return perPageCount;
		const remainingCount = numberOrNull(
	...
	function handleManualEditActivity(msg) {
		...
		if (
			msg.type === "manual_edit_apply_dispatched" &&
			msg.repair
		) {
		...
		if (msg.type === "manual_edit_commit_done") {
			...
			if (wasApplying) {
				const failedCount =
					numberOrNull(msg.failedCount) || 0;
				const appliedCount =
					numberOrNull(msg.appliedCount) ||
					numberOrNull(msg.cleared) ||
					0;
				if (failedCount > 0) {
					showToast(
						"Applied " +
	...
	function mixedTextWrapRestoreHint(el) {
		if (
			...
			return null;
		const siblings = directMixedTextRestoreNodes(
			el.parentElement,
		);
	...
	function restoreMixedTextNodeManualEdit(op) {
		...
		const textNodes = directMixedTextRestoreNodes(
			parent,
		...
		const matches = textNodes.filter(
			(node) =>
				normalizeManualContextText(node.nodeValue) ===
				newText,
		);
	...
	function queryManualEditRef(ref) {
		...
		for (let index = 0; index < parts.length; index += 1) {
			const segment = parseManualEditRefSegment(
				parts[index],
			);
			...
			if (index === 0 && segment.tag === "body") {
				...
				if (
					!elementMatchesManualRefSegment(current, segment)
				)
					return null;
				continue;
			}
	...
	function parseManualEditRefSegment(segment) {
		const nthMatch = String(segment || "").match(
			/:nth-of-type\((\d+)\)$/,
		);
		const nth = nthMatch ? Number(nthMatch[1]) : null;
		const base = nthMatch
			? segment.slice(0, nthMatch.index)
			: segment;
	...
	function elementMatchesManualRefSegment(el, segment) {
		...
		if (
			segment.nth &&
			indexAmongSameTag(el) !== segment.nth
		)
			return false;
		return true;
	}
	...
	function initEditBadgeHitProxies() {
		...
		editBadgeProxyRoot.id =
			PREFIX + "-edit-badge-hit-proxies";
	...
	function editBadgeProxyTargets() {
		if (
			...
			return [];
		return [
			...editBadgeEl.querySelectorAll("button"),
		].filter((target) => {
	...
	function renderEditBadge(mode) {
		...
		if (mode === "idle" || mode === "idle-disabled") {
			...
			Object.assign(
				save.style,
				calloutStyle(PRIMARY_TEXT, ACCENT),
			);
	...
	function positionParamsPanel() {
		...
		const prevDirection =
			paramsPanelEl.dataset.tuneDirection;
		...
		if (
			!tuneOpen &&
			(!prevDirection || prevDirection !== direction)
		) {
	...
	function hideParamsPanel() {
		...
		const direction =
			paramsPanelEl.dataset.tuneDirection || "below";
	...
	async function showVariantInDOM(sessionId, num) {
		if (svelteComponentSession?.sessionId === sessionId) {
			visibleVariant = num;
			const mounted =
				await mountSvelteComponentVariant(num);
		...
		for (const child of wrapper.children) {
			const v = child.dataset
				? child.dataset.impeccableVariant
				: null;
			if (!v) continue;
	...
	function parseOriginalMarkupElement(originalMarkup) {
		...
		const doc = parser.parseFromString(
			'<div id="impeccable-anchor">' +
				originalMarkup +
				"</div>",
			"text/html",
		);
		return (
			doc.getElementById("impeccable-anchor")
	...
	function elementMatchesOriginalMarkup(
		liveEl,
		origContent,
	) {
		...
		const origClasses = normalizeElementClassName(
			origContent,
		)
			.split(/\s+/)
			.filter(Boolean)
		...
		if (
			...
			!origClasses.every((name) =>
				liveEl.classList.contains(name),
			)
		)
			return false;
		...
		if (origClasses.length === 0 && origText.length >= 4) {
			...
			const needle = origText.slice(
				0,
				Math.min(40, origText.length),
			);
			if (
				!liveText.includes(needle) &&
				!(
					liveText.length >= 4 &&
					origText.includes(liveText.slice(0, 40))
				)
			)
				return false;
		}
		...
		for (const c of candidates) {
			...
			if (
				...
				if (
					...
					!(
						text.length >= 4 &&
						needle.includes(text.slice(0, 40))
					)
				)
					continue;
			}
	...
	function findLiveElementForOriginalMarkup(
		originalMarkup,
	) {
		const origContent =
			parseOriginalMarkupElement(originalMarkup);
		...
		if (origText.length >= 4) {
			...
			for (const c of candidates) {
				...
				if (
					...
					!(
						text.length >= 4 &&
						origText.includes(text.slice(0, 40))
					)
				)
					continue;
	...
	function resolveLiveInjectionAnchor(originalMarkup) {
		...
		const attempts = [
			selectedElement,
			findLiveElementFromAnchorSnapshot(
		...
		for (const candidate of attempts) {
			if (
				elementMatchesOriginalMarkup(candidate, origContent)
			)
				return candidate;
		}
		...
		if (
			...
			if (
				origContent.id &&
				selectedElement.id === origContent.id
			)
				return selectedElement;
	...
	function waitForVariantAnchorAndRetry({
		...
		pendingVariantAnchorRetryObserver =
			new MutationObserver(() => {
				...
				if (!wrapperLanded) {
					const liveEl =
						resolveLiveInjectionAnchor(originalMarkup);
		...
		pendingVariantAnchorRetryObserver.observe(
			document.body,
	...
	function loadSvelteRuntime(runtimeModule) {
		const modulePath =
			runtimeModule || "/src/lib/impeccable/__runtime.js";
	...
	async function loadSvelteComponentVariantSource(
		manifest,
		variantNum,
	) {
		const dir = String(
	...
	async function applySvelteComponentVariantStyle(
		variantNum,
	) {
		if (!svelteComponentSession || !variantNum) return;
		const { manifest, sessionId } = svelteComponentSession;
		const source = await loadSvelteComponentVariantSource(
			manifest,
			variantNum,
		);
		...
		if (!css) return;
		const scopedCss = scopeCssToSveltePreview(
			css,
			sessionId,
		);
		if (!scopedCss) return;
		const style = document.createElement("style");
		style.dataset.impeccableSvelteComponentStyle =
			sessionId;
	...
	function removeSvelteComponentVariantStyle(
		session = svelteComponentSession,
	) {
	...
	function scopeCssBlock(css, prefix) {
		...
		while (i < css.length) {
			...
			if (shouldScopeNestedCssAtRule(prelude)) {
				...
				out +=
					prefixCssSelectors(prelude, prefix) +
					" {" +
					body +
					"}";
			}
	...
	function shouldScopeNestedCssAtRule(prelude) {
		return /^@(media|supports|container|layer)\b/i.test(
			prelude || "",
		);
	}
	...
	function prefixCssSelectors(prelude, prefix) {
		return splitCssSelectorList(prelude)
			.map((selector) => {
				const s = unwrapSvelteGlobalSelector(
					selector.trim(),
				);
				if (!s) return "";
	...
	function buildSveltePropValuesFromLiveElement(
		liveEl,
		manifest,
	) {
		...
		const map = buildSvelteExpressionTextMap(
			sourceOriginal,
			liveEl,
		);
	...
	async function mountSvelteComponentVariant(variantNum) {
		if (!svelteComponentSession || !variantNum)
			return false;
		const { manifest, mountTargetEl, sessionId } =
			svelteComponentSession;
		try {
			const previousAnchor =
				getMountedSvelteComponentAnchor(
					svelteComponentSession,
				) || selectedElement;
			svelteComponentSession.swapAnchor =
				...
				svelteComponentSession.swapAnchor ||
				null;
			const runtime = await loadSvelteRuntime(
				manifest.runtimeModule,
			);
			const modulePath =
				"/" +
				...
				".svelte";
			const moduleUrl =
				new URL(modulePath, location.origin).href +
				"?t=" +
				Date.now();
			...
			if (
				svelteComponentSession.mountedInstance &&
				runtime.unmount
			) {
	...
	function teardownSvelteComponentSession(restoreOriginal) {
		...
		if (
			...
			wrapperEl.parentElement.replaceChild(
				detachedOriginal,
				wrapperEl,
			);
	...
	function applyOriginalAttrsToSvelteAnchor(
		el,
		originalMarkup,
	) {
		...
		for (const attr of original.attributes) {
			if (attr.name === "class") {
				for (const className of attr.value
					.split(/\s+/)
					.filter(Boolean)) {
	...
	function commitAcceptedSvelteComponentToDom(sessionId) {
		...
		const anchor = getMountedSvelteComponentAnchor(
			svelteComponentSession,
		);
	...
	async function injectSvelteComponentsFromManifest(
		manifestPath,
		sessionId,
	) {
		const url =
		...
		try {
			...
			const paramsByVariant =
				await loadSvelteComponentParams(manifest);
			currentSessionId = sessionId;
			expectedVariants =
				Number(manifest.count) || expectedVariants || 1;
			...
			if (
				...
				visibleVariant =
					...
					visibleVariant <= arrivedVariants
						? visibleVariant
						: 1;
				await mountSvelteComponentVariant(
					visibleVariant || 1,
				);
			...
			const liveEl =
				findLiveElementForSvelteManifest(manifest);
			if (!liveEl?.parentElement) {
				...
				visibleVariant =
					...
					visibleVariant <= arrivedVariants
						? visibleVariant
						: savedVisibleVariant > 0 &&
							  savedVisibleVariant <= arrivedVariants
							? savedVisibleVariant
							: 1;
				enterRecoveryWaitingForAnchor({
					checkpointReason:
						"svelte_component_anchor_missing",
					trackScroll: true,
				});
			...
			mountTarget.dataset.impeccableComponentMount =
				sessionId;
			...
			const mounted =
				await mountSvelteComponentVariant(visibleVariant);
			if (!mounted) {
			...
			selectedElement =
				mountTarget.firstElementChild || mountTarget;
			setLiveState("CYCLING");
			recoveryWaitingForAnchor = false;
			hideShaderOverlay();
			showOrUpdateCyclingBar();
	...
	function waitForSvelteComponentTargetAndRetry({
		...
		pendingSvelteComponentRetryObserver.observe(
			document.body,
			{ childList: true, subtree: true },
		);
	}
	...
	function abortSvelteComponentInjection(
		sessionId,
		message,
	) {
		try {
	...
	function injectVariantsFromSource(filePath, sessionId) {
		...
		fetch(url)
			...
			.then((html) => {
				...
				if (existingWrapper) {
					existingWrapper.parentElement.replaceChild(
						wrapper,
						existingWrapper,
					);
				} else {
					...
					const liveEl = resolveLiveInjectionAnchor(
					...
					liveEl.parentElement.replaceChild(
						wrapper,
						liveEl,
					);
				}
				...
				expectedVariants = parseInt(
					wrapper.dataset.impeccableVariantCount ||
						arrivedVariants,
				);
				if (arrivedVariants <= 0) {
				...
				selectedElement =
					pickVariantContent(wrapper, visibleVariant) ||
			...
			.catch((err) => {
				console.error(
					"[impeccable] Failed to fetch source:",
					err,
				);
				showToast(
					"Could not load variants. Try refreshing the page.",
					5000,
				);
			});
	}
	...
	function buildSvelteExpressionTextMap(
		sourceOriginal,
		liveOriginal,
	) {
		const map = new Map();
		...
		const liveTexts = collectTextNodes(liveOriginal)
			.map((node) =>
				normalizePreviewText(node.nodeValue || ""),
			)
			.filter(Boolean);
		let liveIndex = 0;
	...
	function expressionTextMatcher(sourceText, tokens) {
		...
		pattern +=
			escapeRegExp(sourceText.slice(cursor)).replace(
				/\s+/g,
				"\\s*",
			) + "$";
	...
	async function selectVariant(next, checkpointReason) {
		...
		const selectionPromise = (async () => {
			...
			showOrUpdateCyclingBar();
			saveSession();
			const shown = await showVariantInDOM(
				currentSessionId,
				next,
			); // calls refreshParamsPanel itself
			if (!shown) {
	...
	function readVisibleVariantFromDOM(sessionId) {
		...
		for (const variant of variants) {
			...
			const idx = parseInt(
				variant.dataset.impeccableVariant || "0",
				10,
			);
			if (idx > 0) return idx;
		}
	...
	function startScrollLock(sessionId, initialTargetY) {
		...
		// key) is unaffected.
		let anchorLockStyle = document.getElementById(
			SCROLL_ANCHOR_LOCK_ID,
		);
		if (!anchorLockStyle) {
		...
		scrollLockAbort.signal.addEventListener(
			...
			() => {
				document
					.getElementById(SCROLL_ANCHOR_LOCK_ID)
					?.remove();
			},
			{ once: true },
		);
		...
		window.addEventListener(
			"touchstart",
			() => markGesture("touchstart"),
			{ passive: true, ...sig },
		);
		window.addEventListener(
			"touchmove",
			() => markGesture("touchmove"),
			{ passive: true, ...sig },
		);
		window.addEventListener(
			"keydown",
		...
		if (
			Math.abs(window.scrollY - scrollLockTargetY) > 0.5
		) {
			window.scrollTo({
				top: scrollLockTargetY,
				left: window.scrollX,
	...
	function startVariantObserver(sessionId) {
		...
		const obs = new MutationObserver((mutations) => {
			...
			for (const m of mutations) {
				if (
					m.target.closest?.("[data-impeccable-variants]")
				) {
					dominated = true;
					break;
				}
			...
			if (
				...
				if (isInsert) {
					...
					selectedElement =
						pickVariantContent(wrapper, "original") ||
						wrapper;
				}
			} else if (
	...
	function connectSSE() {
		...
		evtSource.onerror = () => {
			...
			console.log(
				"[impeccable] Live server unreachable. Cleaning up UI.",
			);
			evtSource.close();
			evtSource = null;
			handleServerLost();
	...
	function sendEvent(msg, opts) {
		...
		function handleFailure(err) {
			if (opts && opts.throwOnError) {
				console.error(
					"[impeccable] Failed to send event:",
					err,
				);
				throw err;
			}
			console.debug(
				"[impeccable] Dropped optional live event:",
				err,
			);
			return null;
		}
	...
	function sendCheckpoint(reason) {
		...
		return sendEvent(checkpointPayload(reason)).catch(
			() => null,
		);
	}
	...
	function handleClick(e) {
		if (
			pendingApplyInFlight &&
			!pendingDockEl?.contains(e.target)
		) {
			if (pickerEl?.style.display !== "none")
				hideActionPicker();
			if (own(e.target)) {
		...
		if (
			...
			exitConfigureToPicking("configure-outside-click", {
				clearHover: true,
			});
			return;
		}
		...
		showBar("configure");
		renderEditBadge(
			hasTextRows(selectedElement) ? "idle" : "hidden",
		);
		startScrollTracking();
		maybePrefetchPage();
	...
	function shouldPassthroughElementNav(deepActive, e) {
		...
		if (e.key !== "ArrowUp" && e.key !== "ArrowDown")
			return false;
		if (
			!/^(INPUT|TEXTAREA)$/.test(deepActive.tagName || "")
		)
			return false;
		if (deepActive.value) return false;
		if (
			deepActive.id === PREFIX + "-input" &&
			state === "CONFIGURING"
		)
			return true;
		if (
			deepActive.id === PREFIX + "-page-chat-input" &&
			state === "PICKING"
		)
			return true;
		return false;
	...
	function handleKeyDown(e) {
		...
		if (
			isPageEditableElement(deepActive) &&
			!isInlineEditActive(deepActive)
		) {
			return;
		}
		...
		if (
			...
			isInlineEditActive(e.target)
		) {
			if (e.key !== "Escape") return;
			e.preventDefault();
			e.stopPropagation();
			const original =
				e.target.dataset.impeccableOriginalText;
	...
	function handleGo() {
		...
		pickedAnchorSnapshot =
			buildPickedAnchorSnapshot(elForCapture);
		const captureRect =
			elForCapture.getBoundingClientRect();
		const snapshot = {
		...
		variantObserver = startVariantObserver(
			currentSessionId,
		);
	...
	function handleInsertCreate() {
		...
		insertPlaceholderSnapshot =
			buildInsertPlaceholderSnapshotFromDom(
				insertAnchorElement,
				placeholderElement,
			);
	...
	function loadModernScreenshot() {
		...
		msLoadPromise = new Promise((resolve, reject) => {
			...
			s.onerror = () => {
				msLoadPromise = null;
				reject(
					new Error("modern-screenshot failed to load"),
				);
			};
			uiAppendStyle(s);
		});
		...
		await Promise.all(
			[...urls].map(async (url) => {
				try {
					...
					const ext =
						url.toLowerCase().match(FONT_EXT_RE)?.[1] ||
						"woff2";
					const mime =
						FONT_MIME[ext] || "application/octet-stream";
					map.set(
						url,
						"data:" +
							mime +
	...
	async function captureElementFromRenderedAncestor(
		ms,
		el,
		opts,
	) {
		...
		const rootCanvas = await ms.domToCanvas(
			captureRoot,
			opts,
		);
		const S = opts.scale;
	...
	async function captureElementToBlob(el, snapshot, rect) {
		...
		if (hasAnnotations) {
			...
			annotNode = buildAnnotationsForCapture(
				rect,
				snapshot,
			);
		...
		try {
			...
			const ancestorCanvas = await ms.domToCanvas(
				backdrop,
				opts,
			);
			const S = opts.scale;
	...
	async function captureAndEmit(
		...
		let paper;
		try {
			({ blob, paper } = await captureElementToBlob(
				el,
				snapshot,
				rect,
			));
...
void main() {
  ...
  gl_FragColor = vec4(mix(ground, u_accent, dotAmt), tex.a);
	...
	function paintsBackdrop(node) {
		...
		for (const child of node.children) {
			...
			const paints =
				...
				(ccs.backgroundImage &&
					ccs.backgroundImage !== "none");
			if (!paints) continue;
			const cr = child.getBoundingClientRect();
			if (
				cr.width >= nr.width * 0.9 &&
				cr.height >= nr.height * 0.9
			)
				return true;
		}
		return false;
	...
	function sampleSurroundingRgb(ctx, sx, sy, sw, sh, W, H) {
		const pad = Math.max(
			2,
			Math.round(Math.min(sw, sh) * 0.12),
		);
		...
		for (const [px, py] of pts) {
			const cx = Math.max(
				0,
				Math.min(W - 1, Math.round(px)),
			);
			const cy = Math.max(
				0,
				Math.min(H - 1, Math.round(py)),
			);
	...
	function hideShaderOverlay() {
		...
		const lose = shaderState.gl?.getExtension?.(
			"WEBGL_lose_context",
		);
		try {
			lose?.loseContext();
	...
	async function showShaderOverlay(el, blob, rect, paper) {
		...
		const uPaper = gl.getUniformLocation(
			program,
			"u_paper",
		);
		...
		function frame() {
			if (!shaderState) return;
			const elapsed =
				(performance.now() - shaderState.startTime) / 1000;
	...
	async function handleAccept() {
		...
		const domVisibleVariant = readVisibleVariantFromDOM(
			currentSessionId,
		);
		...
		sendEvent(acceptPayload, { throwOnError: true })
			...
			.catch(() => {
				if (
					pendingAcceptedSession?.id === acceptedSessionId
				)
					pendingAcceptedSession = null;
				setLiveState("CYCLING");
				showOrUpdateCyclingBar();
	...
	function scheduleAcceptCleanup(accepted) {
		setTimeout(function () {
			if (
				!accepted?.isSvelteComponent &&
				!acceptedDomAlreadyClean(accepted)
			) {
				setTimeout(function () {
	...
	function snapshotAcceptedVariantDom(
		sessionId,
		variantId,
	) {
		const wrapper = document.querySelector(
		...
		return {
			...
			parentSelector: selectorForAcceptedRoot(
	...
	function selectorForAcceptedRoot(root) {
		...
		const classes = [...(root.classList || [])].filter(
			Boolean,
		);
	...
	function commitAcceptedVariantToDom(
		sessionId,
		variantId,
	) {
		const wrapper = document.querySelector(
		...
		const style = wrapper.querySelector(
			"style[data-impeccable-css]",
		);
		if (
			style &&
			!document.querySelector(
				'style[data-impeccable-accepted-css="' +
					sessionId +
					'"]',
			)
		) {
			const promotedStyle = style.cloneNode(true);
			promotedStyle.setAttribute(
				"data-impeccable-accepted-css",
				sessionId,
			);
	...
	function handleDiscard() {
		...
		if (!currentSessionId) return;
		sendEvent(
			{ type: "discard", id: currentSessionId },
			{ throwOnError: true },
		)
			.then(() => {
				markSessionHandled();
	...
	function rememberSessionFileMeta(meta = {}) {
		...
		const sourceFile = normalizeSessionPath(
			meta.sourceFile,
		);
		const previewFile = normalizeSessionPath(
			meta.previewFile,
		);
		const previewMode =
			meta.previewMode ||
	...
	function applySavedSessionMeta(saved) {
		...
		if (
			saved.paramValues &&
			typeof saved.paramValues === "object"
		) {
	...
	function restoreSessionWithoutWrapper(
		reason,
		activeSessions,
	) {
		const saved = loadSession();
		...
		if (
			savedState !== "GENERATING" &&
			savedState !== "CYCLING"
		)
			return false;

		const serverSession = findActiveSessionSummary(
			saved,
			activeSessions,
		);
		if (
		...
		saveSession();
		queueCheckpoint(
			reason || "browser_restore_without_wrapper",
		);
	...
	function restoreFromActiveSessions(
		activeSessions,
		reason,
	) {
		const wrapper = document.querySelector(
	...
	function saveSession() {
		...
		sessionState.saveSession({
			...
			paramValues: { ...paramsCurrentValues },
			insertPlaceholder:
				insertPlaceholderSnapshot || undefined,
	...
	function cleanup() {
		...
		if (
			svelteComponentSession?.sessionId === cleanupSessionId
		) {
			teardownSvelteComponentSession(true);
			...
			setTimeout(function () {
				...
				if (orig) {
					...
					if (content) {
						lateWrapper.parentElement.replaceChild(
							content,
							lateWrapper,
						);
						return;
		...
		if (state === "GENERATING") {
			...
			if (shaderTarget) {
				(async () => {
					try {
						const rect =
							shaderTarget.getBoundingClientRect();
						if (rect.width === 0 || rect.height === 0)
							return;
						const { blob, paper } =
							await captureElementToBlob(
								shaderTarget,
								null,
								rect,
							);
	...
	function detectPageTheme() {
		try {
			...
			const rgb =
				readOpaque(document.body) ||
				readOpaque(document.documentElement);
	...
	function applyGlobalBarLabelState(
		expandInactive,
		forceCollapse = false,
	) {
	...
	function syncGlobalBarExpandedLabels(
		...
		const expandInactive = !!(
			expanded && !pageChatExpanded
		);
	...
	function syncPageChatChrome() {
		...
		const inputFocused =
			pageChatInput &&
			activeElementDeep() === pageChatInput;
		...
		if (pageChatVoiceBtn) {
			const listening =
				pageChatVoiceBtn.dataset.listening === "true";
	...
	function isPageEditableActive() {
		...
		return (
			isPageEditableElement(active) &&
			!isInlineEditActive(active)
		);
	}
	...
	function scheduleSteerFocusRecover(reason) {
		...
		const attempt = () => {
			...
			const pauseLeft =
				steerFocusPauseUntil - performance.now();
			if (pauseLeft > 0) {
				steerFocusRecoverTimer = setTimeout(
					attempt,
					pauseLeft,
				);
				return;
	...
	function notePagePointerDown(e) {
		...
		steerFocusPauseUntil =
			performance.now() + STEER_PAGE_FOCUS_PAUSE_MS;
		pagePointerGesture = {
			x: e.clientX,
			y: e.clientY,
			dragged: false,
	...
	function steerFocusDebugEnabled() {
		try {
			return (
				localStorage.getItem("impeccable-steer-debug") ===
				"1"
			);
		} catch {
			return false;
	...
	function attachSteerFocusDebug() {
		...
		document.addEventListener(
			...
			(e) => {
				...
				steerFocusLog("focusin", {
					target: steerFocusTargetLabel(e.target),
				});
			},
			true,
		);
	...
	function focusConfigureInput(reason) {
		...
		if (!input) {
			steerFocusLog("focusConfigureInput missing", {
				reason,
			});
			return;
		}
	...
	function syncPageChatFocusRing() {
		...
		pageChatEl.dataset.inputFocused = focused
			? "true"
			: "false";
		pageChatEl.style.boxShadow = "none";

		if (pageChatExpanded) {
			pageChatInput.placeholder =
				PAGE_CHAT_PLACEHOLDER_EXPANDED;
			pageChatInput.style.width = "";
			pageChatInput.style.padding = "0 6px";
			pageChatInput.style.opacity = steerLocked
				? "0.72"
				: "1";
			pageChatInput.style.pointerEvents = steerLocked
				? "none"
				: "auto";
			return;
		}
		...
		if (typingReady) {
			...
			pageChatInput.placeholder =
				PAGE_CHAT_PLACEHOLDER_COLLAPSED;
			if (pageChatHint) {
		...
		for (let i = 0; i < 3; i++) {
			wrap.appendChild(
				el("span", {
					...
					animation:
						"impeccable-steer-dot 1.05s ease-in-out " +
						i * 0.14 +
						"s infinite",
				}),
			);
	...
	function preparePageChatInputForTyping() {
		...
		pageChatEl.style.width = pageChatExpandedWidth();
		pageChatEl.style.cursor = steerLocked
			? "default"
			: "text";
		pageChatInput.placeholder =
			PAGE_CHAT_PLACEHOLDER_EXPANDED;
	...
	function lockSteerChat() {
		...
		pageChatEl.setAttribute(
			"aria-label",
			"Processing steer request",
		);
		if (!pageChatDotsEl) {
	...
	function unlockSteerChat(opts) {
		...
		const restoreMessage =
			typeof opts?.restoreMessage === "string"
				? opts.restoreMessage
				: "";
		const keepExpanded = Boolean(
			opts?.error && restoreMessage,
		);
		steerLocked = false;
		...
		pageChatEl.dataset.expanded = keepExpanded
			? "true"
			: "false";
		...
		if (pageChatInput) {
			...
			pageChatInput.value = keepExpanded
				? restoreMessage
				: "";
			pageChatInput.style.width = keepExpanded ? "" : "0";
			pageChatInput.style.padding = keepExpanded
				? "0 6px"
				: "0";
			pageChatInput.style.opacity = keepExpanded
				? "1"
				: "0";
			pageChatInput.style.pointerEvents = "auto";
		}
		...
		if (pageChatHint) {
			...
			pageChatHint.style.display = keepExpanded
				? "none"
				: "";
			pageChatHint.style.visibility = keepExpanded
				? "hidden"
				: "";
		...
		steerPendingMessage = keepExpanded
			? restoreMessage
			: "";
	...
	function syncVoiceUi(listening) {
		...
		if (voiceCtx?.mode === "steer") {
			if (pageChatVoiceBtn) {
				pageChatVoiceBtn.dataset.active = listening
					? "true"
					: "false";
				pageChatVoiceBtn.dataset.listening = listening
					? "true"
					: "false";
			...
			if (pageChatEl)
				pageChatEl.dataset.voiceListening = listening
					? "true"
					: "false";
			syncPageChatChrome();
			...
			if (voiceBtn) {
				voiceBtn.dataset.active = listening
					? "true"
					: "false";
				voiceBtn.dataset.listening = listening
					? "true"
					: "false";
	...
	function startVoice(ctx) {
		...
		rec.onresult = (event) => {
			...
			voiceCtx.input.value = (
				voiceInterimBase + transcript
			).trim();
	...
	function submitSteerMessage() {
		...
		sendEvent({
			...
			if (!res) {
				sendSteerCheckpoint(id, "steer_send_failed", {
					message: text,
				});
				unlockSteerChat({
	...
	function collapsePageChat(opts) {
		...
		pageChatEl.style.cursor = "pointer";
		syncGlobalBarExpandedLabels(
			globalBarEl?.matches(":hover"),
		);
		if (blur) {
			pageChatInput.blur();
			pageChatInput.style.pointerEvents = "none";
	...
	function initPageChat(parent, P) {
		pageChatEl = el("div", {
			...
			margin:
				"0 4px 0 " +
				(GLOBAL_BAR_SECTION_GAP - GLOBAL_BAR_INNER_GAP) +
				"px",
			borderRadius: "7px",
			background: P.chatSurface,
			border: "1px solid transparent",
		...
		pageChatVoiceBtn.addEventListener(
			"pointerdown",
			keepSteerPointerInside,
		);
		pageChatVoiceBtn.addEventListener(
		...
		pageChatInput.addEventListener(
			"pointerdown",
			keepSteerPointerInside,
		);
		pageChatInput.addEventListener(
	...
	function syncAgentPollingUi(connected) {
		...
		globalBarBrandEl.dataset.agentConnected = connected
			? "true"
			: "false";
		...
		globalBarBrandEl.style.cursor = connected
			? "default"
			: "help";
		const mark = globalBarBrandEl.querySelector(
			"[data-brand-mark]",
		);
		if (mark) {
		...
		const dot = globalBarBrandEl.querySelector(
			"[data-agent-dot]",
		);
		if (dot)
	...
	function initGlobalBar() {
		...
		const brand = el("span", {
			...
			padding:
				"0 " +
				(GLOBAL_BAR_SECTION_GAP -
					GLOBAL_BAR_INNER_PAD_LEFT) +
				"px 0 14px",
			background: "transparent",
			color: P.accent,
			flexShrink: "0",
		...
		const inner = el("div", {
			...
			alignItems: "center",
			padding:
				"4px 5px 4px " + GLOBAL_BAR_INNER_PAD_LEFT + "px",
		...
		pendingPillEl.addEventListener(
			"click",
			onPendingPillClick,
		);
		...
		pendingTrashTooltipEl.textContent =
			"Discard copy edits";
		...
		pendingTrashBtn.addEventListener(
			"mouseenter",
			showTrashTooltip,
		);
		pendingTrashBtn.addEventListener(
		...
		const makePendingDecisionBtn = (label, accent) => {
			const btn = el("button", {
				...
				borderRadius: "999px",
				border:
					"1px solid " + (accent ? P.accent : P.hairline),
		...
		pendingKeepFixingBtn = makePendingDecisionBtn(
			"Keep fixing",
			true,
		);
		...
		pendingKeepFixingBtn.addEventListener(
			"click",
			onPendingKeepFixingClick,
		);
		pendingRollbackBtn = makePendingDecisionBtn(
			"Rollback",
			false,
		);
		...
		pendingRollbackBtn.addEventListener(
			"click",
			onPendingRollbackClick,
		);
	...
	function updateGlobalBarState() {
		...
		[
			...
			if (!btn) return;
			btn.disabled = controlsLocked;
			btn.style.cursor = controlsLocked
				? "not-allowed"
				: "pointer";
		...
		syncGlobalBarExpandedLabels(
			globalBarEl && globalBarEl.matches(":hover"),
		);
		...
		document
			.querySelectorAll(".impeccable-overlay")
			.forEach((o) => {
				o.style.pointerEvents =
					pickActive || insertActive ? "none" : "";
			});
		syncPageInteractionCursor();
	}
	...
	function onDetectMessage(e) {
		...
		if (e.data.source === "impeccable-results") {
			...
			if (
				activeDetectScanId &&
				e.data.scanId !== activeDetectScanId
			)
				return;
	...
	function loadDesignPrefs() {
		...
		try {
			...
			if (
				prefs.collapsed &&
				typeof prefs.collapsed === "object"
			) {
				Object.assign(
					designState.collapsed,
					prefs.collapsed,
				);
			}
		} catch {
	...
	function initDesignPanel() {
		...
		designShadow = designHost.attachShadow({
			mode: "open",
		});
		...
		defangOutsideHandlers(designHost, {
			setPointerEvents: false,
		});
	...
	async function fetchDesignSystem() {
		...
		try {
			...
			designState.mdNewerThanJson =
				!!jsonData.mdNewerThanJson;
			designState.raw =
				designState.present && rawRes.ok
					? await rawRes.text()
					: null;
			designState.error =
				jsonData.parseError ||
				jsonData.sidecarError ||
				null;
		} catch (err) {
			designState.error =
				err?.message || "Failed to load design system.";
		} finally {
	...
	function renderDesignBody() {
		...
		if (designState.loading) {
			body.appendChild(
				msgDiv("loading", "Loading design system…"),
			);
			return;
		}
	...
	function renderDesignVisual(body, parsed, sidecar) {
		...
		const narrative =
			sidecar?.narrative || synthesizeNarrative(parsed);
		...
		if (narrative.dos?.length || narrative.donts?.length)
			body.appendChild(
				renderDosDontsCollapsible(narrative),
			);
		if (
			narrative.overview ||
			narrative.northStar ||
			...
			body.appendChild(
				renderOverviewCollapsible(narrative),
			);
		}
	...
	function buildTypographyModels(
		fmTypography,
		typographyMeta,
	) {
		if (!fmTypography) return [];
		const meta = typographyMeta || {};
		return Object.entries(fmTypography).map(
			([key, spec]) => {
				const m = meta[key] || {};
				const { family, fallback } = splitFontFamily(
					spec?.fontFamily,
				);
				return {
					role: key,
	...
	function buildRadiiModels(fmRounded) {
		if (!fmRounded) return [];
		return Object.entries(fmRounded).map(
		...
		for (const g of proseColors.groups) {
			for (const c of g.colors || []) {
				...
				if (
					hay &&
					needles.some(
						(n) => hay.includes(n) || n.includes(hay),
					)
				) {
	...
	function synthesizeNarrative(parsed) {
		...
		return {
			...
			keyCharacteristics:
				md.overview?.keyCharacteristics || [],
			rules: [
				...(md.colors?.rules || []).map((r) => ({
					...r,
					section: "colors",
				})),
		...
		for (const c of colors) {
			...
			if (ramp.length) {
				...
				r.innerHTML = ramp
					.map(
						(v) =>
							`<span style="background:${cssSafe(v)}"></span>`,
					)
					.join("");
				tile.appendChild(r);
			}
	...
	function renderDosDontsCollapsible(n) {
		const total =
			(n.dos?.length || 0) + (n.donts?.length || 0);
		const { wrap, body } = buildCollapsible(
			"dosdonts",
			"Do's and Don'ts",
			total,
	...
	function renderMarkdown(md) {
		...
		for (; i < lines.length; i++) {
			...
			if (heading) {
				...
				out.push(
					`<h${lvl}>${inlineMd(heading[2])}</h${lvl}>`,
				);
				continue;
			}
