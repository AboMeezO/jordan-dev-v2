...
function generatePinnedSkill(command, metadata) {
	const desc =
		...
		`Shortcut for /impeccable ${command}.`;
	const hint =
		metadata[command]?.argumentHint || "[target]";
...
function pin(command, projectRoot) {
	...
	if (harnessDirs.length === 0) {
		console.log(
			"No harness directories with impeccable installed found.",
		);
		return false;
	}
		...
		if (existsSync(skillDir)) {
			...
			if (existsSync(existingMd)) {
				...
				if (!existing.includes(PIN_MARKER)) {
					console.log(
						`  SKIP: ${skillDir} (non-pinned skill already exists)`,
					);
					continue;
				}
			}
...
function unpin(command, projectRoot) {
	...
	for (const skillsDir of harnessDirs) {
		...
		if (!content.includes(PIN_MARKER)) {
			console.log(
				`  SKIP: ${skillDir} (not a pinned skill)`,
			);
			continue;
		}
...
if (action !== "pin" && action !== "unpin") {
	console.error(
		`Unknown action: ${action}. Use 'pin' or 'unpin'.`,
	);
	process.exit(1);
}
