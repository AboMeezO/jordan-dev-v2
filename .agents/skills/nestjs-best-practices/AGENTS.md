...
export class OrdersService {
	...
	async createOrder(dto: CreateOrderDto): Promise<Order> {
		...
		const inventoryService = this.moduleRef.get(
			InventoryService,
		);
		const paymentService =
			this.moduleRef.get(PaymentService);
...
export class OrdersService {
	...
	async createOrder(dto: CreateOrderDto): Promise<Order> {
		...
		const inventory = await this.inventoryService.check(
			dto.items,
		);
...
interface EmailAndSmsSender
	extends EmailSender, SmsSender {}

// Or use intersection types
type MultiChannelSender = EmailSender &
	SmsSender &
	PushSender;
...
export class MockPaymentService implements PaymentGateway {
	async charge(
		...
		if (amount > 1000) {
			throw new Error(
				"Mock does not support large amounts",
			);
		}
...
export class MockPaymentService implements PaymentGateway {
	...
	async refund(
		...
		return {
			...
			amount:
				amount ??
				this.transactions.get(transactionId)!.amount,
		};
	}
}
...
describe("StripeService", () => {
	testPaymentGatewayContract(
		() => new StripeService(mockStripeClient),
	);
});
...
export class ReportsService {
	...
	async generateReport(type: string): Promise<Report> {
		...
		const { ReportsModule } =
			await import("./reports/reports.module");
		const moduleRef = await this.lazyModuleLoader.load(
			() => ReportsModule,
		);

		const reportsService = moduleRef.get(
...
export class AdminService {
	...
	private async getAdminModule(): Promise<ModuleRef> {
		if (!this.adminModule) {
			...
			this.adminModule = await this.lazyModuleLoader.load(
				() => AdminModule,
			);
		}
...
export class ProductsService {
	...
	async getPopular(): Promise<Product[]> {
		...
		// Try cache first
		const cached =
			await this.cache.get<Product[]>(cacheKey);
...
export class OrdersService {
	async getOrdersWithItems(
		...
		for (const order of orders) {
			...
			order.items = await this.itemRepo.find({
...
export class OrdersService {
	...
	async createOrder(
		...
		return this.dataSource.transaction(async (manager) => {
			...
			await this.paymentService.chargeWithManager(
				manager,
				order.id,
			);
...
export class UsersController {
	...
	async findOne(
		...
		const user =
			await this.usersService.findByIdWithPosts(id);
		...
		const job = await this.reportsQueue.add(
			...
			dto,
			{
				...
				delay: dto.scheduledFor
					? Date.parse(dto.scheduledFor) - Date.now()
					: 0,
			},
		);
...
export class ScheduledJobsService implements OnModuleInit {
	constructor(
		@InjectQueue("maintenance") private queue: Queue,
	) {}
...
export class EmailService {
	sendEmail() {
		...
		const apiKey =
			process.env.SENDGRID_API_KEY || "default";
