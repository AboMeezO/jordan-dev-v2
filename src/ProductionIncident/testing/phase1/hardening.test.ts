...
async function createRunningHarness(seed: string) {
  ...
  const firstPlayer = createPlayer(idGenerator, "a", "A", clock.now());
  const secondPlayer = createPlayer(idGenerator, "b", "B", clock.now());

  requireOk(await kernel.sessionManager.joinSession({ player: firstPlayer, sessionId }));
  requireOk(await kernel.sessionManager.joinSession({ player: secondPlayer, sessionId }));
  requireOk(await kernel.sessionManager.startSession({ minimumPlayers: 2, sessionId }));

  return { firstPlayer, harness, secondPlayer, sessionId };
}

const noVote = await createRunningHarness("hardening-no-vote");
...
const noVoteSession = noVote.harness.kernel.stateManager.getSnapshot(noVote.sessionId);
assertSessionStatus(noVoteSession, "running");
assert.equal(noVoteSession.state.activeIncidents.has(noVoteIncident.id), false);
assert.equal(noVoteSession.state.incidentHistory.has(noVoteIncident.id), true);
assert.equal(noVoteSession.state.voteWindows.has(noVoteIncident.id), false);

const replacement = await createRunningHarness("hardening-replacement");
...
const replacementSession = replacement.harness.kernel.stateManager.getSnapshot(
...
assert.equal(
  replacementSession.state.voteWindows
    ...
    ?.votesByPlayerId.get(replacement.firstPlayer.id)?.actionId,
  secondAction.id,
);
...
const tieLowRisk = requireDefined(tieIncident.actionOptions[0]);
const tieHigherRisk = requireDefined(tieIncident.actionOptions[1]);
requireOk(
...
const tieSession = tie.harness.kernel.stateManager.getSnapshot(tie.sessionId);
assertSessionStatus(tieSession, "running");
assert.equal(tieSession.state.incidentHistory.get(tieIncident.id)?.selectedActionId, tieLowRisk.id);

const endDuringVote = await createRunningHarness("hardening-end-during-vote");
...
assert.equal(endDuringVote.harness.scheduler.activeTasks.length > 0, true);
requireOk(
...
assert.equal(endDuringVote.harness.scheduler.activeTasks.length, 0);
...
assert.throws(
  () => validateCatalogs(INCIDENT_TEMPLATES, [ACTION_CATALOG[0]!, duplicateAction]),
  CatalogValidationError,
);
