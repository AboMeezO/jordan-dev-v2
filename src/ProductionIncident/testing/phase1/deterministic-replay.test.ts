...
async function runReplay(seed: string): Promise<ReplaySnapshot> {
  ...
  const finalState = kernel.stateManager.getSnapshot(sessionId);
  ...
  const finalStateSnapshot =
    finalState.state.status === "ended"
      ? {
          ...
          playerIds: [...finalState.state.players.keys()].sort(),
