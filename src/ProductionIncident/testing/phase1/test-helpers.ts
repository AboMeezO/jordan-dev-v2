...
export class RecordingScheduler implements Scheduler {
  private counter = 0;
  private readonly tasks = new Map<TimerId, RecordedScheduledTask>();
...
export function createPhase1Harness(seed = "phase-1-test"): Phase1Harness {
...
): { readonly events: readonly GameEvent[]; readonly value: TValue } {
...
export function requireFailure<TValue>(
  result: EngineCommandResult<TValue>,
): Exclude<EngineCommandResult<TValue>, { readonly ok: true }>["error"] {
  assert.equal(result.ok, false, "Expected command to fail.");
  return result.error;
}
...
export function assertSessionStatus<TStatus extends GameSession["state"]["status"]>(
  session: GameSession | undefined,
  expectedStatus: TStatus,
): asserts session is GameSession & {
  readonly state: Extract<SessionState, { readonly status: TStatus }>;
} {
  if (session === undefined) {
    assert.fail(`Expected session status ${expectedStatus}, but session was undefined.`);
  }
