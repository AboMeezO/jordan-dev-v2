...
export class IncidentEngine {
  ...
  public createIncident(
    ...
    return {
      ...
      votingClosesAt: (createdAt + voteWindowMs) as UnixMillis,
    };
  }

  public selectTemplate(session: GameSession): IncidentTemplate {
    ...
    const candidates = eligible.length > 0 ? eligible : this.templates;
  ...
  private pick<TValue>(values: readonly TValue[]): TValue {
    const value = values[this.randomSource.nextInteger(0, values.length - 1)];
