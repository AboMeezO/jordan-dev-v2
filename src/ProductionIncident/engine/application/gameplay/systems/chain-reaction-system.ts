...
import type { Clock, RandomSource } from "../../../ports/index.js";
...
export class ChainReactionSystem {
  ...
  private readonly activeDepthBySessionId = new Map<SessionId, number>();
  ...
  public constructor(
    ...
    private readonly generateIncident: (sessionId: SessionId) => Promise<void>,
  ...
  public maybeSchedule(
    ...
    const currentDepth = this.activeDepthBySessionId.get(sessionId) ?? 0;

    if (currentDepth >= MAX_CHAIN_DEPTH || !this.isSessionActive(sessionId)) {
      return undefined;
    }

    const instability = (100 - session.stats.serverStability) / 100;
    const shouldChain = this.randomSource.nextFloat() < 0.25 + instability * 0.5;
  ...
  private decrementDepth(sessionId: SessionId): void {
    ...
    this.activeDepthBySessionId.set(sessionId, currentDepth - 1);
  }
}
