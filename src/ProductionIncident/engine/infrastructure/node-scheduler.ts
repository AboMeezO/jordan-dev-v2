...
export class NodeScheduler implements Scheduler {
  private counter = 0;
  private readonly timeouts = new Map<TimerId, NodeJS.Timeout>();
  private readonly tasks = new Map<TimerId, ScheduledTask>();
  ...
  public cancelBySession(sessionId: SessionId): number {
    ...
    for (const task of this.tasks.values()) {
      if (task.sessionId === sessionId && this.cancel(task.id)) {
        cancelled += 1;
      }
    }
  ...
  public scheduleAt(
    ...
    return this.scheduleOnce(Math.max(0, timestamp - Date.now()), handler, sessionId);
  }
  ...
  public scheduleOnce(
    ...
    const task =
      sessionId === undefined
        ...
        : { id: `timer-${this.counter}` as TimerId, sessionId };
