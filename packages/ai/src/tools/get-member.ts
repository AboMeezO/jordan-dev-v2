...
export const getMemberById = createTool({
	...
	inputSchema: z.object({
		guildId: z.string().describe("The guild ID the member belongs to."),
		userId: z.string().describe("The Discord user ID to look up."),
	}),
	execute: async (ctx, input) => {
		const guild = ctx.client.guilds.cache.get(input.guildId);
		if (!guild) return `No guild found with ID ${input.guildId}.`;
		const member = await guild.members.fetch(input.userId).catch(() => null);
		if (!member) return `No member found with ID ${input.userId} in guild ${input.guildId}.`;
