...
export function isSchemaField(value: unknown): value is SchemaField {
	...
	const validKeys = new Set(["type", "default", "optional", "min", "max", "pattern", "description"]);
	...
	for (const [key, value] of Object.entries(def)) {
		...
		if (isSchemaField(value)) {
			...
			flatKeys.set(fullPath, {
				type,
				required: value.default === undefined && !value.optional,
				default: value.default,
			});
			if (value.default !== undefined) {
				setNested(defaults, fullPath.split("."), value.default);
			}
		} else if (typeof value === "object" && value !== null) {
			collectDefaults(value as SchemaDefinition, fullPath, defaults, flatKeys);
		}
	}
}

function compileObject(def: SchemaDefinition): z.ZodObject<z.ZodRawShape> {
...
export function parseSchema(raw: Record<string, unknown>): CompiledSchema {
	const schemaDef = raw as SchemaDefinition;
	const defaults: Record<string, unknown> = {};
	const flatKeys = new Map<string, { type: string; required: boolean; default?: unknown }>();
