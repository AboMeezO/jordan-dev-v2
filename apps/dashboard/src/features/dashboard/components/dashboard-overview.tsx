    ...
    <>
      <section className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_300px]">
        <div className="nd-panel min-w-0 overflow-hidden p-6">
          ...
          <div className="mt-6 flex flex-wrap items-baseline gap-x-4 gap-y-2">
            <span className="font-display text-[clamp(4rem,10vw,7rem)] leading-none tracking-[-0.05em] text-[var(--nd-text-display)]">
              {compactNumber(stats.totalUsers)}
            </span>
            <span className="font-mono text-sm uppercase tracking-[0.16em] text-[var(--nd-text-secondary)]">
              total users
            </span>
          </div>

          <div className="mt-6 flex flex-wrap gap-x-8 gap-y-2 text-sm text-[var(--nd-text-secondary)]">
            <div className="flex items-center gap-2">
              <span className="nd-label">Database</span>
              <span className={system.databaseReady ? 'font-mono text-xs text-[var(--nd-success)]' : 'font-mono text-xs text-[var(--nd-accent)]'}>
                {system.databaseReady ? 'CONNECTED' : 'DISCONNECTED'}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="nd-label">Generated</span>
              <span className="font-mono text-xs text-[var(--nd-text-disabled)]">
        ...
        <div className="nd-panel p-6">
          ...
          <div className="mt-6 grid gap-5">
            <div className="flex items-center justify-between border-b border-[var(--nd-border)] pb-3">
              <span className="nd-label">Verified</span>
              <span className="font-mono text-lg text-[var(--nd-success)]">{compactNumber(stats.verifiedUsers)}</span>
            </div>
            <div className="flex items-center justify-between border-b border-[var(--nd-border)] pb-3">
              <span className="nd-label">Unverified</span>
              <span className={`font-mono text-lg ${stats.unverifiedUsers > 0 ? 'text-[var(--nd-warning)]' : 'text-[var(--nd-text-display)]'}`}>{compactNumber(stats.unverifiedUsers)}</span>
            </div>
            <div className="flex items-center justify-between border-b border-[var(--nd-border)] pb-3">
              <span className="nd-label">Pending Grants</span>
              <span className={`font-mono text-lg ${stats.pendingRoleGrants > 0 ? 'text-[var(--nd-warning)]' : 'text-[var(--nd-text-display)]'}`}>{String(stats.pendingRoleGrants)}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="nd-label">Roles</span>
              <span className="font-mono text-lg text-[var(--nd-text-display)]">{String(stats.totalRoles)}</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="grid min-w-0 gap-6 xl:grid-cols-[1fr_1fr]">
        <div className="nd-panel min-w-0 overflow-hidden">
          <div className="border-b border-[var(--nd-border-visible)] p-5">
            <p className="nd-label">Activity</p>
            <h3 className="mt-1 font-sans text-xl font-medium tracking-[-0.02em] text-[var(--nd-text-display)]">
              Recent Users
            </h3>
          </div>
            ...
            <div className="divide-y divide-[var(--nd-border)]">
              {recentUsers.map((user) => (
                <div className="flex items-center gap-3 px-5 py-3" key={user.id}>
                  <span className="grid size-8 shrink-0 place-items-center rounded-full border border-[var(--nd-border-visible)] font-mono text-xs text-[var(--nd-text-secondary)]">
                    {user.displayName?.charAt(0)?.toUpperCase() ?? '?'}
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-sans text-sm text-[var(--nd-text-primary)]">
                      {user.displayName ?? user.email ?? 'Unknown'}
                    </p>
                    <p className="truncate font-mono text-[11px] text-[var(--nd-text-disabled)]">
                      {user.email ?? 'No email'}
                    </p>
                  </div>
            ...
            <div className="divide-y divide-[var(--nd-border)]">
              {recentVerificationEvents.map((event) => (
                <div className="px-5 py-3" key={event.id}>
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-xs text-[var(--nd-text-secondary)]">{event.type}</span>
                    <span className="font-mono text-[11px] text-[var(--nd-text-disabled)]">
                      {new Date(event.createdAt).toLocaleDateString()}
                    </span>
                  </div>
                  <p className="mt-1 font-mono text-[11px] text-[var(--nd-text-disabled)]">
                    {event.status}
