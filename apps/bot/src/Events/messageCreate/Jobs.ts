...
function pickRoast(roasts: readonly string[], userMention: string): string {
  const roast = roasts[Math.floor(Math.random() * roasts.length)];

  return (roast ?? "{user}, behave.").replace("{user}", userMention);
}
...
async function isWorkingLink(url: string): Promise<boolean> {
  try {
...
export default async function (
  ...
  if (message.channel.id !== process.env.JOBS_CHANNEL) return;
