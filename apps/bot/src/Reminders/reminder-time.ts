...
export function parseReminderTime(
  ...
  const relativeTimestamp = parseRelativeTimestamp(value, now);
  if (relativeTimestamp !== null) {
    return { date: new Date(relativeTimestamp), timestamp: relativeTimestamp };
  }

  const clockTimestamp = parseClockTimestamp(value, now);
  if (clockTimestamp !== null) {
    return { date: new Date(clockTimestamp), timestamp: clockTimestamp };
  }

  const absolute = new Date(value);
  if (Number.isNaN(absolute.getTime()) || absolute.getTime() <= now.getTime()) {
    return null;
  }
