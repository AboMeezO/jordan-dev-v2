...
export class DiscordIncidentRenderer {
	...
	public renderIncidentPrompt(
		...
		const voteCountByActionId =
			this.voteCountByActionId(voteWindow);
		const voteButtons = incident.actionOptions.map(
			(action) =>
				this.actionButton(
					action,
		...
		const instantButtons =
			incident.instantActionOptions.map((action) =>
				this.actionButton(
					action,
		...
		return {
			...
			buttonRows: [
				{ buttons: voteButtons },
				...(instantButtons.length === 0
					? []
					: [{ buttons: instantButtons }]),
			],
			content: [
				`${this.emojis.emoji("incident")} **Production Incident**`,
				this.incidentCounter(
					options.incidentNumber,
	...
	public renderSessionSummary(
		session: GameSession,
	): DiscordMessagePayload {
		return {
			content: this.renderStatsText(
				"System Status",
				session.stats,
	...
	private discordTimestamp(
		timestamp: Incident["votingClosesAt"],
	): string {
