...
export class EngineDiscordBridge {
	private readonly customIdCodec =
	...
	public mapEventToRenderAction(
		event: GameEvent,
	): DiscordRenderAction | undefined {
		switch (event.type) {
			case "incident.generated": {
				const session = this.stateManager.getSnapshot(
					event.sessionId,
				);
				const incident =
				...
				return incident === undefined
					...
					: {
							payload: this.renderer.renderIncidentPrompt(
								incident,
								() =>
									this.customIdCodec.encodeAction({
										key: "a1" as DiscordActionRouteKey,
									}),
								() =>
									this.customIdCodec.encodeInstant({
										key: "i1" as DiscordActionRouteKey,
									}),
							),
							type: "send",
						};
			...
			case "incident.resolved": {
				...
				return incident === undefined
					...
					: {
							...
							payload: this.renderer.renderIncidentOutcome(
								incident,
								event.succeeded,
							),
			...
			case "commentary.cued":
				return {
					payload: this.renderer.renderCommentary(
						event.message,
					),
					type: "send",
				};
