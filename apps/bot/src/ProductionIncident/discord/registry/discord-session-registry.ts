...
export class DiscordSessionRegistry {
	...
	private readonly actionRoutes = new Map<
		DiscordActionRouteKey,
		DiscordActionRoute
	>();
	private readonly byChannelId = new Map<
		string,
		SessionId
	>();
	private readonly bySessionId = new Map<
		SessionId,
		DiscordSessionBinding
	>();
	private readonly incidentMessageIds = new Map<
		SessionId,
	...
	public getByChannel(
		channelId: string,
	): DiscordSessionBinding | undefined {
		const sessionId = this.byChannelId.get(channelId);
		return sessionId === undefined
			? undefined
			: this.bySessionId.get(sessionId);
	}
	...
	public getIncidentMessageId(
		...
		return this.incidentMessageIds
			.get(sessionId)
			?.get(incidentId);
	}
	...
	public setIncidentMessageId(
		...
		const messages =
			this.incidentMessageIds.get(sessionId) ??
			new Map<IncidentId, string>();
	...
	public cleanupSessionActionRoutes(
		sessionId: SessionId,
	): void {
		for (const [
			key,
	...
	public registerActionRoute(
		...
		const route: DiscordActionRoute =
	...
	public updateSession(
		sessionId: SessionId,
		update: Partial<
			Omit<DiscordSessionBinding, "sessionId">
		>,
	): void {
		const binding = this.bySessionId.get(sessionId);
