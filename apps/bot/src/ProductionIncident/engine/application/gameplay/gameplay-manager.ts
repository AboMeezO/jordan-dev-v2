...
const SEVERITY_PENALTY: Readonly<
...
export class ProductionIncidentGameplayManager implements GameplayManager {
	...
	private readonly voteTimerByIncidentId = new Map<
		Incident["id"],
		ReturnType<Scheduler["scheduleOnce"]>
	>();
	private readonly votingSystem: VotingSystem;

	public constructor(
		private readonly dependencies: GameplayManagerDependencies,
	) {
		this.actionGenerationSystem =
			new ActionGenerationSystem(dependencies.actions);
		...
		this.dependencies.eventBus.subscribe(
			"session.started",
			(event) => {
		...
		this.dependencies.eventBus.subscribe(
			"session.ended",
			(event) => {
				this.dependencies.scheduler.cancelBySession(
					event.sessionId,
				);
	...
	public async closeVote(
		...
		const session =
			this.dependencies.stateManager.getSnapshot(
				input.sessionId,
			);
		...
		const incident = session.state.activeIncidents.get(
			input.incidentId,
		);
		const voteWindow = session.state.voteWindows.get(
			input.incidentId,
		);

		if (
			incident === undefined ||
			voteWindow === undefined
		) {
			return this.failure(
				"invalid-session-state",
		...
		await this.dependencies.stateManager.closeVoteWindow(
			input.sessionId,
			input.incidentId,
		);
		const selectedAction =
			this.votingSystem.selectWinningAction(incident, [
				...voteWindow.votesByPlayerId.values(),
			]);
		const voteClosed = this.createVoteClosedEvent(
			input.sessionId,
			input.incidentId,
			selectedAction?.id,
		);
		...
		if (selectedAction === undefined) {
			...
			const expired =
				await this.dependencies.stateManager.replaceIncident(
					input.sessionId,
					expiredIncident,
				);
		...
		const resolved = await this.resolveIncident(
			input.sessionId,
			incident,
			selectedAction,
		);
	...
	public async generateIncident(
		...
		const template =
			this.incidentEngine.selectTemplate(session);
		const escalationLevel =
			this.escalationDirector.getLevel(input.sessionId);
		...
		const actionOptions =
			this.actionGenerationSystem.selectActions(
				template,
				severity,
			);
		const instantActionOptions =
			this.actionGenerationSystem.selectInstantActions(
				template,
			);
		...
		if (votingClosesAt === undefined) {
			throw new Error(
				"Generated incident is missing a voting close timestamp.",
			);
		}

		await this.dependencies.stateManager.addIncident(
			input.sessionId,
			incident,
		);
		...
		const event: GameEvent = {
			eventId:
				this.dependencies.idGenerator.createEventId(),
		...
		const voteOpened: GameEvent = {
			actionIds: incident.actionOptions.map(
				(action) => action.id,
			),
		...
		await this.dependencies.eventBus.publishAll([
			event,
			voteOpened,
		]);
	...
	public async submitVote(
		...
		const player = session.state.players.get(
			input.playerId,
		);
		...
		const action = incident.actionOptions.find(
			(option) => option.id === input.actionId,
		);
		...
		const stateResult =
			await this.dependencies.stateManager.upsertVote(
				input.sessionId,
				{
					actionId: action.id,
					incidentId: incident.id,
	...
	public useInstantAction(
		...
		EngineCommandResult<{ readonly message: string }>
	> {
		const session =
			this.dependencies.stateManager.getSnapshot(
				input.sessionId,
		...
		const action = incident.instantActionOptions.find(
			(option) => option.id === input.actionId,
		);
		...
		return Promise.resolve({
			...
			result: {
				events: [],
				value: {
					message: this.instantActionMessage(
						action,
						incident,
					),
				},
	...
	private async resolveIncident(
		...
		const succeeded =
			this.dependencies.randomSource.nextFloat() <= chance;
		const delta = succeeded
			? action.success.immediate
			: action.failure.immediate;
		const afterStats =
			await this.dependencies.stateManager.applyStatDelta(
				sessionId,
				delta,
			);
		...
		if (!succeeded) {
			...
			events.push(
				...this.escalationDirector.escalateIfNeeded(
					sessionId,
					updatedSession.value,
				),
			);
			const chainEvent =
				this.chainReactionSystem.maybeSchedule(
					sessionId,
					incident.id,
					updatedSession.value,
				);
					...
					.finally(() => {
						if (this.isSessionActive(sessionId)) {
							this.scheduleIncidentTick(
								sessionId,
								this.escalationDirector.nextIncidentDelayMs(
									sessionId,
								),
							);
						}
	...
	private async endActiveSession(
		...
		const session =
			this.dependencies.stateManager.getSnapshot(sessionId);
		...
		await this.dependencies.stateManager.endSession(
			sessionId,
			reason,
			endedAt,
		);
	...
	private createVoteClosedEvent(
		...
		return selectedActionId === undefined
			? base
			: { ...base, selectedActionId };
	}
	...
	private requireSession(
		...
		if (session === undefined) {
			throw new Error(
				"Session disappeared during gameplay command.",
			);
		}
	...
	private cancelVoteTimer(
		incidentId: Incident["id"],
	): void {
		const timer =
			this.voteTimerByIncidentId.get(incidentId);
	...
	private clearVoteTimersForSession(
		sessionId: SessionId,
	): void {
		for (const [incidentId, timer] of this
			.voteTimerByIncidentId) {
	...
	private isSessionActive(sessionId: SessionId): boolean {
		const session =
			this.dependencies.stateManager.getSnapshot(sessionId);
		return (
			session !== undefined && isActiveSession(session)
		);
	}
