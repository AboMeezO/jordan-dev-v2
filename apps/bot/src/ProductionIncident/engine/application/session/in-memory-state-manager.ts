...
export class InMemoryStateManager implements StateManager {
	...
	public addIncident(
		...
		if (
			...
			throw new Error(
				"Cannot add an incident outside an active session state.",
			);
		}

		const activeIncidents = new Map(
			session.state.activeIncidents,
		);
	...
	public applyStatDelta(
		...
		if (session.state.status === "ended") {
			throw new Error(
				"Cannot update stats after a session has ended.",
			);
		}
		...
		const updated: GameSession = {
			...
			stats: {
				developerSanity: this.clampStat(
					session.stats.developerSanity +
						(delta.developerSanity ?? 0),
				),
				infrastructureCost: Math.max(
					0,
					session.stats.infrastructureCost +
						(delta.infrastructureCost ?? 0),
				),
				serverStability: this.clampStat(
					session.stats.serverStability +
						(delta.serverStability ?? 0),
				),
				userHappiness: this.clampStat(
					session.stats.userHappiness +
						(delta.userHappiness ?? 0),
				),
			},
		};
	...
	public assignRoles(
		...
		if (session.state.status === "ended") {
			throw new Error(
				"Cannot assign roles after a session has ended.",
			);
		}
		...
		for (const [
			playerId,
			player,
		] of session.state.players.entries()) {
	...
	public endSession(
		...
		return Promise.resolve(
			this.withoutEvents(endedSession),
		);
	}
	...
	public joinSession(
		...
		if (session.state.status !== "waiting") {
			throw new Error(
				"Players can only join waiting sessions.",
			);
		}
	...
	public removeIncident(
		...
		if (
			...
			throw new Error(
				"Cannot remove an incident outside an active session state.",
			);
		}
	...
	public removePlayer(
		...
		if (session.state.status === "ended") {
			throw new Error(
				"Cannot remove players from ended sessions.",
			);
		}
		...
		switch (session.state.status) {
			case "ended":
				return {
					...
					state: {
						...session.state,
						incidentHistory: cloneIncidentMap(
							session.state.incidentHistory,
						),
						players: new Map(session.state.players),
					},
			...
			case "running":
				return {
					...
					state: {
						...session.state,
						activeIncidents: cloneIncidentMap(
							session.state.activeIncidents,
						),
						incidentHistory: cloneIncidentMap(
							session.state.incidentHistory,
						...
						voteWindows: cloneVoteWindowMap(
							session.state.voteWindows,
						),
					},
				};
			case "waiting":
	...
	private withoutEvents(
		session: GameSession,
	): StateMutationResult<GameSession> {
		return {
			events: [],
...
function isTerminalIncidentStatus(
...
function cloneActionEffect(
	...
	return effect.delayedEffects === undefined
		...
		: {
				...cloned,
				delayedEffects: effect.delayedEffects.map(
					(delta) => ({ ...delta }),
				),
			};
}
...
function cloneIncident(incident: Incident): Incident {
	const base = {
		...
		instantActionOptions:
			incident.instantActionOptions.map(cloneAction),
	};
...
function clonePlayerMap(
	...
	return new Map(
		[...players].map(([id, player]) => [id, { ...player }]),
	);
}
	...
	return new Map(
		[...voteWindows].map(([id, voteWindow]) => [
			id,
			{
				...voteWindow,
				votesByPlayerId: new Map(
					[...voteWindow.votesByPlayerId].map(
						([playerId, vote]) => [
							playerId,
							cloneVote(vote),
						],
