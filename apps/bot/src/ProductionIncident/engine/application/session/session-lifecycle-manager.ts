...
export class SessionLifecycleManager implements SessionManager {
	...
	public async createSession(
		...
		const stateResult =
			await this.dependencies.stateManager.createSession(
				sessionId,
	...
	public getSession(
		...
		return this.dependencies.stateManager.getSnapshot(
			sessionId,
		);
	}
	...
	public async joinSession(
		...
		const stateResult =
			await this.dependencies.stateManager.joinSession(
				input.sessionId,
				input.player,
			);
	...
	public async removePlayer(
		...
		const stateResult =
			await this.dependencies.stateManager.removePlayer(
				sessionId,
				playerId,
			);
		const occurredAt = this.dependencies.clock.now();
	...
	public async startSession(
		...
		const minimumPlayers =
			input.minimumPlayers ?? DEFAULT_MINIMUM_PLAYERS;
		...
		await this.dependencies.stateManager.assignRoles(
			input.sessionId,
			assignments,
		);
		...
		const stateResult =
			await this.dependencies.stateManager.startSession(
				input.sessionId,
				startedAt,
			);
		...
		return this.publishSuccess(stateResult.value, [
			rolesAssigned,
			event,
		]);
	}
