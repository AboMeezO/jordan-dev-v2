...
export interface SessionCreatedEvent extends GameEventBase<"session.created"> {
...
export interface RolesAssignedEvent extends GameEventBase<"roles.assigned"> {
	readonly assignments: ReadonlyMap<PlayerId, RoleId>;
}

export interface SessionStartedEvent extends GameEventBase<"session.started"> {
...
export interface IncidentGeneratedEvent extends GameEventBase<"incident.generated"> {
...
export interface VoteRegisteredEvent extends GameEventBase<"vote.registered"> {
...
export interface IncidentResolvedEvent extends GameEventBase<"incident.resolved"> {
	readonly actionId: ActionId;
...
export interface EscalationUpdatedEvent extends GameEventBase<"escalation.updated"> {
...
export interface ChainReactionScheduledEvent extends GameEventBase<"chainReaction.scheduled"> {
	readonly delayMs: number;
	readonly depth: number;
	readonly sourceIncidentId: IncidentId;
}

export interface CommentaryCuedEvent extends GameEventBase<"commentary.cued"> {
	readonly message: string;
	readonly priority: "high" | "low" | "normal";
	readonly sourceEventType: Exclude<
		GameEventType,
		"commentary.cued"
	>;
}

export interface StatisticsUpdatedEvent extends GameEventBase<"statistics.updated"> {
...
export type GameEventOfType<TType extends GameEventType> =
	Extract<GameEvent, { readonly type: TType }>;

export function assertNever(value: never): never {
	throw new Error(
		`Unhandled discriminated union member: ${String(value)}`,
	);
}
