...
export class CatalogValidationError extends Error {
	public constructor(
		...
		super(
			`Production Incident catalog validation failed:\n${failures.join("\n")}`,
		);
...
function validateActions(
	...
	for (const action of actions) {
		...
		if (action.label.trim().length === 0) {
			failures.push(
				`Action ${action.id} must have a label.`,
			);
		}

		if (action.tags.length === 0) {
			failures.push(
				`Action ${action.id} must include at least one tag.`,
			);
		}

		if (action.successRate < 0 || action.successRate > 1) {
			failures.push(
				`Action ${action.id} successRate must be between 0 and 1.`,
			);
		}
	}
...
function validateTemplates(
	...
	if (templates.length === 0) {
		failures.push(
			"Incident template catalog must not be empty.",
		);
	}
	...
	for (const template of templates) {
		if (seen.has(template.id)) {
			failures.push(
				`Duplicate incident template ID: ${template.id}`,
			);
		}
		...
		if (
			...
			failures.push(
				`Template ${template.id} weight must be positive.`,
			);
		}

		if (template.severityRange.length === 0) {
			failures.push(
				`Template ${template.id} must include at least one severity.`,
			);
		}

		if (template.rootCauses.length === 0) {
			failures.push(
				`Template ${template.id} must include root causes.`,
			);
		}

		if (template.affectedServices.length === 0) {
			failures.push(
				`Template ${template.id} must include affected services.`,
			);
		}

		if (template.titles.length === 0) {
			failures.push(
				`Template ${template.id} must include titles.`,
			);
		}

		if (template.descriptions.length === 0) {
			failures.push(
				`Template ${template.id} must include descriptions.`,
			);
		}

		if (template.actionTags.length === 0) {
			failures.push(
				`Template ${template.id} must include action tags.`,
			);
		}
		...
		for (const tag of template.actionTags) {
			if (!actionTags.has(tag)) {
				failures.push(
					`Template ${template.id} references missing action tag: ${tag}`,
				);
			}
		}
	}
...
function validateReachability(
	...
	for (const action of actions) {
		if (!reachableActionIds.has(action.id)) {
			failures.push(
				`Action ${action.id} is unreachable from all templates.`,
			);
		}
	}
