...
export type EngineCommandResult<TValue> =
  | { readonly error: SessionLifecycleError; readonly ok: false }
  | { readonly ok: true; readonly result: EngineMutationResult<TValue> };

export interface SessionManager {
  cancelSession(sessionId: SessionId): Promise<EngineCommandResult<GameSession>>;
  createSession(
    input: CreateSessionInput,
  ): Promise<EngineCommandResult<GameSession>>;
  endSession(input: EndSessionInput): Promise<EngineCommandResult<GameSession>>;
  getSession(sessionId: SessionId): GameSession | undefined;
  joinSession(input: JoinSessionInput): Promise<EngineCommandResult<GameSession>>;
  removePlayer(
    sessionId: SessionId,
    playerId: PlayerId,
  ): Promise<EngineCommandResult<GameSession>>;
