...
const encodedVote = codec.encodeVote({
	actionId,
	incidentId,
	sessionId,
});
...
assert.throws(() =>
	codec.encodeVote({
		actionId:
			"action-with-a-really-long-identifier-that-would-break-discord-buttons" as ActionId,
		incidentId:
			"incident-with-a-really-long-identifier-that-would-break-discord-buttons" as IncidentId,
		sessionId:
			"session-with-a-really-long-identifier-that-would-break-discord-buttons" as SessionId,
	}),
);

const encodedLobby = codec.encodeLobby({
	action: "join",
	sessionId,
});
...
assert.throws(() =>
	codec.decodeLobby("pi:v1:lobby:session:launch"),
);
...
assert.equal(
	registry.getByChannel("channel-1")?.sessionId,
	sessionId,
);
registry.setIncidentMessageId(
	sessionId,
	incidentId,
	"message-incident",
);
assert.equal(
	registry.getIncidentMessageId(sessionId, incidentId),
	"message-incident",
);
...
const encodedAction = codec.encodeAction({
	key: actionRoute.key,
});
...
assert.equal(
	registry.getActionRoute(actionRoute.key, 15 as UnixMillis)
		?.actionId,
	actionId,
);
assert.equal(
	registry.getActionRoute(
		actionRoute.key,
		21 as UnixMillis,
	),
	undefined,
...
assert.equal(
	registry.getActionRoute(staleRoute.key),
	undefined,
);
...
assert.equal(
	guildOnlyEmojiSummary.missing.includes("incident"),
	true,
);
assert.notEqual(
	emojiRegistry.emoji("incident"),
	"<:pi_incident:guild-incident-emoji>",
);
...
assert.equal(
	applicationEmojiSummary.found.includes("incident"),
	true,
);
assert.equal(
	emojiRegistry.emoji("incident"),
	"<:pi_incident:application-incident-emoji>",
);
const prompt = renderer.renderIncidentPrompt(
	...
	() => encodedAction,
	() => codec.encodeInstant({ key: actionRoute.key }),
	{
...
assert.equal(
	prompt.buttonRows?.[0]?.buttons[0]?.customId,
	encodedAction,
);
assert.match(
	prompt.buttonRows?.[0]?.buttons[0]?.label ?? "",
	/\(1\)$/,
);
...
assert.equal(
	terminalPrompt.buttonRows?.[0]?.buttons[0]?.disabled,
	true,
);
assert.throws(() =>
	codec.decodeAction("pi:v1:a:invalid:key"),
);
assert.deepEqual(
	codec.decodeInstant(
		codec.encodeInstant({ key: actionRoute.key }),
