...
// 1. removePlayer flow
const harness1 = createPhase1Harness(
...
requireOk(
	await k1.sessionManager.joinSession({
		player: p1,
		sessionId: sid1,
	}),
);
...
assert.equal(
	afterRemove.value.state.players.has(p2.id),
	true,
);
assert.equal(
	afterRemove.value.state.players.has(p1.id),
	false,
);
...
const endHarness = createPhase1Harness(
	"player-actions-end-remove",
);
...
requireOk(
	await kEnd.sessionManager.joinSession({
		player: endPlayer,
		sessionId: endSid,
	}),
);
requireOk(
	await kEnd.sessionManager.endSession({
		reason: "shutdown",
		sessionId: endSid,
	}),
);
const removeAfterEnd = requireFailure(
	await kEnd.sessionManager.removePlayer(
		endSid,
		endPlayer.id,
	),
);
...
const harness2 = createPhase1Harness(
...
requireOk(
	await k2.sessionManager.joinSession({
		player: pa,
		sessionId: sid2,
	}),
);
...
requireOk(
	await k2.sessionManager.startSession({
		minimumPlayers: 2,
		sessionId: sid2,
	}),
);
...
const gen2 = requireOk(
	await k2.gameplayManager.generateIncident({
		sessionId: sid2,
	}),
).value;
const instantAction2 = gen2.instantActionOptions[0];
if (instantAction2 === undefined) {
	throw new Error("Expected instant action");
}
...
console.log(
	"player-actions.check.ts: useInstantAction passed",
);
