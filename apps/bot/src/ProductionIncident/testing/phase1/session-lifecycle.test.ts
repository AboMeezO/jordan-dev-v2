...
const lifecycleHarness = createPhase1Harness(
	"phase-1-lifecycle",
);
...
assert.equal(
	joinedFirst.value.state.players.has(firstPlayer.id),
	true,
);
...
assert.equal(
	joinedSecond.value.state.players.has(secondPlayer.id),
	true,
);
...
assert.equal(
	scheduler.activeTasks.some(
		(task) =>
			task.delayMs === 750 && task.sessionId === sessionId,
	),
	true,
);
assertSessionStatus(
...
assertSessionStatus(
	kernel.sessionManager.getSession(sessionId),
	"ended",
);
...
const cancelled = requireOk(
	await cancelHarness.kernel.sessionManager.cancelSession(
		cancelCreated.value.id,
	),
);
...
assertEventTypes(cancelHarness.events, [
	"session.created",
	"session.ended",
]);
