...
const startBeforePlayersHarness = createPhase1Harness(
	"phase-1-invalid-start",
);
...
assert.equal(
	minimumPlayersError.code,
	"minimum-players-not-met",
);
assertSessionStatus(
	startBeforePlayersHarness.kernel.stateManager.getSnapshot(
		emptySession.value.id,
	),
	"waiting",
);
assertEventTypes(startBeforePlayersHarness.events, [
	"session.created",
]);
assert.equal(
	startBeforePlayersHarness.scheduler.activeTasks.length,
	0,
);

const duplicateJoinHarness = createPhase1Harness(
	"phase-1-duplicate-join",
);
...
assert.equal(
	duplicateJoinHarness.kernel.stateManager.getSnapshot(
		duplicateSession.value.id,
	)?.state.players.size,
	1,
);
assertEventTypes(duplicateJoinHarness.events, [
	"session.created",
	"player.joined",
]);

const endedSessionHarness = createPhase1Harness(
	"phase-1-ended-session",
);
...
assertSessionStatus(
	endedSessionHarness.kernel.stateManager.getSnapshot(
		created.value.id,
	),
	"ended",
);
...
const missingSessionHarness = createPhase1Harness(
	"phase-1-missing-session",
);
