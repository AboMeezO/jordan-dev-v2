...
if (selectedAction === undefined) {
	assert.fail(
		"Expected generated incident to include action options.",
	);
}
...
assert.equal(
	closed.value.events.some(
		(event) => event.type === "incident.resolved",
	),
	true,
);
assert.equal(
	events.some(
		(event) => event.type === "statistics.updated",
	),
	true,
);
assert.equal(
...
assert.equal(
	finalSession.state.incidentHistory.get(incident.id)
		?.selectedActionId,
	selectedAction.id,
);
assert.equal(
	finalSession.state.activeIncidents.has(incident.id),
	false,
);
assert.equal(
	finalSession.state.voteWindows.has(incident.id),
	false,
);
