...
for (let index = 0; index < 10; index += 1) {
	...
	const action =
		incident.actionOptions[
			index % incident.actionOptions.length
		];
		...
		for (const player of [
			...
			requireOk(
				await kernel.gameplayManager.submitVote({
					...
					playerId: idGenerator.createPlayerId(
...
assert.equal(
	events.some((event) => event.type === "incident.failed"),
	true,
);
assert.equal(
	events.some(
		(event) => event.type === "chainReaction.scheduled",
	),
	true,
);
assert.equal(
	events.some(
		(event) => event.type === "escalation.updated",
	),
	true,
);
