...
assert.deepEqual(parsed.segments[0]?.operators, [{ index: 5, value: "|" }]);
assert.deepEqual(parsed.segments[1]?.words, ["grep", "fail"]);

const registry = new ChatCommandRegistry();
registry.register(commandTree({
...
assert.deepEqual(resolution.invocation.options.channel, ["ops"]);
assert.deepEqual(resolution.invocation.positionalArgs, ["quoted reason"]);
...
assert.deepEqual(prefixlessResolution.invocation.commandPath, [
  "dev",
  "incident",
  "start",
]);
...
const prefixOnly = parseChatCommandInput("remind later 10m deploy", "");

assert.ok(prefixOnly);
assert.equal(registry.resolve(prefixOnly)?.allowPrefixless, false);
assert.equal(registry.find(["remind", "later"], "!")?.invocation.prefix, "!");
...
assert.ok(reminderGuide.includes("- `!reminders` - Open reminders."));
assert.ok(!reminderGuide.includes("!reminders !reminders"));

const prefixedLookalike = parseChatCommandInput("!remindera 10m nope", "!");
