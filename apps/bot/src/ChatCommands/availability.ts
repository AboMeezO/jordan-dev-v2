...
function checkDmAvailability(
	...
	if (scope?.contexts && !scope.contexts.includes("dm")) {
		return {
			allowed: false,
			reason: "This command cannot be used in DMs.",
		};
	}
	...
	if (
		...
		!ownerIds.has(message.author.id)
	) {
		return {
			allowed: false,
			reason:
	...
	if (
		...
		return {
			allowed: false,
			reason:
				"This command requires a server with appropriate permissions.",
		};
	}
...
function checkGuildAvailability(
	...
	if (!guild) {
		return {
			allowed: false,
			reason: "Unexpected error: guild context missing.",
		};
	}

	if (
		scope?.contexts &&
		!scope.contexts.includes("guild")
	) {
		return {
			allowed: false,
			reason: "This command cannot be used in servers.",
		};
	}

	if (
		scope?.guildIds?.deny &&
		scope.guildIds.deny.includes(guild.id)
	) {
		return {
			allowed: false,
			reason: "This command is not allowed in this server.",
		};
	}

	if (
		scope?.guildIds?.allow &&
		!scope.guildIds.allow.includes(guild.id)
	) {
		return {
			allowed: false,
			reason: "This command is not allowed in this server.",
	...
	if (
		scope?.channelIds?.deny &&
		channel &&
		"id" in channel &&
		scope.channelIds.deny.includes(channel.id)
	) {
		return {
			allowed: false,
			reason:
	...
	if (
		scope?.channelIds?.allow &&
		channel &&
		"id" in channel &&
		!scope.channelIds.allow.includes(channel.id)
	) {
		return {
			allowed: false,
			reason:
	...
	if (
		scope?.categoryIds?.deny ||
		scope?.categoryIds?.allow
	) {
		const parentId =
			"parentId" in channel && channel.parentId
				? channel.parentId
				: undefined;
		...
		if (parentId) {
			if (
				scope.categoryIds.deny &&
				scope.categoryIds.deny.includes(parentId)
			) {
				return {
					allowed: false,
					reason:
						"This command is not allowed in this category.",
				};
			}

			if (
				scope.categoryIds.allow &&
				!scope.categoryIds.allow.includes(parentId)
			) {
				return {
					allowed: false,
					reason:
			...
			return {
				allowed: false,
				reason:
					"This command is not allowed outside specific categories.",
			};
		}
	}
	...
	if (scope?.roleIds?.deny || scope?.roleIds?.allow) {
		...
		if (member) {
				...
				for (const roleId of scope.roleIds.deny) {
					if (memberRoleIds.has(roleId)) {
						return {
							allowed: false,
							reason:
								"You have a role that cannot use this command.",
						};
					}
				}
			}
			...
			if (scope.roleIds?.allow) {
				const hasAllowed = scope.roleIds.allow.some((id) =>
					memberRoleIds.has(id),
				);
...
function checkUserAllowDeny(
	...
	if (
		scope?.userIds?.deny &&
		scope.userIds.deny.includes(userId)
	) {
		return {
			allowed: false,
	...
	if (
		scope?.userIds?.allow &&
		!scope.userIds.allow.includes(userId)
	) {
		return {
			allowed: false,
			reason: "You are not allowed to use this command.",
