...
export function safeInline(text: string, maxLength = MAX_INLINE_LENGTH): string {
...
export function safeCodeBlock(
  ...
  const available = maxLength - codeFence.length - "\n```".length;
...
export function errorOutput(message: string): string {
  return escapeMentions(message).slice(0, MAX_INLINE_LENGTH);
}
...
export function formatErrorBoundary(error: unknown): string {
