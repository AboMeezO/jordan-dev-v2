...
import { parseChatCommandInput, ChatCommandRegistry } from "#ChatCommands";
...
for (const path of knownCommands) {
  ...
  assert.ok(resolved, `Failed to resolve: ${path.join(" ")}`);
  assert.deepEqual(
...
const aliasTest = parseChatCommandInput("gh repo octocat/Hello-World", "");
assert.ok(aliasTest);
const resolvedAlias = registry.resolve(aliasTest);
assert.ok(resolvedAlias);
assert.equal(resolvedAlias.invocation.commandPath[0], "github");

assert.equal(
  registry.find(["nonexistent", "command"], "")?.invocation.commandPath,
  undefined,
);

const allPaths = knownCommands.map((path) => path.join(" "));
...
for (const raw of allPaths) {
  ...
  assert.equal(
    r.command.name,
    r.invocation.commandPath[r.invocation.commandPath.length - 1],
    `Name mismatch for: ${raw}`,
  );

  assert.ok(
    typeof r.command.description === "string" && r.command.description.length > 0,
