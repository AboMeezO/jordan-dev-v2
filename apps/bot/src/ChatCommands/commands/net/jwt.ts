...
function decodeJWT(token: string): Record<string, unknown> {
  ...
  try {
    ...
    throw new Error("Failed to parse JWT segments as JSON.");
  }
}
...
export const jwtCommand = subcommand({
  ...
  usage: {
    formats: ["jwt <token>", "jwt <token> --verify <jwks-url>"],
    arguments: [
      { name: "token", description: "JWT token to decode.", required: true },
    ],
    examples: [
      { command: "jwt eyJhbGciOiJIUzI1NiIs...", description: "Decode a JWT token." },
    ],
  },
  async execute({ invocation, message }) {
    const verifyUrl = invocation.options.verify as string | undefined;
    const raw = invocation.positionalArgs.filter((a) => !a.startsWith("--")).join(" ").trim();
    ...
    try {
      const decoded = decodeJWT(parsed.data.token) as { header: Record<string, unknown>; payload: Record<string, unknown> };
      ...
      if (verifyUrl) {
        try {
          const JWKS = createRemoteJWKSet(new URL(verifyUrl));
      ...
      await message.reply(`JWT decode failed: ${err.message}`);
    }
  },
});
