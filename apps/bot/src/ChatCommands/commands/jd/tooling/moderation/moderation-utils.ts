...
export const BAN_PERMISSION = PermissionFlagsBits.BanMembers;
export const KICK_PERMISSION = PermissionFlagsBits.KickMembers;
export const TIMEOUT_PERMISSION = PermissionFlagsBits.ModerateMembers;
export const AUDIT_PERMISSION = PermissionFlagsBits.ViewAuditLog;
...
export function parseUserId(input: string): string | undefined {
...
export function parseModerationArgs(args: readonly string[]): {
  ...
  return {
    rest,
    targetId: targetInput ? parseUserId(targetInput) : undefined,
  };
}

export function parseReason(args: readonly string[]): string {
...
export async function requireGuild(
  ...
  if (!guild) {
    await context.message.reply(moderationDenyOutput(
      "guild required",
...
export async function resolveModerationTarget(
  ...
  const member = await guild.members.fetch(targetId).catch(() => undefined);
  const user =
    member?.user ?? (await context.client.users.fetch(targetId).catch(() => undefined));
...
export function targetLabel(target: ModerationTarget): string {
  return target.member?.user.tag ?? target.user?.tag ?? target.id;
}

export function roleNamesForMember(member: GuildMember): readonly string[] {
...
export function permissionNamesForMember(member: GuildMember): readonly string[] {
...
export async function ensureCanModerateMember(
  ...
  if (
    caller &&
    member.roles.highest.position >= caller.roles.highest.position &&
    member.id !== caller.id
  ) {
    await context.message.reply(moderationDenyOutput(
...
export function moderationFailureOutput(action: string, error: unknown): string {
