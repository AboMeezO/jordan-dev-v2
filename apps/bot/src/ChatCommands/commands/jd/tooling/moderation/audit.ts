...
export const auditCommand = subcommand({
  ...
  usage: {
    examples: [
      {
        ...
        description: "Show the latest ten audit log entries.",
      },
      {
    ...
    useCases: ["Inspect recent moderation and server management activity."],
  },
  async execute(context) {
    ...
    if (
      !(await requirePermission(context, AUDIT_PERMISSION, "view audit log")) ||
      !(await requireBotPermission(context, guild, AUDIT_PERMISSION, "view audit log"))
    ) {
      return;
    }

    const requestedLimit = Number(context.invocation.rawArgs[0] ?? 10);
    const limit = Number.isSafeInteger(requestedLimit)
      ? Math.min(Math.max(requestedLimit, 1), 25)
      : 10;
    const logs = await guild.fetchAuditLogs({ limit }).catch(async (error: unknown) => {
      await context.message.reply(moderationFailureOutput("audit", error));
      return undefined;
    });
    ...
    if (entries.length === 0) {
      await context.message.reply(shellOutput(["audit=empty"]));
      return;
    }
