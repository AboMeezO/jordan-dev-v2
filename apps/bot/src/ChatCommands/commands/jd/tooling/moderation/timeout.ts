...
export const timeoutCommand = subcommand({
  ...
  usage: {
    arguments: [
      { description: "Member mention or Discord user ID.", name: "member" },
      { description: "Duration such as `10m`, `2h`, or `7d`.", name: "duration" },
      { description: "Reason for the timeout.", name: "reason", required: false },
    ],
    examples: [
      {
        command: "jd tools moderation timeout @user 10m cooldown",
        description: "Timeout a member for ten minutes.",
      },
    ],
    formats: ["jd tools moderation timeout <member> <duration> [reason]"],
    notes: [
      ...MODERATION_USAGE_NOTES,
      "Discord timeouts cannot exceed 28 days.",
    ],
    useCases: ["Temporarily stop a member from sending messages."],
  },
  async execute(context) {
    ...
    if (
      !(await requirePermission(context, TIMEOUT_PERMISSION, "moderate members")) ||
      !(await requireBotPermission(context, guild, TIMEOUT_PERMISSION, "moderate members"))
    ) {
      return;
    }

    const { rest, targetId } = parseModerationArgs(context.invocation.rawArgs);
    ...
    if (!(await ensureCanModerateMember(context, target, "timeout"))) {
      return;
    }

    const reason = parseReason(reasonArgs);
    try {
      await target.member.timeout(durationMs, moderationReason(context, reason));
    } catch (error) {
      await context.message.reply(moderationFailureOutput("timeout", error));
      return;
    }
