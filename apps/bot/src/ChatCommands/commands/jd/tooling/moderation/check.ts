...
export const checkCommand = subcommand({
	description:
		"Check moderation action availability for a user.",
	name: "check",
		...
		examples: [
			{
				...
				description:
					"Check whether the bot can ban, kick, or timeout the member.",
			},
		],
		formats: ["jd tools moderation check <user>"],
		useCases: [
			"Verify hierarchy and permissions before moderation actions.",
		],
	},
	async execute(context) {
		...
		const { targetId } = parseModerationArgs(
