...
export const banCommand = subcommand({
  ...
  usage: {
    arguments: [
      ...
      { description: "Reason for the ban.", name: "reason", required: false },
    ],
    examples: [
      {
        command: "jd tools moderation ban @user repeated spam",
        description: "Ban a member and write the reason to audit logs.",
      },
    ],
  ...
  async execute(context) {
    ...
    if (!target || !(await ensureCanModerateMember(context, target, "ban"))) {
      return;
    }
    ...
    try {
      ...
      await context.message.reply(moderationFailureOutput("ban", error));
      return;
    }
