...
export const base64Command = subcommand({
	...
	usage: {
		...
		examples: [
			{
				command: 'base64 encode "hello world"',
				description: "Encode text to Base64.",
			},
			{
				command: 'base64 decode "aGVsbG8="',
				description: "Decode Base64 to text.",
			},
		],
		useCases: [
			"Quickly encode or decode Base64 without leaving Discord.",
		],
	},
	async execute({ invocation, message }) {
		...
		try {
			if (result.data.mode === "encode") {
				...
				if (hasBinary) {
					await message.reply(
						"Decoded output contains binary data and cannot be displayed as text.",
					);
					return;
				}
			...
			await message.reply(
				"Invalid input. Check your text or Base64 encoding.",
			);
		}
	},
});
