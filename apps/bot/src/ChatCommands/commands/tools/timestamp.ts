...
export const timestampCommand = subcommand({
	...
	description:
		"Generate Discord timestamps from dates or relative durations.",
		...
		arguments: [
			{
				name: "input",
				description:
					"now, a date string, or relative duration like 2h, 30m, 7d.",
				required: false,
			},
		],
		options: [
		...
		examples: [
			{
				...
				description: "Current Discord timestamp.",
			},
			{
				command: "timestamp 2h",
				description: "Timestamp 2 hours from now.",
			},
			{
				command: "timestamp 2025-01-01 --style=R",
				description: "Relative timestamp for a date.",
			},
		],
		useCases: [
			"Create Discord timestamps for reminders or events.",
			"Convert relative durations to absolute timestamps.",
		],
	},
	async execute({ invocation, message }) {
		const input =
			invocation.positionalArgs.join(" ").trim() || "now";
		const style =
			extractOptionString(invocation.options, "style") ??
			"F";

		if (
			!DISCORD_STYLES.includes(
				style as (typeof DISCORD_STYLES)[number],
			)
		) {
			await message.reply(
		...
		if (input === "now") {
			...
			const durationMs = ms(
				input as Parameters<typeof ms>[0],
			);
