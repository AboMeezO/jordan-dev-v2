...
export const snowflakeCommand = subcommand({
  ...
  usage: {
    ...
    arguments: [{ name: "id", description: "Discord snowflake ID, mention, or channel/role mention.", required: true }],
    examples: [
      { command: "snowflake 123456789012345678", description: "Decode a snowflake ID." },
      { command: "snowflake <@123456789012345678>", description: "Decode a user mention." },
    ],
    useCases: ["Find when a Discord ID was created.", "Get internal snowflake components."],
  },
  async execute({ invocation, message }) {
    const raw = invocation.positionalArgs[0] ?? "";
    const cleaned = raw.replace(/[<@!&#>]/g, "");

    const result = snowflakeSchema.safeParse({ input: cleaned });
    ...
    try {
      ...
      await message.reply("Invalid snowflake ID. IDs must be numeric Discord snowflakes.");
    }
  },
});
