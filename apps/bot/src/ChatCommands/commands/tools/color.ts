...
export const colorCommand = subcommand({
	...
	aliases: ["colour", "hex"],
	description:
		"Parse and convert colors between HEX, RGB, HSL, and decimal.",
	...
	usage: {
		...
		arguments: [
			{
				name: "input",
				description: "HEX, RGB, HSL, or decimal color.",
				required: true,
			},
		],
		examples: [
			{
				command: "color #5865F2",
				description: "Convert Discord blurple.",
			},
			{
				command: "color rgb(88, 101, 242)",
				description: "Convert RGB color.",
			},
			{
				command: "color 5793266",
				description: "Convert decimal color.",
			},
		],
		useCases: [
			"Convert colors for Discord embeds or roles.",
			"Preview color values in different formats.",
		],
	},
	async execute({ invocation, message }) {
		...
		if (!color.isValid()) {
			await message.reply(
				"Invalid color input. Try formats like: #5865F2, rgb(88,101,242), hsl(235,86%,65%), or 5793266.",
			);
			return;
		}
