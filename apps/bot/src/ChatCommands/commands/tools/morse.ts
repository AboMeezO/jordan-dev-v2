...
export const morseCommand = subcommand({
  ...
  usage: {
    ...
    arguments: [{ name: "input", description: "Text or Morse code to convert.", required: true }],
    options: [
      { name: "mode", description: "encode or decode", valueName: "mode" },
    ],
    examples: [
      { command: 'morse encode "hello world"', description: "Encode text to Morse code." },
      { command: 'morse decode ".... . .-.. .-.. --- / .-- --- .-. .-.. -.."', description: "Decode Morse to text." },
    ],
  ...
  async execute({ invocation, message }) {
    ...
    try {
      ...
      await message.reply("An error occurred while processing Morse code.");
    }
  },
});
...
async function handleEncode(
  input: string,
  message: { reply: (content: string | object) => Promise<unknown> },
): Promise<void> {
  ...
  if (unsupported.length > 0) {
    ...
    await message.reply(safeInline(encoded + warning, 1900));
    return;
  }
...
async function handleDecode(
  ...
  for (const word of words) {
    ...
    for (const code of letters) {
      ...
      if (!letter) {
        await message.reply(`Unknown Morse code: \`${code}\``);
        return;
      }
  ...
  await message.reply(safeInline(decodedWords.join(" "), 1900));
}
