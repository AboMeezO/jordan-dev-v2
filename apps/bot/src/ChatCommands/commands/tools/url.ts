...
export const urlCommand = subcommand({
  ...
  usage: {
    ...
    arguments: [{ name: "input", description: "URL or text to process.", required: true }],
    options: [
      { name: "mode", description: "encode, decode, or parse", valueName: "mode" },
    ],
    examples: [
      { command: 'url encode "hello world"', description: "URL-encode text." },
      { command: 'url decode "hello%20world"', description: "URL-decode text." },
      { command: "url parse https://example.com/path?q=1#hash", description: "Parse a URL into components." },
    ],
    useCases: ["Debug URL encoding issues.", "Inspect URL components."],
  },
  async execute({ invocation, message }) {
    ...
    try {
      if (result.data.mode === "encode") {
        const encoded = encodeURIComponent(result.data.input);
        await message.reply(safeInline(encoded, 1900));
        return;
      }

      if (result.data.mode === "decode") {
        const decoded = decodeURIComponent(result.data.input);
      ...
      await message.reply(safeInline(parts.join("\n"), 1900));
    } catch {
      await message.reply("Invalid URL. Make sure it includes a protocol (e.g. https://).");
    }
  },
});
