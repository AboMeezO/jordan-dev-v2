...
export const whichCommand = commandTree({
    ...
    examples: [
      {
        ...
        description: "Show metadata for the whoami command.",
      },
      {
        command: "which json",
  ...
  async execute({ invocation, message, registry }) {
    const input = invocation.positionalArgs.join(" ").trim().toLowerCase();

    if (!input) {
      await message.reply(shellOutput(["usage=which <tool-name>"]));
      return;
    }

    const allCommands = collectAllCommands(registry.listRootCommands());

    const direct = allCommands.find(
      (cmd) => cmd.name.toLowerCase() === input || cmd.aliases?.some((a) => a.toLowerCase() === input),
    );
    ...
    if (direct) {
      ...
      if (direct.permission && direct.permission !== "public") {
      ...
      if (direct.availability?.contexts) {
        lines.push(`contexts=${direct.availability.contexts.join(", ")}`);
      }
...
interface FlatCommandInfo {
  ...
  readonly availability: { readonly contexts?: readonly string[] } | undefined;
}

function collectAllCommands(
  definitions: readonly { readonly name: string; readonly aliases?: readonly string[]; readonly category?: string; readonly description: string; readonly permission?: string; readonly enabled?: boolean; readonly ownerOnly?: boolean; readonly devOnly?: boolean; readonly cooldown?: number; readonly subcommands?: readonly unknown[]; readonly availability?: { readonly contexts?: readonly string[] } }[],
): FlatCommandInfo[] {
  const result: FlatCommandInfo[] = [];

  function walk(
    defs: readonly { readonly name: string; readonly aliases?: readonly string[]; readonly category?: string; readonly description: string; readonly permission?: string; readonly enabled?: boolean; readonly ownerOnly?: boolean; readonly devOnly?: boolean; readonly cooldown?: number; readonly subcommands?: readonly unknown[]; readonly availability?: { readonly contexts?: readonly string[] } }[],
  ): void {
