...
export const sudoCommand = commandTree({
  ...
  description: "Execute another bot chat command with administrator privileges.",
  name: "sudo",
    ...
    examples: [
      {
        ...
        description: "Run a moderator command as a server administrator.",
      },
      {
  ...
  async execute(context) {
    const target = context.invocation.rawArgs.join(" ").trim();

    if (target.length === 0) {
      await context.message.reply(shellOutput(["usage=sudo <command>"]));
      return;
    }

    const parsed =
      parseChatCommandInput(target, context.invocation.prefix || "!") ??
      parseChatCommandInput(target, "");

    if (!parsed) {
      await context.message.reply(shellOutput(["error=command not found"]));
      return;
    }
    ...
    if (parsed.prefix === "" && !resolution.allowPrefixless) {
      await context.message.reply(shellOutput(["error=prefix required"]));
      return;
    }

    if (resolution.invocation.commandPath[0] === "sudo") {
      await context.message.reply(shellOutput(["error=sudo recursion blocked"]));
      return;
    }
    ...
    if (
      ...
      await context.message.reply(shellOutput(["error=owner permission required"]));
      return;
    }

    if (comparePermissionLevels(resolution.permission, "administrator") > 0) {
      await context.message.reply(shellOutput(["error=permission denied"]));
      return;
    }
