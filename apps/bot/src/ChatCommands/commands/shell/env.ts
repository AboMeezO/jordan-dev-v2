...
import { channelKind, channelPath, shellOutput } from "./format.js";

export const envCommand = commandTree({
  allowPrefixless: true,
  description: "Print safe Discord command environment values.",
  name: "env",
  permission: "public",
  usage: {
    examples: [
      {
        command: "env",
        description: "Show safe server/channel/user context for command execution.",
      },
    ],
    formats: ["env"],
    notes: ["This command intentionally does not print process environment variables."],
    useCases: ["Debug command context without exposing secrets."],
  },
