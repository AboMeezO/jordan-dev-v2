	...
	usage: {
		...
		examples: [
			...
			},
			{
				command: "ls jd tools",
				description:
					"List subcommands below a nested command group.",
			},
		],
		formats: ["ls", "ls <command-path>"],
		useCases: [
			"Discover commands using shell-like syntax.",
		],
	},
	async execute(context) {
		if (context.invocation.positionalArgs.length === 0) {
			await context.message.reply(
				shellOutput(
					sortedRootCommands(context).map(
		...
		if (!resolution) {
			await context.message.reply(
				shellOutput(["not found"]),
			);
			return;
		}

		const names = resolution.subcommands.map(
			(command) => command.name,
		);
		await context.message.reply(
			shellOutput(names.length > 0 ? names : ["."]),
		);
	},
});
