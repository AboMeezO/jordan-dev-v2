...
export const githubUserCommand = subcommand({
  ...
  async execute({ invocation, message }) {
    ...
    try {
      ...
      infoLines.push("");
      infoLines.push(`**${user.public_repos}** public repos · **${user.followers}** followers · **${user.following}** following`);
      ...
      infoLines.push(`📅 Joined <t:${Math.floor(new Date(user.created_at).getTime() / 1000)}:D>`);
