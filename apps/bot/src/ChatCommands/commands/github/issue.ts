...
export const githubIssueCommand = subcommand({
		...
		arguments: [
			{
				name: "issue",
				description:
					"Issue in owner/repo#number format or URL.",
				required: true,
			},
		],
		examples: [
			{
				command: "github issue octocat/Hello-World#1",
				description: "Show issue #1.",
			},
		],
	},
	async execute({ invocation, message }) {
		...
		if (!match) {
			await message.reply(
				"Invalid format. Use owner/repo#number.",
			);
			return;
		}

		const [, owner, repo, issueNumberStr] = match as [
			string,
			string,
			string,
			string,
		...
		try {
			...
			const stateLabel =
				isPr &&
				issue.state === "closed" &&
				(issue as { merged?: boolean }).merged
					? "merged"
					: (issue.state ?? "open");
			const color =
				STATE_COLORS[stateLabel] ?? STATE_COLORS.open!;
			...
			if (issue.closed_at) {
				infoLines.push(
					`✅ Closed <t:${Math.floor(new Date(issue.closed_at).getTime() / 1000)}:D>`,
				);
			}
			...
			const container = new ContainerBuilder()
				...
				.addActionRowComponents(
					new ActionRowBuilder<ButtonBuilder>().addComponents(
						new ButtonBuilder()
							.setLabel(
								`View ${isPr ? "PR" : "Issue"} #${issue.number}`,
							)
			...
			if (status === 404) {
				await message.reply(
					"Issue not found. Check the repository and issue number.",
				);
			} else if (status === 403) {
				await message.reply(
				...
				await message.reply(
					"Could not fetch issue information.",
				);
			}
		}
	},
