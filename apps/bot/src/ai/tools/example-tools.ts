...
export const getServerRoles = createTool({
	name: "getServerRoles",
	description: "List all roles in the current Discord server with their member counts.",
...
export const getChannelTopic = createTool({
	...
	description: "Get the topic and description of any text channel in the server.",
	inputSchema: z.object({
		channelName: z.string().describe("The name of the channel to look up"),
	}),
	execute: async (ctx, input) => {
		...
		const channel = guild.channels.cache.find(
			(ch) => ch.name === input.channelName && ch.isTextBased(),
		);

		if (!channel) return `No text channel named "${input.channelName}" found.`;
		if (!("topic" in channel)) return `Channel "${input.channelName}" has no topic field.`;

		const topic = (channel as { topic: string | null }).topic;
		return topic
...
export const getMemberJoinDate = createTool({
	...
	description: "Get when a member joined the server and created their account.",
	inputSchema: z.object({
	...
	execute: async (ctx, input) => {
		...
		if (!member) return `No member found with ID ${input.userId}.`;

		const joined = member.joinedAt?.toLocaleDateString() ?? "unknown";
		const created = member.user.createdAt.toLocaleDateString();
