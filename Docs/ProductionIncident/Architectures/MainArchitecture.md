...
export interface IncidentActionConfig {
  ...
  allowedRoles?: string[];
  successRate: number;
  effects: { stability?: number; happiness?: number; sanity?: number };
  failEffects?: { stability?: number; happiness?: number; sanity?: number };
}
...
// src/templates/memory_leak.json
{
  ...
  "tags": ["MEMORY"],
  "rootCauses": ["recursive_job", "cache_overflow", "unclosed_connections"],
  "affectedServices": ["Authentication", "Payments", "Uploads"],
  "titles": ["Memory leak detected", "RAM usage exceeded", "Container memory exhaustion"],
  "descriptions": ["Memory consumption spiked rapidly", "One service reached memory limit"],
...
- **Message formatting:** Build messages using `EmbedBuilder` and `MessageActionRow`/`ButtonBuilder`.  For consistency, create a utility to style all embeds/buttons (use similar colors, truncation logic, etc.)【25†L579-L584】.  Example:
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription("Memory leak detected in Payments service")
    .setColor("#ff4444")
    .setTimestamp();
  const buttons = new ActionRowBuilder<ButtonBuilder>()
    .addComponents(
      new ButtonBuilder().setCustomId("restart_service").setLabel("Restart Service").setStyle(ButtonStyle.Primary),
      // ...other buttons
    );
...
The engine runs *in real time* on a loop of incidents. Key systems:
